 ...
 * Text Domain: masjid-app
 * Domain Path: /languages
 * Requires at least: 6.0
 * Tested up to: 7.0
 * Requires PHP: 8.1
 * Requires Plugins: awesome-events, muslim-prayer-times
...
define('MASJIDAPP_VERSION', '1.0.0');
define('MASJIDAPP_PLUGIN_FILE', __FILE__);
define('MASJIDAPP_PLUGIN_DIR', plugin_dir_path(__FILE__));
define('MASJIDAPP_PLUGIN_URL', plugin_dir_url(__FILE__));
define('MASJIDAPP_OPTION_KEY', 'masjidapp_settings');
define('MASJIDAPP_CREDENTIAL_OPTION_KEY', 'masjidapp_firebase_credentials');
define('MASJIDAPP_DB_VERSION', '1.1.0');

/**
 * Main Masjid App Plugin Class
 */
class Masjid_App_Plugin {
    ...
    public function init() {
        $this->load_includes();
        if (MASJIDAPP_DB_VERSION !== get_option('masjidapp_db_version')) {
            Masjid_App_Push_Notifications::create_table();
            update_option('masjidapp_db_version', MASJIDAPP_DB_VERSION, false);
        }
        if (!wp_next_scheduled('masjidapp_cleanup_push_jobs')) {
            wp_schedule_event(time() + HOUR_IN_SECONDS, 'daily', 'masjidapp_cleanup_push_jobs');
        }
    }
    ...
    private function load_includes() {
        ...
        require_once MASJIDAPP_PLUGIN_DIR . 'includes/class-settings.php';
        require_once MASJIDAPP_PLUGIN_DIR . 'includes/class-content-only-renderer.php';
        require_once MASJIDAPP_PLUGIN_DIR . 'includes/class-rest-api.php';

        new Masjid_App_API_Trace();
        new Masjid_App_Settings();
        new Masjid_App_Content_Only_Renderer();
        new Masjid_App_REST_API();
        new Masjid_App_Push_Notifications();
    }
...
Masjid_App_Plugin::get_instance();
