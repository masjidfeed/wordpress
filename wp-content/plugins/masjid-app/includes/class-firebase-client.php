...
use Masjid_App\Dependencies\Firebase\JWT\BeforeValidException;
use Masjid_App\Dependencies\Firebase\JWT\ExpiredException;
use Masjid_App\Dependencies\Firebase\JWT\JWK;
use Masjid_App\Dependencies\Firebase\JWT\JWT;
use Masjid_App\Dependencies\Firebase\JWT\Key;

class Masjid_App_Firebase_Client {
    ...
    const TOKEN_CACHE_KEY = 'masjidapp_firebase_access_token';
    const JWKS_CACHE_KEY = 'masjidapp_firebase_app_check_jwks';
    ...
    public function __construct(
        ...
        foreach (array('project_id', 'client_email', 'private_key') as $field) {
            if (empty($service_account[$field])) {
                throw new Masjid_App_Firebase_Exception(sprintf('The Firebase service account is missing the "%s" field.', $field));
            }
        }
    ...
    public function wordpress_transport(string $method, string $url, array $options): array {
        ...
        if (is_wp_error($response)) {
            throw new Masjid_App_Firebase_Api_Connection_Failed($response->get_error_message());
        }
        $headers = array();
    ...
    public function access_token(): string {
        ...
        if ($response['status'] >= 400) {
            throw new Masjid_App_Firebase_Authentication_Error($this->error_message($response, 'Could not fetch a Firebase OAuth access token.'));
        }
        $data = json_decode($response['body'], true);
        if (empty($data['access_token'])) {
            throw new Masjid_App_Firebase_Authentication_Error($this->error_message($response, 'The OAuth token response did not contain an access token.'));
        }
    ...
    public function verify_app_check_token(string $token): array {
        ...
        try {
            ...
            throw new Masjid_App_Firebase_Invalid_App_Check_Token($exception->getMessage(), 0, $exception);
        } catch (Throwable $exception) {
            throw new Masjid_App_Firebase_Failed_Verify_App_Check_Token($exception->getMessage(), 0, $exception);
        }
        $payload = array(
        ...
        if (!in_array('projects/' . $this->service_account['project_id'], $payload['aud'], true)) {
            throw new Masjid_App_Firebase_Failed_Verify_App_Check_Token('The "aud" claim must include the project ID.');
        }
        if (!str_starts_with($payload['iss'], self::APP_CHECK_ISSUER_PREFIX)) {
            throw new Masjid_App_Firebase_Failed_Verify_App_Check_Token('The provided App Check token has incorrect "iss" (issuer) claim.');
        }
        return $payload;
    }
    ...
    private function app_check_keys(bool $force_refresh): array {
        ...
        if (!is_string($jwks_json) || '' === $jwks_json) {
            ...
            if ($response['status'] >= 400) {
                throw new Masjid_App_Firebase_Failed_Verify_App_Check_Token($this->error_message($response, 'Could not fetch the App Check public keys.'));
            }
        ...
        if (!is_array($parsed)) {
            throw new Masjid_App_Firebase_Failed_Verify_App_Check_Token('The App Check public keys response was not valid JSON.');
        }
        try {
    ...
    private function decode_json(array $response): array {
        ...
        $data = json_decode($response['body'], true);
        if (!is_array($data)) {
            throw new Masjid_App_Firebase_Server_Error($this->error_message($response, 'The Firebase API returned a malformed JSON response.'));
        }
        return $data;
    }

    private function exception_for_response(array $response): Masjid_App_Firebase_Exception {
        ...
        switch ($response['status']) {
            case 400:
                return new Masjid_App_Firebase_Invalid_Message($message);
            case 401:
            case 403:
                return new Masjid_App_Firebase_Authentication_Error($message);
            case 404:
                return new Masjid_App_Firebase_Not_Found($message);
            case 429:
                return new Masjid_App_Firebase_Quota_Exceeded($message, 0, null, $retry_after);
            case 500:
                return new Masjid_App_Firebase_Server_Error($message);
            case 503:
                return new Masjid_App_Firebase_Server_Unavailable($message, 0, null, $retry_after);
            default:
                return new Masjid_App_Firebase_Exception($message, $response['status']);
        }
    }
