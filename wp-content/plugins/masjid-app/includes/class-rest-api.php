...
class Masjid_App_REST_API {

    const NAMESPACE_ = 'masjid/v1';
    const LIST_LIMIT = 20;
    const CLIENT_CACHE_TTL = 60;
    const SERVER_CACHE_TTL = DAY_IN_SECONDS;
    const CONFIG_CACHE_KEY = 'masjidapp_config_response_v5';
    const CONFIG_SHARED_CACHE_TTL = HOUR_IN_SECONDS;
    const EVENTS_CACHE_KEY = 'masjidapp_events_response_v8';
    const EVENTS_SHARED_CACHE_TTL = 5 * MINUTE_IN_SECONDS;
    const ANNOUNCEMENTS_CACHE_KEY = 'masjidapp_announcements_response_v3';
    const FRIDAY_ANNOUNCEMENTS_CACHE_KEY = 'masjidapp_announcements_response_v8_friday';
    const REGULAR_ANNOUNCEMENTS_CACHE_KEY = 'masjidapp_announcements_response_v8_regular';
    ...
    public function __construct() {
        ...
        add_action('add_option_' . MASJIDAPP_OPTION_KEY, array($this, 'invalidate_settings_caches'));
        add_action('update_option_' . MASJIDAPP_OPTION_KEY, array($this, 'invalidate_settings_caches'));
    ...
    public function validate_client_user_agent($response, $handler, $request) {
        if (null !== $response || !$this->is_masjid_app_request($request) || 'OPTIONS' === $request->get_method()) {
            return $response;
        }

        if (null === self::parse_client_user_agent($request->get_header('user-agent'))) {
            return new WP_Error(
                'masjidapp_invalid_user_agent',
                __('User-Agent must use appname/version format.', 'masjid-app'),
                array('status' => 400)
            );
        }
    ...
    public function get_config($request) {
        ...
        $opts = Masjid_App_Settings::get_all_options();
        ...
        $response = array(
            ...
            'pushNotifications' => array(
                ...
                'configured' => Masjid_App_Credential_Store::has_credentials()
                ...
                'firebase' => Masjid_App_Settings::get_public_firebase_config(),
            ),
        );
     ...
     * GET /salahapi
     *
     * Proxies the Muslim Prayer Times plugin's SalahAPI document so masjid-app
    ...
    public function get_salahapi($request) {
        if (!function_exists('muslprti_salah_api_endpoint')) {
            return new WP_Error(
                'masjidapp_missing_dependency',
                __('The Muslim Prayer Times plugin is required to provide prayer time data.', 'masjid-app'),
                array('status' => 503)
            );
        }
    ...
    public function get_events($request) {
        if (!class_exists('Awesome_Events_Event_Meta')) {
            return new WP_Error(
                'masjidapp_missing_dependency',
                __('The Awesome Events plugin is required to provide event data.', 'masjid-app'),
                array('status' => 503)
            );
        }
    ...
    public function get_post_item($request) {
        ...
        if (!$post || 'post' !== $post->post_type || 'publish' !== $post->post_status || '' !== $post->post_password) {
            return new WP_Error(
                'masjidapp_post_not_found',
                __('Post not found.', 'masjid-app'),
                array('status' => 404)
            );
        }
    ...
    public function register_push_token($request) {
        ...
        try {
            ...
            return new WP_Error('masjidapp_firebase_registration_failed', __('The device could not be registered for push notifications.', 'masjid-app'), array('status' => 502));
        }
    }
    ...
    public function unregister_push_token($request) {
        try {
            ...
            return new WP_Error('masjidapp_firebase_unregistration_failed', __('The device could not be unregistered from push notifications.', 'masjid-app'), array('status' => 502));
        }
    }

    public function authorize_push_registration($request) {
        if (!Masjid_App_Settings::get_option('push_enabled')) {
            return new WP_Error('masjidapp_push_disabled', __('Push notifications are disabled.', 'masjid-app'), array('status' => 503));
        }
        if (!hash_equals((string) Masjid_App_Settings::get_option('masjid_id'), (string) $request->get_param('masjidId'))) {
            return new WP_Error('masjidapp_tenant_mismatch', __('The requested tenant does not match this site.', 'masjid-app'), array('status' => 400));
        }
        if (!$this->consume_registration_rate_limit()) {
            return new WP_Error('masjidapp_rate_limited', __('Too many push registration attempts. Try again later.', 'masjid-app'), array('status' => 429));
        }

        $app_check_token = trim((string) $request->get_header('x-firebase-appcheck'));
        if ('' === $app_check_token) {
            return new WP_Error('masjidapp_app_check_required', __('A Firebase App Check token is required.', 'masjid-app'), array('status' => 401));
        }
        $firebase_config = Masjid_App_Settings::get_public_firebase_config();
        $platform = (string) $request->get_param('platform');
        $platform_app_id = $firebase_config[$platform]['appId'] ?? '';
        if ('' === $platform_app_id) {
            return new WP_Error('masjidapp_firebase_not_configured', __('Firebase mobile application configuration is incomplete.', 'masjid-app'), array('status' => 503));
        }
        try {
            $this->authorized_firebase = new Masjid_App_Firebase();
            $this->authorized_firebase->verify_app_check($app_check_token, array($platform_app_id));
        } catch (Throwable $exception) {
            return new WP_Error('masjidapp_app_check_invalid', __('The Firebase App Check token is invalid.', 'masjid-app'), array('status' => 401));
        }
        return true;
    }
    ...
    public function expose_cache_headers($headers, $request) {
        if ($this->is_masjid_app_request($request)) {
            $headers[] = 'ETag';
        }
     ...
     */
    private function is_masjid_app_request($request) {
    ...
    private function consume_registration_rate_limit() {
        ...
        $key = 'masjidapp_push_rate_' . hash('sha256', $address);
    ...
    private function get_post_cache_key($post_id) {
        return 'masjidapp_post_response_v4_' . absint($post_id);
    }
