...
class Masjid_App_Settings {
    ...
    public function __construct() {
        ...
        add_action('admin_post_masjidapp_validate_firebase', array($this, 'handle_validate_firebase'));
        add_action('admin_post_masjidapp_test_push', array($this, 'handle_test_push'));
        add_action('admin_post_masjidapp_toggle_api_tracing', array($this, 'handle_toggle_api_tracing'));
        add_action('admin_post_masjidapp_clear_api_traces', array($this, 'handle_clear_api_traces'));
    }
    ...
    public static function get_option($key, $default = null) {
        $settings = get_option(MASJIDAPP_OPTION_KEY, array());
...
JS;
    ...
    private function render_term_checkboxes($terms, $field, $selected) {
        if (empty($terms)) {
            echo '<p class="description">' . esc_html__('No terms found.', 'masjid-app') . '</p>';
            return;
        }
    ...
    public function handle_validate_firebase() {
        $this->authorize_push_action('masjidapp_validate_firebase');
        try {
            (new Masjid_App_Firebase())->validate();
    ...
    public function handle_test_push() {
        $this->authorize_push_action('masjidapp_test_push');
    ...
    public function handle_toggle_api_tracing() {
        $this->authorize_push_action('masjidapp_toggle_api_tracing');
        $enabled = !empty($_POST['enabled']);
        Masjid_App_API_Trace::set_enabled($enabled);
    ...
    public function handle_clear_api_traces() {
        $this->authorize_push_action('masjidapp_clear_api_traces');
        Masjid_App_API_Trace::clear();
    ...
    private function sanitize_deep_link_template($value, $previous) {
        ...
        if (!$valid) {
            add_settings_error(MASJIDAPP_OPTION_KEY, 'invalid_deep_link', __('The deep-link override is invalid. Use a URI scheme and only supported placeholders.', 'masjid-app'));
    ...
    private function process_firebase_uploads(array $out, array $input) {
        $service = $this->read_json_upload('masjidapp_service_account');
        $ios = $this->read_plist_upload('masjidapp_ios_config');
        $android = $this->read_json_upload('masjidapp_android_config');
        foreach (array($service, $ios, $android) as $result) {
            if (is_wp_error($result)) {
                add_settings_error(MASJIDAPP_OPTION_KEY, $result->get_error_code(), $result->get_error_message());
                return $out;
            }
        }
        ...
        if (count(array_unique($project_ids)) > 1) {
            add_settings_error(MASJIDAPP_OPTION_KEY, 'firebase_project_mismatch', __('Firebase service-account, iOS, and Android files must belong to the same project.', 'masjid-app'));
            return $out;
        }

        if (is_array($service_config)) {
            $saved = Masjid_App_Credential_Store::save($service_config);
            if (is_wp_error($saved)) {
                add_settings_error(MASJIDAPP_OPTION_KEY, $saved->get_error_code(), $saved->get_error_message());
                return $out;
            }
        ...
        if (!empty($input['remove_firebase_credentials'])) {
            Masjid_App_Credential_Store::delete();
        }
        return $out;
    }
    ...
    private function read_json_upload($field) {
        ...
        return is_array($decoded) ? $decoded : new WP_Error('invalid_' . $field, __('An uploaded Firebase JSON file is invalid.', 'masjid-app'));
    }
    ...
    private function read_plist_upload($field) {
        ...
        if (!$xml || !isset($xml->dict)) {
            return new WP_Error('invalid_' . $field, __('The uploaded Firebase plist is invalid.', 'masjid-app'));
        }
        $values = array();
    ...
    private function read_upload($field) {
        ...
        if (UPLOAD_ERR_OK !== (int) $_FILES[$field]['error'] || (int) $_FILES[$field]['size'] > 1024 * 1024) {
            return new WP_Error('upload_' . $field, __('A Firebase configuration upload failed or exceeded 1 MB.', 'masjid-app'));
        }
        $contents = file_get_contents($_FILES[$field]['tmp_name']);
        return is_string($contents) ? $contents : new WP_Error('upload_' . $field, __('A Firebase configuration upload could not be read.', 'masjid-app'));
    }

    private function validate_service_account(array $data) {
        foreach (array('project_id', 'client_email', 'private_key') as $key) {
            if (empty($data[$key]) || !is_string($data[$key])) {
                return new WP_Error('invalid_service_account', __('The Firebase service-account JSON is missing required fields.', 'masjid-app'));
            }
        }
        return $data;
    }
    ...
    private function extract_ios_config(array $data) {
        foreach (array('GOOGLE_APP_ID', 'GCM_SENDER_ID', 'PROJECT_ID', 'API_KEY', 'BUNDLE_ID') as $key) {
            if (empty($data[$key])) {
                return new WP_Error('invalid_ios_config', __('The Firebase iOS plist is missing required fields.', 'masjid-app'));
            }
        }
    ...
    private function extract_android_config(array $data) {
        ...
        if (empty($project['project_id']) || empty($project['project_number']) || !$api_key || !$app_id || !$package) {
            return new WP_Error('invalid_android_config', __('The Android google-services.json file is missing required fields.', 'masjid-app'));
        }
    ...
    private function render_push_tools() {
        ...
        if ($notice) {
            $messages = array(
                'firebase_valid' => __('Firebase configuration is valid.', 'masjid-app'),
                'test_sent' => __('Test notification sent.', 'masjid-app'),
                'token_required' => __('Enter an FCM token.', 'masjid-app'),
                'api_tracing_enabled' => __('API call tracing enabled.', 'masjid-app'),
                'api_tracing_disabled' => __('API call tracing disabled.', 'masjid-app'),
                'api_traces_cleared' => __('API call traces cleared.', 'masjid-app'),
            );
        ...
        <form method="post" action="<?php echo esc_url(admin_url('admin-post.php')); ?>" style="display:inline-block;margin-right:8px;">
            <input type="hidden" name="action" value="masjidapp_validate_firebase" /><?php wp_nonce_field('masjidapp_validate_firebase'); ?>
            <?php submit_button(__('Validate connection', 'masjid-app'), 'secondary', 'submit', false); ?>
        </form>
        <form method="post" action="<?php echo esc_url(admin_url('admin-post.php')); ?>" style="margin-top:12px;">
            <input type="hidden" name="action" value="masjidapp_test_push" /><?php wp_nonce_field('masjidapp_test_push'); ?>
            <label for="masjidapp_test_token"><strong><?php esc_html_e('Test FCM token', 'masjid-app'); ?></strong></label><br />
        ...
        if (!$jobs) {
            echo '<p>' . esc_html__('No notifications have been queued.', 'masjid-app') . '</p>';
            return;
        }
        echo '<table class="widefat striped"><thead><tr><th>' . esc_html__('Created', 'masjid-app') . '</th><th>' . esc_html__('Post', 'masjid-app') . '</th><th>' . esc_html__('Trigger', 'masjid-app') . '</th><th>' . esc_html__('Status', 'masjid-app') . '</th><th>' . esc_html__('Attempts', 'masjid-app') . '</th><th>' . esc_html__('Result', 'masjid-app') . '</th></tr></thead><tbody>';
        foreach ($jobs as $job) {
            ...
            if ('failed' === $job->status) {
                echo '<form method="post" action="' . esc_url(admin_url('admin-post.php')) . '"><input type="hidden" name="action" value="masjidapp_retry_push" /><input type="hidden" name="job_id" value="' . (int) $job->id . '" />';
                wp_nonce_field('masjidapp_retry_push_' . $job->id);
                submit_button(__('Retry', 'masjid-app'), 'small', 'submit', false);
                echo '</form>';
            } else {
    ...
    private function render_api_trace_tools() {
        $enabled = Masjid_App_API_Trace::is_enabled();
        $entries = Masjid_App_API_Trace::get_entries();
        ?>
        ...
        <form method="post" action="<?php echo esc_url(admin_url('admin-post.php')); ?>" style="display:inline-block;margin-right:8px;">
            ...
            <?php wp_nonce_field('masjidapp_toggle_api_tracing'); ?>
            <?php submit_button($enabled ? __('Disable tracing', 'masjid-app') : __('Enable tracing', 'masjid-app'), 'secondary', 'submit', false); ?>
        </form>
        <?php if ($entries) : ?>
            <form method="post" action="<?php echo esc_url(admin_url('admin-post.php')); ?>" style="display:inline-block;">
                <input type="hidden" name="action" value="masjidapp_clear_api_traces" />
                <?php wp_nonce_field('masjidapp_clear_api_traces'); ?>
                <?php submit_button(__('Clear traces', 'masjid-app'), 'secondary', 'submit', false); ?>
            </form>
            <table class="widefat striped" style="margin-top:12px;">
                <thead><tr><th><?php esc_html_e('Time (UTC)', 'masjid-app'); ?></th><th><?php esc_html_e('Activity', 'masjid-app'); ?></th><th><?php esc_html_e('Status', 'masjid-app'); ?></th><th><?php esc_html_e('Duration', 'masjid-app'); ?></th><th><?php esc_html_e('App Check', 'masjid-app'); ?></th><th><?php esc_html_e('Details', 'masjid-app'); ?></th></tr></thead>
                <tbody>
                    <?php foreach ($entries as $entry) : ?>
                        ...
                        <tr>
                            ...
                            <td><details><summary><?php esc_html_e('View', 'masjid-app'); ?></summary><pre style="max-width:520px;overflow:auto;white-space:pre-wrap;"><?php echo esc_html(wp_json_encode($details, JSON_PRETTY_PRINT | JSON_UNESCAPED_SLASHES)); ?></pre></details></td>
                        </tr>
                    <?php endforeach; ?>
                </tbody>
            </table>
        <?php else : ?>
            <p class="description"><?php esc_html_e('No API calls or Firebase sends captured.', 'masjid-app'); ?></p>
        <?php endif;
    }
    ...
    private function redirect_with_notice($notice, $type) {
        wp_safe_redirect(add_query_arg(array('page' => self::PAGE_SLUG, 'masjidapp_notice' => $notice, 'masjidapp_notice_type' => $type), admin_url('options-general.php')));
        exit;
    }
}
