...
class Masjid_App_API_Trace {

    const ENABLED_OPTION = 'masjidapp_api_trace_enabled';
    const ENTRIES_OPTION = 'masjidapp_api_trace_entries';
    ...
    public function start_trace($response, $handler, $request) {
        if (self::is_enabled() && $this->is_masjid_app_request($request)) {
    ...
    public function finish_trace($response, $handler, $request) {
        ...
        $client = Masjid_App_REST_API::parse_client_user_agent($user_agent);
        $entry = array(
            'id' => wp_generate_uuid4(),
            'timestamp' => gmdate('c'),
