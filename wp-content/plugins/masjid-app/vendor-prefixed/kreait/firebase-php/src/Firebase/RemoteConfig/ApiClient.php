...
class ApiClient
{
    ...
    public function __construct(string $projectId, private readonly ClientInterface $client, private readonly RemoteConfigApiExceptionConverter $errorHandler)
    {
    ...
    {
        if (in_array($versionNumber, [null, '', '0'], \true)) {
            $versionNumber = VersionNumber::fromValue(0);
        }
        return $this->requestApi('GET', 'remoteConfig', ['query' => ['version_number' => (string) $versionNumber]]);
    }
    /**
     * @throws RemoteConfigException
     */
    ...
    {
        return $this->requestApi('PUT', 'remoteConfig', ['headers' => ['Content-Type' => 'application/json; UTF-8', 'If-Match' => $template->etag()], 'query' => ['validate_only' => 'true'], 'body' => Json::encode($template)]);
    }
    /**
     * @throws RemoteConfigException
     */
    ...
    {
        return $this->requestApi('PUT', 'remoteConfig', ['headers' => ['Content-Type' => 'application/json; UTF-8', 'If-Match' => $template->etag()], 'body' => Json::encode($template)]);
    }
    /**
    ...
    public function listVersions(FindVersions $query, ?string $nextPageToken = null): ResponseInterface
    {
        ...
        return $this->requestApi('GET', $uri, ['query' => array_filter(['startTime' => $since, 'endTime' => $until, 'endVersionNumber' => $lastVersionNumber, 'pageSize' => $pageSize, 'pageToken' => $nextPageToken], fn(?string $value): bool => $value !== null)]);
    }
    /**
     * @throws RemoteConfigException
     */
    public function rollbackToVersion(VersionNumber $versionNumber): ResponseInterface
    {
        ...
        return $this->requestApi('POST', $uri, ['json' => ['version_number' => (string) $versionNumber]]);
    }
    /**
