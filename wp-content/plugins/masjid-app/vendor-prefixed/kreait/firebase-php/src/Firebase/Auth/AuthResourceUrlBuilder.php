...
final class AuthResourceUrlBuilder
{
     ...
     */
    private function __construct(private readonly string $apiVersion, private readonly string $urlFormat)
    {
    }
    /**
    ...
    {
        $api ??= '';
        $url = strtr($this->urlFormat, ['{version}' => $this->apiVersion, '{api}' => $api]);
        assert($url !== '');
        if ($params !== null) {
