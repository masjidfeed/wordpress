...
use Masjid_App\Dependencies\Kreait\Firebase\Auth\CreateSessionCookie\GuzzleApiClientHandler;
use Masjid_App\Dependencies\Kreait\Firebase\Auth\SignIn\GuzzleHandler;
use Masjid_App\Dependencies\Kreait\Firebase\Exception\Auth\EmailNotFound;
use Masjid_App\Dependencies\Kreait\Firebase\Exception\Auth\ExpiredOobCode;
use Masjid_App\Dependencies\Kreait\Firebase\Exception\Auth\InvalidOobCode;
use Masjid_App\Dependencies\Kreait\Firebase\Exception\Auth\OperationNotAllowed;
use Masjid_App\Dependencies\Kreait\Firebase\Exception\Auth\UserDisabled;
...
class ApiClient
{
     ...
     */
    public function __construct(private readonly string $projectId, private readonly ?string $tenantId, private readonly ClientInterface $client, private readonly GuzzleHandler $signInHandler, private readonly ClockInterface $clock, private readonly AuthApiExceptionConverter $errorHandler)
    {
        $this->awareAuthResourceUrlBuilder = $tenantId !== null ? TenantAwareAuthResourceUrlBuilder::forProjectAndTenant($projectId, $tenantId) : ProjectAwareAuthResourceUrlBuilder::forProject($projectId);
    ...
    public function createUser(CreateUser $request): ResponseInterface
    {
        ...
        return $this->requestApi($url, Json::decode(Json::encode($request), \true));
    }
    /**
     * @throws AuthException
     */
    ...
    {
        $url = $this->awareAuthResourceUrlBuilder->getUrl('/accounts:update');
        return $this->requestApi($url, ['localId' => $uid, 'customAttributes' => Json::encode((object) $claims)]);
    }
    /**
    ...
    {
        $batchSize ??= 1000;
        $urlParams = array_filter(['maxResults' => (string) $batchSize, 'nextPageToken' => (string) $nextPageToken], fn(string $value): bool => $value !== '');
    ...
    {
        $data = ['localIds' => $uids, 'force' => $forceDeleteEnabledUsers];
    ...
    public function queryUsers(UserQuery $query): ResponseInterface
    {
        ...
        return $this->requestApi($url, Json::decode(Json::encode($query), \true));
    }
    /**
     * @throws AuthException
    ...
    {
        $url = $this->authResourceUrlBuilder->getUrl('/accounts:resetPassword');
        return $this->requestApi($url, ['oobCode' => $oobCode, 'newPassword' => $newPassword]);
    }
    /**
     * @throws AuthException
     */
    ...
    {
        $url = $this->awareAuthResourceUrlBuilder->getUrl('/accounts:update');
        return $this->requestApi($url, ['localId' => $uid, 'validSince' => (string) time()]);
    }
    /**
    ...
    public function unlinkProvider(string $uid, array $providers): ResponseInterface
    {
        ...
        return $this->requestApi($url, ['localId' => $uid, 'deleteProvider' => $providers]);
    }
    public function createSessionCookie(string $idToken, int|DateInterval $ttl): string
    {
        return (new GuzzleApiClientHandler($this->client, $this->projectId))->handle(CreateSessionCookie::forIdToken($idToken, $this->tenantId, $ttl, $this->clock));
    }
    ...
    {
        return (new CreateActionLink\GuzzleApiClientHandler($this->client, $this->projectId))->handle(CreateActionLink::new($type, $email, $actionCodeSettings, $this->tenantId, $locale));
    }
    /**
