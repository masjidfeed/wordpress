...
namespace Masjid_App\Dependencies\Kreait\Firebase\Auth\CreateSessionCookie;
...
use Masjid_App\Dependencies\Kreait\Firebase\Auth\CreateSessionCookie;
use Masjid_App\Dependencies\Kreait\Firebase\Auth\ProjectAwareAuthResourceUrlBuilder;
use Masjid_App\Dependencies\Kreait\Firebase\Auth\TenantAwareAuthResourceUrlBuilder;
...
final class GuzzleApiClientHandler
{
     ...
     */
    public function __construct(private readonly ClientInterface $client, private readonly string $projectId)
    {
    }
    ...
    {
        $data = ['idToken' => $action->idToken(), 'validDuration' => $action->ttlInSeconds()];
        ...
        $headers = array_filter(['Content-Type' => 'application/json; charset=UTF-8', 'Content-Length' => (string) ($body->getSize() ?? '')], fn($value): bool => $value !== '' && $value !== '0');
