...
use Masjid_App\Dependencies\GuzzleHttp\Promise\Utils;
...
use Masjid_App\Dependencies\Kreait\Firebase\Exception\Messaging\NotFound;
...
use Masjid_App\Dependencies\Kreait\Firebase\Messaging\ApiClient;
...
use Masjid_App\Dependencies\Kreait\Firebase\Messaging\MessageTarget;
use Masjid_App\Dependencies\Kreait\Firebase\Messaging\MulticastSendReport;
use Masjid_App\Dependencies\Kreait\Firebase\Messaging\Processor\SetApnsContentAvailableIfNeeded;
use Masjid_App\Dependencies\Kreait\Firebase\Messaging\Processor\SetApnsPushTypeIfNeeded;
...
use Masjid_App\Dependencies\Kreait\Firebase\Messaging\SendReport;
...
{
    public function __construct(private readonly ApiClient $messagingApi, private readonly AppInstanceApiClient $appInstanceApi, private readonly MessagingApiExceptionConverter $exceptionConverter)
    {
    }
    ...
    public function sendAll($messages, bool $validateOnly = \false): MulticastSendReport
    {
        ...
        $config = ['fulfilled' => function (ResponseInterface $response, int $index) use ($messages, &$sendReports): void {
            $message = $messages[$index];
            $json = Json::decode((string) $response->getBody(), \true);
    ...
    public function validateRegistrationTokens($registrationTokenOrTokens): array
    {
        ...
        $report = $this->sendMulticast(new RawMessageFromArray([]), $tokens, \true);
        return ['valid' => $report->validTokens(), 'unknown' => $report->unknownTokens(), 'invalid' => $report->invalidTokens()];
    }
    ...
    {
        $topics = array_map(static fn($topic): Topic => $topic instanceof Topic ? $topic : Topic::fromValue($topic), $topics);
        $tokens = RegistrationTokens::fromValue($registrationTokenOrTokens);
    ...
    public function unsubscribeFromAllTopics($registrationTokenOrTokens): array
    {
        ...
        foreach ($tokens as $token) {
            $promises[$token->value()] = $this->appInstanceApi->getAppInstanceAsync($token)->then(function (AppInstance $appInstance) use ($token): array {
                $topics = [];
    ...
    {
        $check = Json::decode(Json::encode($message), \true);
        return array_key_exists(MessageTarget::CONDITION, $check) || array_key_exists(MessageTarget::TOKEN, $check) || array_key_exists(MessageTarget::TOPIC, $check);
    }
    private function withChangedTarget(Message $message, string $target, string $value): Message
    {
        $message = Json::decode(Json::encode($message), \true);
        unset($message[MessageTarget::CONDITION], $message[MessageTarget::TOKEN], $message[MessageTarget::TOPIC]);
