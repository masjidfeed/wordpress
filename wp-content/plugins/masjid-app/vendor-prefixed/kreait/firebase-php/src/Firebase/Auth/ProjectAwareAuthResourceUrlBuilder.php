...
final class ProjectAwareAuthResourceUrlBuilder
{
     ...
     */
    private function __construct(private readonly string $projectId, private readonly string $apiVersion, private readonly string $urlFormat)
    {
    }
    /**
     * @param non-empty-string $projectId
    ...
    {
        $api ??= '';
        $url = strtr($this->urlFormat, ['{version}' => $this->apiVersion, '{projectId}' => $this->projectId, '{api}' => $api]);
        if ($params !== null) {
