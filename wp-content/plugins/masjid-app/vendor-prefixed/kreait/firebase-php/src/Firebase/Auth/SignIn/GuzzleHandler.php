...
namespace Masjid_App\Dependencies\Kreait\Firebase\Auth\SignIn;
...
use Masjid_App\Dependencies\GuzzleHttp\Psr7\Utils;
use Masjid_App\Dependencies\Kreait\Firebase\Auth\AuthResourceUrlBuilder;
use Masjid_App\Dependencies\Kreait\Firebase\Auth\IsTenantAware;
use Masjid_App\Dependencies\Kreait\Firebase\Auth\SignIn;
...
use Masjid_App\Dependencies\Kreait\Firebase\Util;
...
final class GuzzleHandler
{
     ...
     */
    private static array $defaultBody = ['returnSecureToken' => \true];
    /**
     * @var array<non-empty-string, mixed>
     */
    private static array $defaultHeaders = ['Content-Type' => 'application/json; charset=UTF-8'];
    public function __construct(private readonly string $projectId, private readonly ClientInterface $client)
    {
    }
    ...
    private function customToken(SignInWithCustomToken $action): Request
    {
        ...
        $body = Utils::streamFor(Json::encode([...$this->prepareBody($action), 'token' => $action->customToken()], JSON_FORCE_OBJECT));
        $headers = self::$defaultHeaders;
        return new Request('POST', $url, $headers, $body);
    }
    private function emailAndPassword(SignInWithEmailAndPassword $action): Request
    {
        ...
        $body = Utils::streamFor(Json::encode([...$this->prepareBody($action), 'email' => $action->email(), 'password' => $action->clearTextPassword(), 'returnSecureToken' => \true], JSON_FORCE_OBJECT));
        $headers = self::$defaultHeaders;
        return new Request('POST', $url, $headers, $body);
    }
    private function emailAndOobCode(SignInWithEmailAndOobCode $action): Request
    {
        ...
        $body = Utils::streamFor(Json::encode([...$this->prepareBody($action), 'email' => $action->email(), 'oobCode' => $action->oobCode(), 'returnSecureToken' => \true], JSON_FORCE_OBJECT));
        $headers = self::$defaultHeaders;
        return new Request('POST', $url, $headers, $body);
    }
    private function idpCredentials(SignInWithIdpCredentials $action): Request
    {
        ...
        $postBody = array_filter(['access_token' => $action->accessToken(), 'id_token' => $action->idToken(), 'providerId' => $action->provider(), 'oauth_token_secret' => $action->oauthTokenSecret(), 'nonce' => $action->rawNonce()], fn(?string $value): bool => $value !== null);
        $rawBody = array_filter([...$this->prepareBody($action), 'postBody' => http_build_query($postBody), 'returnIdpCredential' => \true, 'requestUri' => $action->requestUri(), 'idToken' => $action->linkingIdToken()], fn($value): bool => $value !== null);
    ...
    {
        $body = Query::build(['grant_type' => 'refresh_token', 'refresh_token' => $action->refreshToken()]);
