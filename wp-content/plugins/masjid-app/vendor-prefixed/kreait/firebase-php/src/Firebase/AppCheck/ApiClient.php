...
{
    public function __construct(private readonly ClientInterface $client, private readonly AppCheckApiExceptionConverter $errorHandler)
    {
    }
    /**
    ...
    {
        $response = $this->requestApi('POST', 'apps/' . $appId . ':exchangeCustomToken', ['headers' => ['Content-Type' => 'application/json; UTF-8'], 'body' => Json::encode(['customToken' => $customToken])]);
        /** @var AppCheckTokenShape $decoded */
        $decoded = Json::decode((string) $response->getBody(), \true);
        return $decoded;
    }
    /**
