...
class ApiClient
{
    public function __construct(private readonly ClientInterface $client, private readonly UrlBuilder $resourceUrlBuilder, private readonly DatabaseApiExceptionConverter $errorHandler)
    {
    }
    /**
    ...
    {
        $response = $this->requestApi('GET', $path, ['headers' => ['X-Firebase-ETag' => 'true']]);
        $value = Json::decode((string) $response->getBody(), \true);
    ...
    {
        $response = $this->requestApi('PUT', $path, ['headers' => ['if-match' => $etag], 'json' => $value]);
        return Json::decode((string) $response->getBody(), \true);
    }
    /**
     * @throws DatabaseException
    ...
    {
        $this->requestApi('DELETE', $path, ['headers' => ['if-match' => $etag]]);
    }
    /**
     * @throws DatabaseException
     */
    public function updateRules(string $path, RuleSet $ruleSet): mixed
    {
        ...
        $response = $this->requestApi('PUT', $path, ['body' => $encodedRules]);
        return Json::decode((string) $response->getBody(), \true);
    }
    /**
     * @throws DatabaseException
    ...
    public function push(string $path, mixed $value): string
    {
        ...
        return Json::decode((string) $response->getBody(), \true)['name'];
    }
    /**
     * @throws DatabaseException
     */
    ...
    private function requestApi(string $method, string $path, ?array $options = []): ResponseInterface
    {
        $options ??= [];
        $uri = new Uri($path);
        $url = $this->resourceUrlBuilder->getUrl($uri->getPath(), Query::parse($uri->getQuery()));
