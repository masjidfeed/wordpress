...
final class MessageData implements JsonSerializable
{
    ...
    public static function fromArray(array $data): self
    {
        ...
        foreach ($data as $key => $value) {
            ...
            if (self::isBinary($value)) {
                throw new InvalidArgumentException("The message data field '{$key}' seems to contain binary data. As this can lead to broken messages, " . 'please convert it to a string representation first, e.g. with bin2hex() or base64encode().');
            }
    ...
    {
        return mb_detect_encoding($value, mb_detect_order(), \true) === \false;
    }
    /**
    ...
    private static function assertValidKey(string $key): string
    {
        ...
        if (in_array($check, $reservedWords, \true)) {
