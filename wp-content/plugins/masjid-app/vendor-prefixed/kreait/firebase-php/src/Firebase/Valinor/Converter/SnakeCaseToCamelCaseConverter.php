...
namespace Masjid_App\Dependencies\Kreait\Firebase\Valinor\Converter;
...
final class SnakeCaseToCamelCaseConverter
{
    ...
    public function __invoke(iterable $values, callable $next): object
    {
        ...
        $camelCaseConverted = array_combine(array_map(fn(int|string $key): string => lcfirst(str_replace('_', '', ucwords((string) $key, '_'))), array_keys($values)), $values);
