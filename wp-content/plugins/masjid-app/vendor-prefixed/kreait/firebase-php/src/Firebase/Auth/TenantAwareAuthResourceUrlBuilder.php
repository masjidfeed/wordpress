...
final class TenantAwareAuthResourceUrlBuilder
{
     ...
     */
    private function __construct(private readonly string $projectId, private readonly string $tenantId, private readonly string $apiVersion, private readonly string $urlFormat)
    {
    }
    /**
     * @param non-empty-string $projectId
    ...
    public static function forProjectAndTenant(string $projectId, string $tenantId, ?string $version = null): self
    {
        ...
        $urlFormat = $emulatorHost !== null ? str_replace('{host}', $emulatorHost, self::EMULATOR_URL_FORMAT) : self::URL_FORMAT;
    ...
    {
        $api ??= '';
        $url = strtr($this->urlFormat, ['{version}' => $this->apiVersion, '{projectId}' => $this->projectId, '{tenantId}' => $this->tenantId, '{api}' => $api]);
        if ($params !== null) {
