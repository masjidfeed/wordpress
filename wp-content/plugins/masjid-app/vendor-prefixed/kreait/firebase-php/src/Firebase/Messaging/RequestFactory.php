...
final class RequestFactory
{
    ...
    {
        $request = $this->requestFactory->createRequest('POST', 'https://fcm.googleapis.com/v1/projects/' . $projectId . '/messages:send');
        if ($this->environmentSupportsHTTP2) {
            $request = $request->withProtocolVersion('2.0');
        }
        $payload = ['message' => $message];
        if ($validateOnly === \true) {
            $payload['validate_only'] = \true;
        }
        $body = $this->streamFactory->createStream(Json::encode($payload));
        return $request->withBody($body)->withHeader('Content-Type', 'application/json; charset=UTF-8')->withHeader('Content-Length', (string) $body->getSize())->withHeader('Accept', 'application/json, text/plain;q=0.9');
    }
    /**
