...
final class UrlBuilder
{
     ...
     */
    private function __construct(private readonly string $scheme, private readonly string $host, private readonly array $defaultQueryParams)
    {
    }
    /**
    ...
    public function getUrl(string $path, array $queryParams = []): string
    {
        ...
        $path = '/' . trim($path, '/');
        $url = strtr('{scheme}://{host}{path}?{queryParams}', ['{scheme}' => $this->scheme, '{host}' => $this->host, '{path}' => $path, '{queryParams}' => http_build_query($allQueryParams)]);
    ...
    private static function parseDatabaseUrl(string $databaseUrl): array
    {
        ...
        if (!in_array($emulatorHost, ['', '0', null], \true)) {
            return ['scheme' => 'http', 'host' => $emulatorHost, 'query' => ['ns' => $namespace]];
        }
        return ['scheme' => 'https', 'host' => $namespace . '.' . $host, 'query' => []];
    }
}
