...
class Template implements JsonSerializable
{
    ...
    {
        return array_values(array_unique(array_map(static fn(Condition $c): string => $c->name(), $this->conditions)));
    }
    /**
     * @param non-empty-string $name
     */
    ...
    {
        $template = clone $this;
        $template->conditions = array_values(array_filter($this->conditions, static fn(Condition $c): bool => $c->name() !== $name));
        return $template;
    }
    /**
     * @return array{
    ...
    public function jsonSerialize(): array
    {
        ...
        return ['conditions' => $conditions !== [] ? $conditions : null, 'parameters' => $parameters !== [] ? $parameters : null, 'parameterGroups' => $parameterGroups !== [] ? $parameterGroups : null];
    }
    /**
     * @param non-empty-string $name
    ...
    private static function buildParameter(string $name, array $data): Parameter
    {
        ...
        $parameter = Parameter::named($name)->withDescription((string) ($data['description'] ?? ''))->withDefaultValue($data['defaultValue'] ?? null)->withValueType($valueType);
    ...
    {
        $group = ParameterGroup::named($name)->withDescription((string) ($parameterGroupData['description'] ?? ''));
    ...
    private function assertThatAllConditionalValuesAreValid(Parameter $parameter): void
    {
        ...
        foreach ($parameter->conditionalValues() as $conditionalValue) {
            if (!in_array($conditionalValue->conditionName(), $conditionNames, \true)) {
