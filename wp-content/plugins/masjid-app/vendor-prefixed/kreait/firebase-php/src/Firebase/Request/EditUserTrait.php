...
namespace Masjid_App\Dependencies\Kreait\Firebase\Request;
...
trait EditUserTrait
{
    ...
    {
        $disableUser = null;
        if ($this->markAsDisabled === \true) {
            $disableUser = \true;
        } elseif ($this->markAsEnabled === \true) {
            $disableUser = \false;
        }
        return array_filter(['localId' => $this->uid, 'disableUser' => $disableUser, 'displayName' => $this->displayName, 'email' => $this->email, 'emailVerified' => $this->emailIsVerified, 'phoneNumber' => $this->phoneNumber, 'photoUrl' => $this->photoUrl, 'password' => $this->clearTextPassword, 'mfa' => $this->multiFactor], static fn($value): bool => $value !== null);
    }
