...
use Masjid_App\Dependencies\Kreait\Firebase\Request;
use Stringable;
    ...
    public static function withProperties(array $properties): self
    {
        ...
        foreach ($properties as $key => $value) {
            switch (mb_strtolower((string) preg_replace('/[^a-z]/i', '', $key))) {
                ...
                case 'phonenumber':
                case 'phone':
                    if (in_array($value, [\false, null, ''], \true)) {
                ...
                case 'removeproviders':
                    $request = array_reduce((array) $value, static fn(self $request, $provider): UpdateUser => $request->withRemovedProvider($provider), $request);
                    break;
