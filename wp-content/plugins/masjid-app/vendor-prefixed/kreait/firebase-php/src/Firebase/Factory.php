...
use Masjid_App\Dependencies\Beste\Cache\InMemoryCache;
use Masjid_App\Dependencies\Beste\Clock\SystemClock;
use Masjid_App\Dependencies\Beste\Clock\WrappingClock;
use Masjid_App\Dependencies\Firebase\JWT\CachedKeySet;
use Masjid_App\Dependencies\Google\Auth\ApplicationDefaultCredentials;
use Masjid_App\Dependencies\Google\Auth\Credentials\ServiceAccountCredentials;
use Masjid_App\Dependencies\Google\Auth\FetchAuthTokenCache;
use Masjid_App\Dependencies\Google\Auth\FetchAuthTokenInterface;
use Masjid_App\Dependencies\Google\Auth\HttpHandler\HttpHandlerFactory;
use Masjid_App\Dependencies\Google\Auth\Middleware\AuthTokenMiddleware;
use Masjid_App\Dependencies\Google\Auth\ProjectIdProviderInterface;
use Masjid_App\Dependencies\Google\Auth\SignBlobInterface;
use Masjid_App\Dependencies\Google\Cloud\Storage\StorageClient;
use Masjid_App\Dependencies\GuzzleHttp\Client;
use Masjid_App\Dependencies\GuzzleHttp\HandlerStack;
use Masjid_App\Dependencies\GuzzleHttp\MessageFormatter;
use Masjid_App\Dependencies\GuzzleHttp\Psr7\HttpFactory;
use Masjid_App\Dependencies\GuzzleHttp\Psr7\Utils as GuzzleUtils;
...
use Masjid_App\Dependencies\Kreait\Firebase\Auth\SignIn\GuzzleHandler;
use Masjid_App\Dependencies\Kreait\Firebase\Database\UrlBuilder;
use Masjid_App\Dependencies\Kreait\Firebase\Exception\AppCheckApiExceptionConverter;
use Masjid_App\Dependencies\Kreait\Firebase\Exception\AuthApiExceptionConverter;
use Masjid_App\Dependencies\Kreait\Firebase\Exception\DatabaseApiExceptionConverter;
use Masjid_App\Dependencies\Kreait\Firebase\Exception\InvalidArgumentException;
use Masjid_App\Dependencies\Kreait\Firebase\Exception\MessagingApiExceptionConverter;
use Masjid_App\Dependencies\Kreait\Firebase\Exception\RemoteConfigApiExceptionConverter;
...
use Masjid_App\Dependencies\Kreait\Firebase\Http\HttpClientOptions;
use Masjid_App\Dependencies\Kreait\Firebase\Http\Middleware;
...
use Masjid_App\Dependencies\Kreait\Firebase\Messaging\AppInstanceApiClient;
use Masjid_App\Dependencies\Kreait\Firebase\Messaging\RequestFactory;
use Masjid_App\Dependencies\Kreait\Firebase\Valinor\Mapper;
use Masjid_App\Dependencies\Kreait\Firebase\Valinor\Normalizer;
use Masjid_App\Dependencies\Kreait\Firebase\Valinor\Source;
...
use Masjid_App\Dependencies\Psr\Log\LoggerInterface;
...
final class Factory
{
    ...
    {
        $clientOptions = $this->httpClientOptions->withGuzzleMiddleware(middleware: Middleware::log($logger, $formatter ?? new MessageFormatter(), $logLevel ?? LogLevel::INFO, $errorLogLevel ?? LogLevel::NOTICE), name: 'http_logs');
        $factory = clone $this;
    ...
    {
        $clientOptions = $this->httpClientOptions->withGuzzleMiddleware(middleware: Middleware::log($logger, $formatter ?? new MessageFormatter(MessageFormatter::DEBUG), $logLevel ?? LogLevel::INFO, $errorLogLevel ?? LogLevel::NOTICE), name: 'http_debug_logs');
        $factory = clone $this;
        $factory->httpClientOptions = $clientOptions;
        return $factory;
    }
    ...
    public function createAppCheck(): Contract\AppCheck
    {
        ...
        $http = $this->createApiClient(['base_uri' => 'https://firebaseappcheck.googleapis.com/v1/projects/' . $projectId . '/']);
        $keySet = new CachedKeySet('https://firebaseappcheck.googleapis.com/v1/jwks', new Client($this->httpClientOptions->guzzleConfig()), $this->httpFactory, $this->keySetCache ?? $this->defaultCache, 21600, \true);
        return new AppCheck(new AppCheck\ApiClient($http, new AppCheckApiExceptionConverter($this->errorResponseParser)), new AppCheckTokenGenerator($serviceAccount->clientEmail, $serviceAccount->privateKey, $this->clock), new AppCheckTokenVerifier($projectId, $keySet));
    }
    public function createAuth(): Contract\Auth
    {
        ...
        $authApiClient = new ApiClient($projectId, $this->tenantId, $httpClient, $signInHandler, $this->clock, new AuthApiExceptionConverter($this->errorResponseParser));
    ...
    {
        $middlewares = array_filter([Middleware::ensureJsonSuffix(), $this->databaseAuthVariableOverrideMiddleware]);
        ...
        return new Database(GuzzleUtils::uriFor($databaseUrl), new Database\ApiClient($http, $resourceUrlBuilder, new DatabaseApiExceptionConverter($this->errorResponseParser)));
    }
    public function createRemoteConfig(): Contract\RemoteConfig
    {
        $http = $this->createApiClient(['base_uri' => "https://firebaseremoteconfig.googleapis.com/v1/projects/{$this->getProjectId()}/remoteConfig"]);
        return new RemoteConfig(new RemoteConfig\ApiClient($this->getProjectId(), $http, new RemoteConfigApiExceptionConverter($this->errorResponseParser)));
    }
    public function createMessaging(): Contract\Messaging
    {
        $projectId = $this->getProjectId();
        $errorHandler = new MessagingApiExceptionConverter($this->clock);
        $requestFactory = new RequestFactory(requestFactory: $this->httpFactory, streamFactory: $this->httpFactory);
        $messagingApiClient = new Messaging\ApiClient($this->createApiClient(), $projectId, $requestFactory);
        $appInstanceApiClient = new AppInstanceApiClient($this->createApiClient(['base_uri' => 'https://iid.googleapis.com', 'headers' => ['access_token_auth' => 'true']]), $errorHandler);
    ...
    {
        $apiClient = new DynamicLink\ApiClient($this->createApiClient(), $this->httpFactory, $this->httpFactory);
    ...
    public function getDebugInfo(): array
    {
        ...
        return ['credentialsType' => $credentials, 'databaseUrl' => $databaseUrl, 'defaultStorageBucket' => $this->defaultStorageBucket, 'projectId' => $projectId, 'serviceAccount' => $this->getServiceAccount(), 'tenantId' => $this->tenantId, 'tokenCacheType' => $this->authTokenCache !== null ? $this->authTokenCache::class : $this->defaultCache::class, 'verifierCacheType' => $this->verifierCache !== null ? $this->verifierCache::class : $this->defaultCache::class];
    }
    /**
    ...
    {
        $config = ['projectId' => $this->getProjectId(), 'authCache' => $this->authTokenCache ?? $this->defaultCache];
        $credentials = $this->getGoogleAuthTokenCredentials();
        if ($credentials !== null) {
    ...
    private function getProjectId(): string
    {
        ...
        $credentials = $this->getGoogleAuthTokenCredentials();
        $projectId = $credentials instanceof ProjectIdProviderInterface ? $credentials->getProjectId() : Util::getenv('GOOGLE_CLOUD_PROJECT');
    ...
    private function getGoogleAuthTokenCredentials(): ?FetchAuthTokenInterface
    {
        ...
        $serviceAccount = $this->getServiceAccount();
        if ($serviceAccount !== null) {
            return $this->googleAuthTokenCredentials = new ServiceAccountCredentials(self::API_CLIENT_SCOPES, $this->normalizeServiceAccount($serviceAccount));
        }
        try {
    ...
    {
        return $this->getMapper()->allowSuperfluousKeys()->snakeToCamelCase()->map(ServiceAccount::class, Source::parse($value));
    }
    /**
     * @return array<non-empty-string, mixed>
     */
    ...
    {
        return $this->getNormalizer()->camelToSnakeCase()->toArray($serviceAccount);
    }
}
