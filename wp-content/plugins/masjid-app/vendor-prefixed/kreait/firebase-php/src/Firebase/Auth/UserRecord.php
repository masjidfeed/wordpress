...
final class UserRecord
{
    ...
    {
        $validSince = array_key_exists('validSince', $data) ? DT::toUTCDateTimeImmutable($data['validSince']) : null;
        $customClaims = array_key_exists('customAttributes', $data) ? Json::decode($data['customAttributes'], \true) : [];
        $providerUserInfo = array_key_exists('providerUserInfo', $data) ? self::userInfoFromResponseData($data) : [];
        return new self($data['localId'], $data['email'] ?? null, $data['emailVerified'] ?? \false, $data['displayName'] ?? null, $data['phoneNumber'] ?? null, $data['photoUrl'] ?? null, $data['disabled'] ?? \false, self::userMetaDataFromResponseData($data), $providerUserInfo, self::mfaInfoFromResponseData($data), $data['passwordHash'] ?? null, $data['salt'] ?? null, $customClaims, $data['tenantId'] ?? null, $validSince);
    }
    /**
    ...
    {
        return array_map(static fn(array $userInfoData): UserInfo => UserInfo::fromResponseData($userInfoData), $data['providerUserInfo']);
    }
}
