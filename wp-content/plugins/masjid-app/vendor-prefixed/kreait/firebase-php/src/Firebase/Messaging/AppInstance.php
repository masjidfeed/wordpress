...
final class AppInstance implements JsonSerializable
{
     ...
     */
    private function __construct(private readonly RegistrationToken $registrationToken, private readonly TopicSubscriptions $topicSubscriptions, private readonly array $rawData)
    {
    }
    /**
     * @internal
    ...
    public function isSubscribedToTopic(Topic|string $topic): bool
    {
        ...
        return $this->topicSubscriptions->filter(static fn(TopicSubscription $subscription): bool => $topic->value() === $subscription->topic()->value())->count() > 0;
    }
    /**
