...
final class MulticastSendReport implements Countable
{
    ...
    {
        return $this->successes()->filter(static fn(SendReport $report): bool => $report->target()->type() === MessageTarget::TOKEN)->map(static fn(SendReport $report): string => $report->target()->value());
    }
    /**
    ...
    {
        return $this->filter(static fn(SendReport $report): bool => $report->messageWasSentToUnknownToken())->map(static fn(SendReport $report): string => $report->target()->value());
    }
    /**
    ...
    {
        return $this->filter(static fn(SendReport $report): bool => $report->messageTargetWasInvalid())->map(static fn(SendReport $report): string => $report->target()->value());
    }
