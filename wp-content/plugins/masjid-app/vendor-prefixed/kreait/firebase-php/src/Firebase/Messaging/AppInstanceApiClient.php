...
use Masjid_App\Dependencies\GuzzleHttp\Promise\Create;
...
{
    public function __construct(private readonly ClientInterface $client, private readonly MessagingApiExceptionConverter $errorHandler)
    {
    }
    /**
    ...
    public function subscribeToTopics(array $topics, RegistrationTokens $tokens): array
    {
        ...
        foreach ($topics as $topic) {
            ...
            $promises[$topicName] = $this->client->requestAsync('POST', '/iid/v1:batchAdd', ['json' => ['to' => '/topics/' . $topicName, 'registration_tokens' => $tokenStrings]])->then(static fn(ResponseInterface $response): mixed => Json::decode((string) $response->getBody(), \true));
        }
    ...
    public function unsubscribeFromTopics(array $topics, RegistrationTokens $tokens): array
    {
        ...
        foreach ($topics as $topic) {
            $topicName = $topic->value();
            $promises[$topicName] = $this->client->requestAsync('POST', '/iid/v1:batchRemove', ['json' => ['to' => '/topics/' . $topicName, 'registration_tokens' => $tokenStrings]])->then(static fn(ResponseInterface $response): mixed => Json::decode((string) $response->getBody(), \true));
        }
        $responses = Utils::settle($promises)->wait();
        $result = [];
        foreach ($responses as $topicName => $response) {
    ...
    {
        return $this->client->requestAsync('GET', '/iid/' . $registrationToken->value() . '?details=true')->then(static function (ResponseInterface $response) use ($registrationToken): AppInstance {
