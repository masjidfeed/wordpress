...
final class WebPushConfig implements JsonSerializable
{
    ...
    private const VALID_URGENCIES = [self::URGENCY_VERY_LOW, self::URGENCY_LOW, self::URGENCY_NORMAL, self::URGENCY_HIGH];
    /**
    ...
    private static function ensureValidHeaders(array $headers): array
    {
        ...
        if (in_array($headers['Urgency'], self::VALID_URGENCIES, \true)) {
            return $headers;
        }
        throw new InvalidArgument(sprintf('The Urgency in the WebPushConfig header must must be one of %s', implode(',', self::VALID_URGENCIES)));
    }
}
