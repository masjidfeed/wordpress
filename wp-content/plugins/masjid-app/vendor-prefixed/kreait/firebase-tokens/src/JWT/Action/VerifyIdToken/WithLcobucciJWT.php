...
use Masjid_App\Dependencies\Beste\Clock\FrozenClock;
use DateInterval;
...
use Masjid_App\Dependencies\Kreait\Firebase\JWT\InsecureToken;
use Masjid_App\Dependencies\Kreait\Firebase\JWT\SecureToken;
use Masjid_App\Dependencies\Kreait\Firebase\JWT\Signer\None;
...
use Masjid_App\Dependencies\Lcobucci\JWT\Validation\Constraint\IssuedBy;
use Masjid_App\Dependencies\Lcobucci\JWT\Validation\Constraint\LooseValidAt;
use Masjid_App\Dependencies\Lcobucci\JWT\Validation\Constraint\PermittedFor;
use Masjid_App\Dependencies\Lcobucci\JWT\Validation\Constraint\SignedWith;
...
final class WithLcobucciJWT implements Handler
{
     ...
     */
    public function __construct(private readonly string $projectId, private readonly Keys $keys, private readonly ClockInterface $clock)
    {
    ...
    public function handle(VerifyIdToken $action): Token
    {
        ...
        $errors = [];
        $constraints = [new LooseValidAt($clock, $leeway), new IssuedBy(...["https://securetoken.google.com/{$this->projectId}"]), new PermittedFor($this->projectId)];
        ...
        try {
            ...
            $errors = array_filter(array_map(static fn(ConstraintViolation $violation): string => $violation->getMessage(), $e->violations()));
        }
        if (!empty($errors)) {
    ...
    private function assertUserAuthedAt(UnencryptedToken $token, DateTimeInterface $now): void
    {
        ...
        if (!$authTime) {
            throw RequiredConstraintsViolated::fromViolations(new ConstraintViolation('The token is missing the "auth_time" claim.'));
        }
        if (is_numeric($authTime)) {
            $authTime = new DateTimeImmutable('@' . (int) $authTime);
        }
        if ($now < $authTime) {
            throw RequiredConstraintsViolated::fromViolations(new ConstraintViolation("The token's user must have authenticated in the past"));
        }
    }
    private function assertTenantId(UnencryptedToken $token, string $tenantId): void
    {
        ...
        if (!is_string($tenant)) {
            throw RequiredConstraintsViolated::fromViolations(new ConstraintViolation('The ID token does not contain a tenant identifier'));
        }
        if ($tenant !== $tenantId) {
            throw RequiredConstraintsViolated::fromViolations(new ConstraintViolation("The token's tenant ID did not match with the expected tenant ID"));
        }
    }
}
