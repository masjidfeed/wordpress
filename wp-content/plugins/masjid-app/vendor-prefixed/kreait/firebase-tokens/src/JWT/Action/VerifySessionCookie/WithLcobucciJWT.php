...
final class WithLcobucciJWT implements Handler
{
    ...
    public function handle(VerifySessionCookie $action): Token
    {
        ...
        $errors = [];
        $constraints = [new LooseValidAt($clock, $leeway), new IssuedBy(...["https://session.firebase.google.com/{$this->projectId}"]), new PermittedFor($this->projectId)];
        ...
        try {
            ...
            $errors = array_map(static fn(ConstraintViolation $violation): string => '- ' . $violation->getMessage(), $e->violations());
        }
        if (!empty($errors)) {
