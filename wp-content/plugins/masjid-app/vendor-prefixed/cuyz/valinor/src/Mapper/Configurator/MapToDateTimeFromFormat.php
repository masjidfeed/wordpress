...
use Attribute;
use Masjid_App\Dependencies\CuyZ\Valinor\Mapper\AsConverter;
...
final class MapToDateTimeFromFormat
{
    ...
    public function map(string $value): DateTimeImmutable
    {
        ...
        if ($date === \false) {
            throw MessageBuilder::newError('Value {source_value} does not match the date format {format}.')->withParameter('format', "`{$this->format}`")->withCode('cannot_parse_datetime_format')->build();
        }
        return $date;
    }
}
