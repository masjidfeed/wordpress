...
use Masjid_App\Dependencies\CuyZ\Valinor\Mapper\Tree\Exception\SourceMustBeIterable;
...
{
    public function __construct(private HttpRequestNodeBuilder $httpRequestNodeBuilder)
    {
    }
    public function build(Shell $shell): Node
    {
        ...
        if ($type->isUnsealed()) {
            if ($type instanceof ShapedListType) {
                ...
                $elementType = $unsealedType instanceof VacantType ? MixedType::get() : $unsealedType->subType();
                ...
                foreach ($value as $key => $val) {
                    ...
                    $expectedKey++;
                    $child = $shell->child((string) $key, $elementType)->withValue($val)->build();
                    if ($child->isValid()) {
                        $children[$key] = $child->value();
                    } else {
                        $errors[] = $child;
    ...
    private function wrapSingleValueIfNeeded(Shell $shell): Shell
    {
        ...
        // are allowed.
        $alreadyShaped = is_array($value) && array_key_exists($key, $value) && (!$elementIsTraversable || $shell->allowSuperfluousKeys || count($value) === 1);
        ...
        return $shell->withValue([$key => $value])->withPathMap(["{$shell->path}.{$key}" => $shell->path]);
    }
}
