...
final readonly class Arguments implements IteratorAggregate, Countable
{
    ...
    public function __construct(Argument ...$arguments)
    {
        ...
        $this->arguments = $args;
        $this->shapedArray = new ShapedArrayType(elements: array_map(static fn(Argument $argument) => new ShapedArrayElement(key: new StringValueType($argument->name()), type: $argument->type(), optional: !$argument->isRequired(), attributes: $argument->attributes()), $this->arguments));
    }
    ...
    public function withoutSelfReferencingSingleArgument(mixed $value, string $className): self
    {
        ...
        $subTypes = $type->types();
        $filtered = array_filter($subTypes, static fn(Type $subType) => !$subType instanceof ObjectType || $subType->className() !== $className);
