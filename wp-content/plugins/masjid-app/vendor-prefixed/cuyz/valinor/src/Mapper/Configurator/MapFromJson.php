...
final class MapFromJson
{
    ...
    {
        try {
            $decoded = json_decode($value, associative: \true, flags: JSON_THROW_ON_ERROR);
        } catch (JsonException) {
            throw MessageBuilder::newError('Value {source_value} is not valid JSON.')->withCode('invalid_json')->build();
        }
        return $next($decoded);
    }
}
