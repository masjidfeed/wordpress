...
use Masjid_App\Dependencies\CuyZ\Valinor\Mapper\Tree\Exception\MissingNodeValue;
use Masjid_App\Dependencies\CuyZ\Valinor\Mapper\Tree\Exception\UnresolvableShellType;
...
final class Shell
{
    ...
    public function child(string $name, Type $type): self
    {
        ...
        if (array_key_exists($name, $this->childrenWithScalarValueCasting)) {
            ...
            $self->allowCastingToInteger = \true;
            $self->allowCastingToFloat = \true;
            $self->allowCastingToString = \true;
        }
        return $self;
    }
    ...
    public function wrapSingleValueIfNeeded(): self
    {
        // @infection-ignore-all / We don't want to test the clone behavior
        $self = clone $this;
        $self->wrapSingleValueIfNeeded = \true;
        return $self;
    }
    public function shouldApplyConverters(): self
    {
        // @infection-ignore-all / We don't want to test the clone behavior
        $self = clone $this;
        $self->shouldApplyConverters = \true;
        return $self;
    }
