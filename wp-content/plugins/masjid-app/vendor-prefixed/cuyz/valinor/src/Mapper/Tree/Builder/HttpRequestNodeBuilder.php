...
use Masjid_App\Dependencies\CuyZ\Valinor\Mapper\Http\FromBody;
use Masjid_App\Dependencies\CuyZ\Valinor\Mapper\Http\FromQuery;
use Masjid_App\Dependencies\CuyZ\Valinor\Mapper\Http\FromRoute;
use Masjid_App\Dependencies\CuyZ\Valinor\Mapper\Http\HttpRequest;
use Masjid_App\Dependencies\CuyZ\Valinor\Mapper\Tree\Exception\CannotMapHttpRequestToUnsealedShapedArray;
use Masjid_App\Dependencies\CuyZ\Valinor\Mapper\Tree\Exception\CannotUseBothFromBodyAttributes;
use Masjid_App\Dependencies\CuyZ\Valinor\Mapper\Tree\Exception\CannotUseBothFromQueryAttributes;
use Masjid_App\Dependencies\CuyZ\Valinor\Mapper\Tree\Exception\HttpRequestKeyCollision;
    ...
    public function build(Shell $shell): Node
    {
        ...
        foreach ($type->elements as $key => $element) {
            ...
            if ($attributes->has(FromRoute::class)) {
                ...
                $queryAttributes++;
                $queryAsRoot = $queryAsRoot || $attributes->firstOf(FromQuery::class)->instantiate()->asRoot;
                ...
                $bodyAttributes++;
                $bodyAsRoot = $bodyAsRoot || $attributes->firstOf(FromBody::class)->instantiate()->asRoot;
                // @phpstan-ignore property.notFound
                if ($bodyAsRoot) {
                    if ($bodyAttributes > 1) {
        ...
        return $shell->allowScalarValueCastingForChildren(array_keys($route + $query))->withValue($requestObjects + $route + $query + $body)->build();
    }
}
