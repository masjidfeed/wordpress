...
use Masjid_App\Dependencies\CuyZ\Valinor\Type\Parser\Exception\Union\InvalidClassStringElements;
...
final class ClassStringType implements StringType, CompositeType
{
    ...
    public function accepts(mixed $value): bool
    {
        ...
        foreach ($this->subTypes as $type) {
            if ($type instanceof ObjectType && is_a($value, $type->className(), \true)) {
                return \true;
            }
        }
        return \false;
    ...
    public function compiledAccept(Node $node): Node
    {
        ...
        foreach ($this->subTypes as $type) {
            if ($type instanceof ObjectType) {
                $conditions[] = call('is_a', [$node, value($type->className()), value(\true)]);
            }
        }
