...
final class NonEmptyArrayType implements CompositeTraversableType, DumpableType
{
    ...
    {
        $condition = logicalAnd($node->different(value([])), call('is_array', [$node]));
        if ($this === self::native()) {
            return $condition;
        }
        return $condition->and(call(Polyfill::array_all_name(), [$node, shortClosure(return: logicalAnd($this->keyType->compiledAccept(variable('key'))->wrap(), $this->subType->compiledAccept(variable('item'))->wrap()), parameters: [param('item', 'mixed'), param('key', 'mixed')])]));
    }
