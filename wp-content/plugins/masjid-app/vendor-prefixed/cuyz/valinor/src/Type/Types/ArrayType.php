...
final class ArrayType implements CompositeTraversableType, DumpableType
{
    ...
    {
        return new self(ArrayKeyType::default(), $type, simple: \true);
    }
    ...
    public function compiledAccept(Node $node): Node
    {
        ...
        return $condition->and(call(Polyfill::array_all_name(), [$node, shortClosure(logicalAnd($this->keyType->compiledAccept(variable('key'))->wrap(), $this->subType->compiledAccept(variable('item'))->wrap()), parameters: [param('item', 'mixed'), param('key', 'mixed')])]));
    }
