...
use Masjid_App\Dependencies\CuyZ\Valinor\Type\Parser\Exception\Iterable\InvalidShapedArrayUnsealedType;
use Masjid_App\Dependencies\CuyZ\Valinor\Type\Parser\Exception\Iterable\ShapedArrayElementDuplicatedKey;
...
final class ShapedArrayType implements CompositeType, DumpableType
{
    ...
    {
        $conditions = [call('is_array', [$node]), ...array_map(function (ShapedArrayElement $element) use ($node) {
        ...
        if ($this->unsealedType) {
            ...
            $conditions[] = call(Polyfill::array_all_name(), [call('array_diff_key', [$node, value($elementsKeys)]), shortClosure(logicalAnd($unsealedType->keyType()->compiledAccept(variable('key'))->wrap(), $unsealedType->subType()->compiledAccept(variable('item'))->wrap()), parameters: [param('item', 'mixed'), param('key', 'mixed')])]);
        }
