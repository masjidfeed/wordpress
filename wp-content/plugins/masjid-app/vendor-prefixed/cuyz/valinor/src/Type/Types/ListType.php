...
final class ListType implements CompositeTraversableType, DumpableType
{
    ...
    {
        $condition = logicalAnd(call('is_array', [$node]), call('array_is_list', [$node]));
        if ($this === self::native()) {
            return $condition;
        }
        return $condition->and(call(Polyfill::array_all_name(), [$node, shortClosure($this->subType->compiledAccept(variable('item'))->wrap(), parameters: [param('item', 'mixed')])]));
    }
