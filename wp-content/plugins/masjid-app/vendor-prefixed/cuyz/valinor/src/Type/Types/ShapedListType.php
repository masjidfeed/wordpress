...
use Masjid_App\Dependencies\CuyZ\Valinor\Type\Parser\Exception\Iterable\InvalidShapedListUnsealedType;
use Masjid_App\Dependencies\CuyZ\Valinor\Type\Parser\Exception\Iterable\ShapedListInvalidKey;
use Masjid_App\Dependencies\CuyZ\Valinor\Type\Parser\Exception\Iterable\ShapedListMandatoryAfterOptionalElement;
...
final class ShapedListType implements CompositeType, DumpableType
{
     ...
     */
    public static function from(array $elements, Type|null $unsealedType = null): self
    {
    ...
    {
        $conditions = [call('is_array', [$node]), call('array_is_list', [$node]), ...array_map(function (ShapedArrayElement $element) use ($node) {
            $key = value($element->key()->value());
            return ternary(condition: call('array_key_exists', [$key, $node]), ifTrue: $element->type()->compiledAccept($node->key($key)), ifFalse: value($element->isOptional()))->wrap();
        }, array_values($this->elements))];
        if ($this->unsealedType) {
            if ($this->unsealedType instanceof VacantType) {
                return value(\false);
            }
            $conditions[] = call(Polyfill::array_all_name(), [call('array_slice', [$node, value(count($this->elements))]), shortClosure($this->unsealedType->subType()->compiledAccept(variable('item'))->wrap(), parameters: [param('item', 'mixed')])]);
        } else {
            $conditions[] = call('count', [$node])->isLessOrEqualsTo(value(count($this->elements)));
        }
    ...
    public function matches(Type $other): bool
    {
        ...
        if ($other instanceof self) {
            ...
            if ($other->unsealedType) {
                return $this->unsealedType && $this->unsealedType->matches($other->unsealedType);
            }
            return \true;
        }
    ...
    {
        $hasOptional = $this->hasOptionalElements();
        $parts = array_map(static fn(ShapedArrayElement $element) => $hasOptional ? $element->key()->value() . ($element->isOptional() ? '?: ' : ': ') . $element->type()->toString() : $element->type()->toString(), $this->elements);
    ...
    {
        return Polyfill::array_any($this->elements, static fn(ShapedArrayElement $element) => $element->isOptional());
    }
}
