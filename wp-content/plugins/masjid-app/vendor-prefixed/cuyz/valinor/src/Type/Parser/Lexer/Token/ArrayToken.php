...
use Masjid_App\Dependencies\CuyZ\Valinor\Type\Parser\Exception\Iterable\ArrayClosingBracketMissing;
use Masjid_App\Dependencies\CuyZ\Valinor\Type\Parser\Exception\Iterable\ArrayCommaMissing;
use Masjid_App\Dependencies\CuyZ\Valinor\Type\Parser\Exception\Iterable\ArrayExpectedCommaOrClosingBracket;
use Masjid_App\Dependencies\CuyZ\Valinor\Type\Parser\Exception\Iterable\ArrayMissingSubType;
use Masjid_App\Dependencies\CuyZ\Valinor\Type\Parser\Exception\Iterable\ShapedArrayClosingBracketMissing;
use Masjid_App\Dependencies\CuyZ\Valinor\Type\Parser\Exception\Iterable\ShapedArrayColonTokenMissing;
use Masjid_App\Dependencies\CuyZ\Valinor\Type\Parser\Exception\Iterable\ShapedArrayCommaMissing;
use Masjid_App\Dependencies\CuyZ\Valinor\Type\Parser\Exception\Iterable\ShapedArrayElementTypeMissing;
use Masjid_App\Dependencies\CuyZ\Valinor\Type\Parser\Exception\Iterable\ShapedArrayUnexpectedTokenAfterSealedType;
use Masjid_App\Dependencies\CuyZ\Valinor\Type\Parser\Exception\Iterable\ShapedArrayWithoutElementsWithSealedType;
use Masjid_App\Dependencies\CuyZ\Valinor\Type\Parser\Exception\Iterable\ShapedListMixedKeys;
...
final class ArrayToken implements TraversingToken
{
    ...
    private function shapedType(TokenStream $stream): Type
    {
        ...
        return $this->isList ? ShapedListType::from($elements, $unsealedType) : ShapedArrayType::from($elements, $unsealedType);
    }
    /**
    ...
    {
        if (!$this->isList) {
            return 'array{' . implode(', ', array_map(static fn(ShapedArrayElement $element) => $element->toString(), $elements));
        }
        $hasOptional = Polyfill::array_any($elements, static fn(ShapedArrayElement $element) => $element->isOptional());
        return 'list{' . implode(', ', array_map(static fn(ShapedArrayElement $element) => $hasOptional ? $element->key()->value() . ($element->isOptional() ? '?: ' : ': ') . $element->type()->toString() : $element->type()->toString(), $elements));
    }
}
