...
use Masjid_App\Dependencies\CuyZ\Valinor\Type\Parser\Exception\Magic\KeyOfClosingBracketMissing;
use Masjid_App\Dependencies\CuyZ\Valinor\Type\Parser\Exception\Magic\KeyOfIncorrectSubType;
use Masjid_App\Dependencies\CuyZ\Valinor\Type\Parser\Exception\Magic\KeyOfMissingSubType;
use Masjid_App\Dependencies\CuyZ\Valinor\Type\Parser\Exception\Magic\KeyOfOpeningBracketMissing;
...
final class KeyOfToken implements TraversingToken
{
    ...
    public function traverse(TokenStream $stream): Type
    {
        ...
        if ($subType instanceof EnumType) {
            $keys = array_map(fn(UnitEnum $case) => StringValueType::quoted("'{$case->name}'"), array_values($subType->cases()));
            if (count($keys) > 1) {
