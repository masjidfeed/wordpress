...
final class IterableType implements CompositeTraversableType, DumpableType
{
    ...
    {
        $condition = logicalAnd(call('is_iterable', [$node]), negate($node->instanceOf(Generator::class)));
        if ($this === self::native()) {
            return $condition;
        }
        return $condition->and(call(Polyfill::array_all_name(), [call('iterator_to_array', [$node]), shortClosure(logicalAnd($this->keyType->compiledAccept(variable('key'))->wrap(), $this->subType->compiledAccept(variable('item'))->wrap()), parameters: [param('item', 'mixed'), param('key', 'mixed')])]));
    }
