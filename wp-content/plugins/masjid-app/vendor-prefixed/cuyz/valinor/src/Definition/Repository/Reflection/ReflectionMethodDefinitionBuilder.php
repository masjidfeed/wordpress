...
final class ReflectionMethodDefinitionBuilder
{
    ...
    public function __construct(AttributesRepository $attributesRepository, private TypeParserFactory $typeParserFactory)
    {
    ...
    public function for(ReflectionMethod $reflection, ReflectionTypeResolver $typeResolver): MethodDefinition
    {
        ...
        return new MethodDefinition($reflection->name, $signature, new Attributes(...$this->attributesRepository->for($reflection)), new Parameters(...$parameters), $reflection->isStatic(), $reflection->isPublic(), $returnType);
    }
}
