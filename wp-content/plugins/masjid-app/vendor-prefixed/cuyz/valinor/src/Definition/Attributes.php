...
final class Attributes implements IteratorAggregate, Countable
{
    ...
    public function has(string $className): bool
    {
        foreach ($this->attributes as $attribute) {
            if (is_a($attribute->class->type->className(), $className, \true)) {
                return \true;
            }
        }
        return \false;
    ...
    public function firstOf(string $className): AttributeDefinition
    {
        ...
        $filtered = array_filter($this->attributes, static fn(AttributeDefinition $attribute) => is_a($attribute->class->type->className(), $className, \true));
