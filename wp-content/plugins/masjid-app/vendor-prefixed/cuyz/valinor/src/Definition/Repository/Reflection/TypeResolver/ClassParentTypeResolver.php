...
use Masjid_App\Dependencies\CuyZ\Valinor\Type\Parser\VacantTypeAssignerParser;
...
final class ClassParentTypeResolver
{
    ...
    private function findDeclaringInterface(ReflectionClass $reflection, ReflectionProperty|ReflectionMethod $member): ReflectionClass
    {
        ...
        foreach ($interfaces as $interface) {
            ...
            $hasMember = $member instanceof ReflectionProperty ? $interface->hasProperty($member->name) : $interface->hasMethod($member->name);
            if ($hasMember) {
                $parent = $interface;
            }
        }
    ...
    private function unresolvableGenericsFor(ReflectionClass $class, UnresolvableType $unresolvableType): array
    {
        ...
        return array_values(array_map(static fn(GenericType $type) => new UnresolvableType($type->symbol, $unresolvableType->message()), $templates));
    }
}
