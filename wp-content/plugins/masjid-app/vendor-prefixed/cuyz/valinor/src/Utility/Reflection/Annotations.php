...
use Masjid_App\Dependencies\CuyZ\Valinor\Type\Parser\Lexer\TokenizedAnnotation;
...
final class Annotations
{
    ...
    public static function forTemplates(ReflectionClass|ReflectionFunctionAbstract $reflection): array
    {
        ...
        return (new self($docBlock))->filteredByPriority('@phpstan-template', '@phpstan-template-covariant', '@psalm-template', '@psalm-template-covariant', '@template', '@template-covariant');
    }
    /**
     * @param class-string $className
     * @return list<TokenizedAnnotation>
    ...
    {
        return (new self($function->getDocComment()))->filteredByPriority('@valinor-param', '@phpstan-param', '@psalm-param', '@param');
    }
    public static function forProperty(ReflectionProperty $function): ?string
    {
        return (new self($function->getDocComment()))->firstOf('@valinor-var', '@phpstan-var', '@psalm-var', '@var')?->raw();
    }
    public static function forFunctionReturnType(ReflectionFunctionAbstract $function): ?string
    {
        return (new self($function->getDocComment()))->firstOf('@valinor-return', '@phpstan-return', '@psalm-return', '@return')?->raw();
    }
    ...
    {
        return array_values(array_filter($this->annotations, static fn(TokenizedAnnotation $tokenizedAnnotation) => in_array($tokenizedAnnotation->name(), $allowed, \true)));
    }
    /**
     * @return list<TokenizedAnnotation>
     */
