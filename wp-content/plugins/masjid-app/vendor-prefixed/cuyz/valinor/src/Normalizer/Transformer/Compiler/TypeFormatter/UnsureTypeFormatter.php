...
final class UnsureTypeFormatter implements TypeFormatter
{
    ...
    public function manipulateTransformerClass(AnonymousClassNode $class, TransformerDefinitionBuilder $definitionBuilder): AnonymousClassNode
    {
        ...
        return $class->withMethod(name: $methodName, parameters: [param('value', 'mixed'), param('references', WeakMap::class)], returnType: 'mixed', body: [if_(condition: negate((new TypeAcceptNode(variable('value'), $this->unsureType->nativeType()))->wrap()), body: return_($defaultDefinition->typeFormatter()->formatValueNode(variable('value')))), return_($this->delegate->formatValueNode(variable('value')))]);
    }
    /**
