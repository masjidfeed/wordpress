...
final class RegisteredTransformersFormatter implements TypeFormatter
{
    ...
    public function manipulateTransformerClass(AnonymousClassNode $class, TransformerDefinitionBuilder $definitionBuilder): AnonymousClassNode
    {
        ...
        $nodes = [variable('next')->assign(shortClosure($this->delegate->formatValueNode(variable('value'))))->asStatement(), ...array_map(fn(int $key, Type $transformerType) => if_(condition: new TypeAcceptNode(variable('value'), $transformerType), body: variable('next')->assign(shortClosure(return: this()->access('transformers')->key(value($key))->call(arguments: [variable('value'), variable('next')])))->asStatement()), array_keys($this->transformerTypes), $this->transformerTypes), ...array_map(function (AttributeDefinition $attribute) {
            $node = variable('next')->assign(shortClosure(return: (new NewAttributeNode($attribute))->wrap()->callMethod(method: 'normalize', arguments: [variable('value'), variable('next')])))->asStatement();
            ...
            return $node;
        }, $this->transformerAttributes), return_(variable('next')->call())];
