...
use function Masjid_App\Dependencies\CuyZ\Valinor\Compiler\closure;
...
use function Masjid_App\Dependencies\CuyZ\Valinor\Compiler\yield_;
use function hash;
...
final class TraversableFormatter implements TypeFormatter
{
    ...
    public function manipulateTransformerClass(AnonymousClassNode $class, TransformerDefinitionBuilder $definitionBuilder): AnonymousClassNode
    {
        ...
        return $class->withMethod(name: $methodName, parameters: [param('value', 'iterable'), param('references', WeakMap::class)], returnType: 'iterable', body: [if_(condition: call('is_array', [variable('value')]), body: return_(call(name: 'array_map', arguments: [shortClosure(return: $subDefinition->typeFormatter()->formatValueNode(variable('item')), parameters: [param('item', 'mixed')]), variable('value')]))), return_(closure(body: [forEach_(value: variable('value'), key: 'key', item: 'item', body: yield_(key: variable('key'), value: $subDefinition->typeFormatter()->formatValueNode(variable('item')))->asStatement())], uses: ['value', 'references'])->wrap()->call())]);
    }
    /**
