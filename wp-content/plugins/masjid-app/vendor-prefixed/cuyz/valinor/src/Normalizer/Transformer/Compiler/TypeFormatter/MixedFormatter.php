...
final class MixedFormatter implements TypeFormatter
{
    ...
    {
        return this()->callMethod(method: 'transform_mixed', arguments: [$valueNode, variable('references')]);
    }
    public function manipulateTransformerClass(AnonymousClassNode $class, TransformerDefinitionBuilder $definitionBuilder): AnonymousClassNode
    {
        ...
        $nodes[] = return_(this()->access('delegate')->callMethod('transform', [variable('value')]));
        return $class->withMethod(name: 'transform_mixed', parameters: [param('value', 'mixed'), param('references', WeakMap::class)], returnType: 'mixed', body: $nodes);
    }
}
