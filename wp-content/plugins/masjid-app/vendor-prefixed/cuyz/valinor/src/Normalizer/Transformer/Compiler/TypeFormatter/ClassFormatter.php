...
use Masjid_App\Dependencies\CuyZ\Valinor\Compiler\Library\NewAttributeNode;
...
use Masjid_App\Dependencies\CuyZ\Valinor\Normalizer\Exception\CircularReferenceFoundDuringNormalization;
...
use WeakMap;
use function Masjid_App\Dependencies\CuyZ\Valinor\Compiler\array_;
use function Masjid_App\Dependencies\CuyZ\Valinor\Compiler\call;
use function Masjid_App\Dependencies\CuyZ\Valinor\Compiler\clone_;
...
use function Masjid_App\Dependencies\CuyZ\Valinor\Compiler\throw_;
...
final class ClassFormatter implements TypeFormatter
{
    ...
    public function manipulateTransformerClass(AnonymousClassNode $class, TransformerDefinitionBuilder $definitionBuilder): AnonymousClassNode
    {
        ...
        $nodes = [...$this->checkCircularObjectReference(), variable('values')->assign($valuesNode)->asStatement()];
        $transformedNodes = [variable('transformed')->assign(value([]))->asStatement()];
        ...
        foreach ($this->class->properties as $property) {
            ...
            if ($keyTransformerAttributes === []) {
                ...
                foreach ($keyTransformerAttributes as $attribute) {
                    $transformedNodes[] = variable('key')->assign((new NewAttributeNode($attribute))->wrap()->callMethod(method: 'normalizeKey', arguments: [variable('key')]))->asStatement();
                }
                $key = variable('key');
            }
            $transformedNodes[] = variable('transformed')->key($key)->assign($typeFormatter->formatValueNode(variable('values')->key(value($property->name))))->asStatement();
        ...
        if ($shouldUseTransformedNodes) {
            $nodes = [...$nodes, ...$transformedNodes, return_(variable('transformed'))];
        } else {
            $nodes[] = return_(variable('values'));
        }
        return $class->withMethod(name: $methodName, parameters: [param('value', str_contains($this->class->name, '@anonymous') ? 'object' : $this->class->name), param('references', WeakMap::class)], returnType: 'array', body: $nodes);
    }
    /**
     * @return list<Node>
     */
    ...
    {
        return [if_(condition: call('isset', [variable('references')->key(variable('value'))]), body: throw_(newClass(CircularReferenceFoundDuringNormalization::class, variable('value')))->asStatement()), variable('references')->assign(clone_(variable('references')))->asStatement(), variable('references')->key(variable('value'))->assign(variable('value'))->asStatement()];
    }
    /**
    ...
    private function valuesNode(Node $valueNode): Node
    {
        ...
        return shortClosure(return: call(name: 'get_object_vars', arguments: [this()]))->wrap()->callMethod('call', [$valueNode]);
    }
    /**
