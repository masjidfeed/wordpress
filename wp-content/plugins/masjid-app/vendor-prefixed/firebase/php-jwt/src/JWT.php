...
class JWT
{
    ...
    {
        if (!\function_exists('sodium_crypto_sign_verify_detached') && !\function_exists('Masjid_App\Dependencies\sodium_crypto_sign_verify_detached')) {
    ...
    {
        if (!class_exists('Masjid_App\Dependencies\phpseclib3\Crypt\RSA')) {
        ...
        $rsa = \Masjid_App\Dependencies\phpseclib3\Crypt\PublicKeyLoader::load($key);
        return $rsa->withPadding(\Masjid_App\Dependencies\phpseclib3\Crypt\RSA::SIGNATURE_PSS)->withHash('sha256')->sign($message);
    }
    /**
    ...
    ): bool
    {
        ...
        $rsa = \Masjid_App\Dependencies\phpseclib3\Crypt\PublicKeyLoader::load($key);
        return $rsa->withPadding(\Masjid_App\Dependencies\phpseclib3\Crypt\RSA::SIGNATURE_PSS)->withHash('sha256')->verify($message, $signature);
    }
}
