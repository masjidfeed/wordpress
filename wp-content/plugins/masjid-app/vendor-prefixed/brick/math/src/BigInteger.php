...
namespace Masjid_App\Dependencies\Brick\Math;

use Masjid_App\Dependencies\Brick\Math\Exception\DivisionByZeroException;
use Masjid_App\Dependencies\Brick\Math\Exception\IntegerOverflowException;
use Masjid_App\Dependencies\Brick\Math\Exception\InvalidArgumentException;
use Masjid_App\Dependencies\Brick\Math\Exception\MathException;
use Masjid_App\Dependencies\Brick\Math\Exception\NegativeNumberException;
use Masjid_App\Dependencies\Brick\Math\Exception\NoInverseException;
use Masjid_App\Dependencies\Brick\Math\Exception\NumberFormatException;
use Masjid_App\Dependencies\Brick\Math\Exception\RandomSourceException;
use Masjid_App\Dependencies\Brick\Math\Exception\RoundingNecessaryException;
use Masjid_App\Dependencies\Brick\Math\Internal\Calculator;
use Masjid_App\Dependencies\Brick\Math\Internal\CalculatorRegistry;
use Masjid_App\Dependencies\Brick\Math\Internal\Safe;
use LogicException;
use Override;
use Throwable;
...
final readonly class BigInteger extends BigNumber
{
     ...
     * @pure
     */
    public static function fromBytes(string $bytes, bool $signed = \true): BigInteger
    {
        if ($bytes === '') {
            // @phpstan-ignore identical.alwaysFalse
     ...
     * @throws RandomSourceException    If random byte generation fails.
     */
    public static function randomRange(BigNumber|int|string $min, BigNumber|int|string $max, ?callable $randomBytesGenerator = null): BigInteger
    {
    ...
    public function nthRoot(int $n, RoundingMode $roundingMode = RoundingMode::Unnecessary): BigInteger
    {
        ...
        $nextStep = $isPositive ? $calculator->add($truncatedRoot, '1') : $calculator->sub($truncatedRoot, '1');
