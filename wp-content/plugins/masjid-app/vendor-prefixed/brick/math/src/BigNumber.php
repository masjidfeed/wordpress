...
abstract readonly class BigNumber implements JsonSerializable, Stringable
{
     ...
     */
    private const PARSE_REGEXP_NUMERICAL = '/^' . '(?<sign>[\-\+])?' . '(?<integral>[0-9]+)?' . '(?<point>\.)?' . '(?<fractional>[0-9]+)?' . '(?:[eE](?<exponent>[\-\+]?[0-9]+))?' . '\z/';
    /**
     * The regular expression used to parse rational numbers.
     *
     * The end anchor must be \z, not $: the latter would also match before a trailing newline.
     */
    private const PARSE_REGEXP_RATIONAL = '/^' . '(?<sign>[\-\+])?' . '(?<numerator>[0-9]+)' . '\/' . '(?<denominator>[0-9]+)' . '\z/';
    /**
    ...
    private static function _of(BigNumber|int|string $value): BigNumber
    {
        ...
        if (str_contains($value, '/')) {
            ...
            return new BigRational(new BigInteger($numerator), new BigInteger($denominator), \false, \true);
        }
