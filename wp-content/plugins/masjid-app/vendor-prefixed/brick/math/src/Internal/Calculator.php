...
namespace Masjid_App\Dependencies\Brick\Math\Internal;

use Masjid_App\Dependencies\Brick\Math\RoundingMode;
use function chr;
use function intdiv;
use function ltrim;
use function ord;
...
abstract readonly class Calculator
{
    ...
    public function nthRoot(string $n, int $k): string
    {
        ...
        for (;;) {
            $nx = $this->divQ($this->add($this->mul($kMinusOneStr, $x), $this->divQ($m, $this->pow($x, $k - 1))), $kStr);
            if ($this->cmp($nx, $x) >= 0) {
                break;
            }
            $x = $nx;
