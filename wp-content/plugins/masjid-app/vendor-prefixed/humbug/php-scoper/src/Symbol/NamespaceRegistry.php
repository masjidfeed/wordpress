 ...
 */
namespace Masjid_App\Dependencies\Humbug\PhpScoper\Symbol;
...
use function ltrim;
use function Masjid_App\Dependencies\Safe\preg_match;
...
final readonly class NamespaceRegistry
{
    ...
    {
        return new self(array_values(array_unique(array_map(static fn(string $namespaceName) => strtolower(trim($namespaceName, '\\')), $namespaceNames), SORT_STRING)), array_values(array_unique($namespaceRegexes, SORT_STRING)));
    }
    /**
