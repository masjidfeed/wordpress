...
class EnumMethodsExistCheckTest extends CheckTestCase
{
    ...
    public function testMissingMethodInStubsIsReported(): void
    {
        ...
        $reflEnum = $this->makeEnum($enumId, [$this->makeMethod('cases'), $this->makeMethod('from')]);
        $stubEnum = $this->makeEnum($enumId, [$this->makeMethod('cases')]);
        $provider = $this->createMockReflectionProviderWithEnums([$reflEnum]);
        $stubs = $this->createMockStorageManager();
        $stubs->method('getEnums')->willReturn([$stubEnum]);
    ...
    public function testMethodFromBackedEnumInterfaceIsCounted(): void
    {
        ...
        $enumId = 'Masjid_App\Dependencies\Dom\AdjacentPosition';
        $reflEnum = $this->makeEnum($enumId, [$this->makeMethod('cases'), $this->makeMethod('from'), $this->makeMethod('tryFrom')]);
        $backedEnum = $this->makeInterface('\BackedEnum', [$this->makeMethod('from'), $this->makeMethod('tryFrom')]);
        $unitEnum = $this->makeInterface('\UnitEnum', [$this->makeMethod('cases')]);
        $stubEnum = $this->makeEnum($enumId);
    ...
    public function testEnumLevelKnownProblemSkipsAllMethodChecks(): void
    {
        ...
        $knownProblemsProvider->method('getProblems')->willReturn([new ProblemDefinition(entityType: EntityType::ENUM_TYPE, entityId: $enumId, type: ProblemType::INTERNAL_IMPLEMENTATION, affectedChecks: [CheckType::CLASS_METHODS_EXIST], versionRange: new PhpVersionRange(PhpVersions::PHP_8_1, PhpVersions::LATEST), reason: 'Enum-level skip reason')]);
        KnownProblemsRegistry::reset();
