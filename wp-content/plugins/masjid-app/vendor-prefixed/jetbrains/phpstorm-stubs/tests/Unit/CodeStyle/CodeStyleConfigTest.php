...
namespace Masjid_App\Dependencies\StubTests\Unit\CodeStyle;
...
namespace Masjid_App\Dependencies\Scratch;
...
PHP;
    ...
    public function testCsFixPreservesCommentOnlyBodiesButStillCollapsesEmptyOnes(): void
    {
        ...
        $command = sprintf('%s %s fix %s --config=%s --using-cache=no 2>&1', escapeshellarg(\PHP_BINARY), escapeshellarg($binary), escapeshellarg($file), escapeshellarg($config));
        $output = [];
        $exitCode = 0;
        ...
        self::assertStringContainsString('// Intentionally empty: the parent already does the work.', $fixed, 'A line comment that is a body\'s sole content must survive cs-fix');
        self::assertStringContainsString('/* keep this */', $fixed, 'A block comment that is a body\'s sole content must survive cs-fix');
        self::assertStringNotContainsString('public function noOp(): void {}', $fixed, 'A body holding a comment is not empty and must not be collapsed');
        self::assertStringContainsString('public function trulyEmpty(): void {}', $fixed, 'A genuinely empty body must still be collapsed onto one line');
    }
}
