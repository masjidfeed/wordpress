...
class ClassStaleMethodsCheckTest extends CheckTestCase
{
    ...
    {
        $stubClass = $this->makeStubClass('\DOMDocument', [$this->makeMethod('renameNode'), $this->makeMethod('createElement')], 'dom/dom_c.php');
    ...
    public function testIntentionallyStubOnlyMethodsAreNotReported(string $methodName): void
    {
        ...
        self::assertFalse($this->runCheck($stubClass, [$this->makeClass('\DOMDocument')])->hasFailures(), "{$methodName} is declared in stubs on purpose and must never be reported");
    }
    // ── Version awareness ────────────────────────────────────────────────────
    ...
    public function testInheritedParentMethodsAreNotAttributedToTheChild(): void
    {
        ...
        $result = $this->runCheck($stubClass, [$reflClass]);
        self::assertFalse($result->hasFailures(), 'Only methods declared on the class itself may be judged');
    }
    // ── Known problems ───────────────────────────────────────────────────────
    ...
    {
        $registry = $this->registryWith(new ProblemDefinition(entityType: EntityType::METHOD, entityId: '\DOMDocument::renameNode', type: ProblemType::INTERNAL_IMPLEMENTATION, affectedChecks: [CheckType::CLASS_STALE_METHODS], versionRange: new PhpVersionRange(PhpVersions::EARLIEST, PhpVersions::LATEST), reason: 'Stale method skip reason'));
    ...
    {
        $registry = $this->registryWith(new ProblemDefinition(entityType: EntityType::CLASS_TYPE, entityId: '\DOMDocument', type: ProblemType::INTERNAL_IMPLEMENTATION, affectedChecks: [CheckType::CLASS_STALE_METHODS], versionRange: new PhpVersionRange(PhpVersions::EARLIEST, PhpVersions::LATEST), reason: 'Whole-class skip reason'));
        $stubClass = $this->makeStubClass('\DOMDocument', [$this->makeMethod('renameNode')], 'dom/dom_c.php');
        $provider = $this->createMockReflectionProviderWithClasses([$this->makeClass('\DOMDocument')]);
        $stubs = $this->createMockStorageManager();
        $stubs->method('getClasses')->willReturn([$stubClass]);
    ...
    {
        $stubClass = $this->makeStubClass('\DOMDocument', [$this->makeMethod('zzz'), $this->makeMethod('aaa')], 'dom/dom_c.php');
        $failures = $this->runCheck($stubClass, [$this->makeClass('\DOMDocument')])->getFailures();
        self::assertSame(['\DOMDocument::aaa', '\DOMDocument::zzz'], array_keys($failures), 'Failures must be reported in sorted order so the output is stable across runs');
    }
