...
class FunctionReturnTypesCheckTest extends CheckTestCase
{
    ...
    public function testPhp7xReturnTypeWorksForVariousTypes(): void
    {
        ...
        foreach ($returnTypes as $returnType) {
            ...
            // Assert
            $this->assertFalse($result->hasFailures(), "Failed for return type: {$returnType}");
            $this->assertEquals(1, $result->getSuccessCount());
        }
    }
    ...
    public function testLanguageLevelTypeAwareWithMultipleVersions(): void
    {
        ...
        $functionName = 'test_function';
        // Create stub function with LanguageLevelTypeAware data
        $stubFunction = $this->createMockFunctionWithVersionAwareType($functionName, null, ['7.4' => 'int', '8.0' => 'bool'], 'mixed');
        $reflectionProvider = $this->createMockReflectionProvider([]);
        $stubsManager = $this->createMockStorageManager();
        $stubsManager->method('getFunctions')->willReturn([$stubFunction]);
        $check = new FunctionReturnTypesCheck($reflectionProvider);
        // Test PHP 7.3: should use default 'mixed'
        $reflectionFunction73 = $this->createMockFunction($functionName, [], null);
        // null = mixed
    ...
    public function testComplexUnionTypeMatching(): void
    {
        ...
        $reflectionFunction = $this->createMockFunction($functionName, [], $this->createType('false|Dom\Element|\Dom\Attr'));
        $stubFunction = $this->createMockFunction($functionName, [], $this->createType('\Dom\Attr|\Dom\Element|false'));
        $reflectionProvider = $this->createMockReflectionProvider([$reflectionFunction]);
        $stubsManager = $this->createMockStorageManager();
        $stubsManager->method('getFunctions')->willReturn([$stubFunction]);
        $check = new FunctionReturnTypesCheck($reflectionProvider);
     ...
     */
    private function createMockFunctionWithVersionAwareType(string $name, $signatureType = null, ?array $languageLevelTypes = null, ?string $defaultType = null): \Masjid_App\Dependencies\StubTests\Framework\Model\PHPFunction
    {
        $function = new \Masjid_App\Dependencies\StubTests\Framework\Model\PHPFunction();
        $function->setId($name);
