...
use Masjid_App\Dependencies\StubTests\Framework\Validator\Functions\FunctionParameterDefaultValueCheck;
...
class FunctionParameterDefaultValueCheckTest extends CheckTestCase
{
    ...
    {
        $fnId = '\implode';
        $reflParams = [$this->makeParam('separator', hasDefaultValue: \true, defaultValue: '', optional: \true)];
        $stubParams = [$this->makeParam('separator', hasDefaultValue: \true, defaultValue: '', optional: \true)];
        $reflFunc = $this->createMockFunction($fnId, $reflParams);
        $stubFunc = $this->createMockFunction($fnId, $stubParams);
        $provider = $this->createMockReflectionProvider([$reflFunc], []);
        $stubs = $this->createMockStorageManager();
    ...
    public function testMismatchedDefaultIsFailure(): void
    {
        ...
        $reflParams = [$this->makeParam('flags', hasDefaultValue: \true, defaultValue: 0, optional: \true)];
        // reflection: 0
        $stubParams = [$this->makeParam('flags', hasDefaultValue: \true, defaultValue: 4, optional: \true)];
        // stub: 4 — wrong
        $reflFunc = $this->createMockFunction($fnId, $reflParams);
        $stubFunc = $this->createMockFunction($fnId, $stubParams);
        $provider = $this->createMockReflectionProvider([$reflFunc], []);
    ...
    {
        $fnId = '\func';
        $reflParams = [$this->makeParam('a', hasDefaultValue: \true, defaultValue: 1, optional: \true), $this->makeParam('b', hasDefaultValue: \true, defaultValue: 'x', optional: \true)];
        $stubParams = [
            $this->makeParam('a', hasDefaultValue: \true, defaultValue: 2, optional: \true),
            // wrong
            $this->makeParam('b', hasDefaultValue: \true, defaultValue: 'y', optional: \true),
        ];
        $reflFunc = $this->createMockFunction($fnId, $reflParams);
        $stubFunc = $this->createMockFunction($fnId, $stubParams);
        $provider = $this->createMockReflectionProvider([$reflFunc], []);
