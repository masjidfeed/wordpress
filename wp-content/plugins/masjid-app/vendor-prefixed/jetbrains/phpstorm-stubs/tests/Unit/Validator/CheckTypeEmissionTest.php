...
class CheckTypeEmissionTest extends TestCase
{
    ...
    public function testEveryReferencedCheckTypeIsEmittedBySomeCheck(): void
    {
        ...
        foreach ($referenced as $name => $value) {
            $this->assertContains($value, $emitted, "CheckType::{$name} ('{$value}') is referenced by a known-problem definition but no check emits that name, " . 'so the suppression would silently never fire. Either point the definition at the name the check ' . 'actually reports, or make a check emit this name.');
        }
    }
    /**
    ...
    private function collectEmittedCheckNames(): array
    {
        ...
        $iterator = new \RecursiveIteratorIterator(new \RecursiveDirectoryIterator($validatorDir, \FilesystemIterator::SKIP_DOTS));
        $names = [];
        ...
        return array_map(static fn(string $case): string => constant(CheckType::class . '::' . $case)->value, array_keys($names));
    }
}
