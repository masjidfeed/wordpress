...
class StubParameterParserTest extends BaseTestCase
{
    ...
    public function testItParsesTheByReferenceFlagPerParameter()
    {
        ...
        self::assertSame(['plain' => \false, 'out' => \true, 'rest' => \false], array_combine(array_map(fn(PHPParameter $p) => $p->getName(), $parameters), array_map(fn(PHPParameter $p) => $p->isPassedByReference(), $parameters)));
    }
    ...
    {
        $parameters = $this->getParametersFromFunction('complete_function.txt');
        self::assertSame(['param1' => 0, 'param2' => 1, 'param3' => 2], array_combine(array_map(fn(PHPParameter $p) => $p->getName(), $parameters), array_map(fn(PHPParameter $p) => $p->getPosition(), $parameters)));
    }
}
