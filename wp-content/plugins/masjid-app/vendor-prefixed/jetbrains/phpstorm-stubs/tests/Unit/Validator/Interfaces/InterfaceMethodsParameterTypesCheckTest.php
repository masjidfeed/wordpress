...
class InterfaceMethodsParameterTypesCheckTest extends CheckTestCase
{
    ...
    public function testMatchingParameterTypeIsSuccess(): void
    {
        ...
        $reflIface = $this->makeInterface($interfaceId, [$this->makeMethod('offsetGet', parameters: [$this->makeParam('offset', $stringType)])]);
        $stubIface = $this->makeInterface($interfaceId, [$this->makeMethod('offsetGet', parameters: [$this->makeParam('offset', $stringType)])]);
        $provider = $this->createMockReflectionProviderWithInterfaces([$reflIface]);
        $stubs = $this->createMockStorageManager();
        $stubs->method('getInterfaces')->willReturn([$stubIface]);
        $result = (new ClassMethodsParameterTypesCheck(reflectionProvider: $provider, entityTypeConfig: EntityTypeConfig::forInterface()))->run($stubs, $interfaceId, '8.0');
    ...
    public function testMismatchedParameterTypeIsFailure(): void
    {
        ...
        $reflIface = $this->makeInterface($interfaceId, [$this->makeMethod('offsetGet', parameters: [$this->makeParam('offset', $this->createType('string'))])]);
        $stubIface = $this->makeInterface($interfaceId, [$this->makeMethod('offsetGet', parameters: [$this->makeParam('offset', $this->createType('int'))])]);
        $provider = $this->createMockReflectionProviderWithInterfaces([$reflIface]);
        $stubs = $this->createMockStorageManager();
        $stubs->method('getInterfaces')->willReturn([$stubIface]);
        $result = (new ClassMethodsParameterTypesCheck(reflectionProvider: $provider, entityTypeConfig: EntityTypeConfig::forInterface()))->run($stubs, $interfaceId, '8.0');
    ...
    public function testParameterTypeFromParentInterfaceIsCounted(): void
    {
        ...
        $stringType = $this->createType('string');
        $reflIface = $this->makeInterface($childId, [$this->makeMethod('offsetGet', parameters: [$this->makeParam('offset', $stringType)])]);
        $parentStub = $this->makeInterface('\ParentInterface', [$this->makeMethod('offsetGet', parameters: [$this->makeParam('offset', $stringType)])]);
    ...
    public function testInterfaceLevelKnownProblemSkipsAllMethods(): void
    {
        ...
        $reflIface = $this->makeInterface($interfaceId, [$this->makeMethod('doWork', parameters: [$this->makeParam('value', $this->createType('string'))])]);
        $stubIface = $this->makeInterface($interfaceId, [$this->makeMethod('doWork', parameters: [$this->makeParam('value', $this->createType('int'))])]);
        ...
        $knownProblemsProvider->method('getProblems')->willReturn([new ProblemDefinition(entityType: EntityType::INTERFACE_TYPE, entityId: $interfaceId, type: ProblemType::INTERNAL_IMPLEMENTATION, affectedChecks: [CheckType::PARAMETER_TYPES], versionRange: new PhpVersionRange(PhpVersions::PHP_7_0, PhpVersions::LATEST), reason: 'Interface-level skip')]);
        KnownProblemsRegistry::reset();
