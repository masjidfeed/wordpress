...
class ClassFinalMethodsCheckTest extends CheckTestCase
{
    ...
    {
        $className = '\MyClass';
        $reflClass = $this->createMockClassWithProperties($className, null, null, null, [$this->makeMethod('doWork', isFinal: \true)]);
        $stubClass = $this->createMockClassWithProperties($className, null, null, null, [$this->makeMethod('doWork', isFinal: \true)]);
        $provider = $this->createMockReflectionProvider([], [$reflClass]);
        $stubs = $this->createMockStorageManager();
        $stubs->method('getClasses')->willReturn([$stubClass]);
        $result = (new ClassFinalMethodsCheck($provider))->run($stubs, $className, '8.0');
    ...
    public function testMultipleMethodsMixedResultsAreAllReported(): void
    {
        ...
        $reflClass = $this->createMockClassWithProperties($className, null, null, null, [
            ...
            // matches
            $this->makeMethod('badMethod', isFinal: \true),
        ]);
        $stubClass = $this->createMockClassWithProperties($className, null, null, null, [$this->makeMethod('okMethod'), $this->makeMethod('badMethod')]);
        $provider = $this->createMockReflectionProvider([], [$reflClass]);
        $stubs = $this->createMockStorageManager();
        $stubs->method('getClasses')->willReturn([$stubClass]);
        $result = (new ClassFinalMethodsCheck($provider))->run($stubs, $className, '8.0');
    ...
    public function testMethodMissingInStubsIsNotReportedAsFinalMismatch(): void
    {
        ...
        $className = '\MyClass';
        $reflClass = $this->createMockClassWithProperties($className, null, null, null, [$this->makeMethod('onlyInReflection', isFinal: \true)]);
        $stubClass = $this->createMockClassWithProperties($className);
        // no methods
        $provider = $this->createMockReflectionProvider([], [$reflClass]);
        $stubs = $this->createMockStorageManager();
    ...
    public function testClassLevelKnownProblemSkipsAllMethods(): void
    {
        ...
        $knownProblemsProvider->method('getProblems')->willReturn([new ProblemDefinition(entityType: EntityType::CLASS_TYPE, entityId: $className, type: ProblemType::INTERNAL_IMPLEMENTATION, affectedChecks: [CheckType::CLASS_FINAL_METHODS], versionRange: new PhpVersionRange(PhpVersions::EARLIEST, PhpVersions::LATEST), reason: 'Class-level skip')]);
        KnownProblemsRegistry::reset();
    ...
    public function testMethodLevelKnownProblemSkipsSpecificMismatch(): void
    {
        ...
        $reflClass = $this->createMockClassWithProperties($className, null, null, null, [
            $this->makeMethod('okMethod'),
            // matches
            $this->makeMethod('doWork', isFinal: \true),
        ]);
        $stubClass = $this->createMockClassWithProperties($className, null, null, null, [$this->makeMethod('okMethod'), $this->makeMethod('doWork', isFinal: \false)]);
        $knownProblemsProvider = $this->createStub(\Masjid_App\Dependencies\StubTests\Framework\Validator\KnownProblems\KnownProblemsProvider::class);
        $knownProblemsProvider->method('getProblems')->willReturn([new ProblemDefinition(entityType: EntityType::METHOD, entityId: $mismatchedId, type: ProblemType::INTERNAL_IMPLEMENTATION, affectedChecks: [CheckType::CLASS_FINAL_METHODS], versionRange: new PhpVersionRange(PhpVersions::EARLIEST, PhpVersions::LATEST), reason: 'Method-level skip')]);
        KnownProblemsRegistry::reset();
        $registry = KnownProblemsRegistry::getInstance($knownProblemsProvider);
        $provider = $this->createMockReflectionProvider([], [$reflClass]);
        $stubs = $this->createMockStorageManager();
    ...
    public function testStubMethodOutsideVersionRangeIsSkipped(): void
    {
        ...
        $className = '\MyClass';
        $reflClass = $this->createMockClassWithProperties($className, null, null, null, [$this->makeMethod('newMethod', isFinal: \true)]);
    ...
    public function testPsUnreservePrefixMethodFinalMismatchIsReported(): void
    {
        ...
        $className = '\Generator';
        $reflClass = $this->createMockClassWithProperties($className, null, null, null, [$this->makeMethod('throw', isFinal: \true)]);
    ...
    public function testPsUnreservePrefixMethodFinalMatchIsSuccess(): void
    {
        $className = '\Generator';
        $reflClass = $this->createMockClassWithProperties($className, null, null, null, [$this->makeMethod('throw', isFinal: \true)]);
        $stubClass = $this->createMockClassWithProperties($className, null, null, null, [$this->makeMethod('PS_UNRESERVE_PREFIX_throw', isFinal: \true)]);
        $provider = $this->createMockReflectionProvider([], [$reflClass]);
        $stubs = $this->createMockStorageManager();
        $stubs->method('getClasses')->willReturn([$stubClass]);
        $result = (new ClassFinalMethodsCheck($provider))->run($stubs, $className, '8.0');
    ...
    public function testFinalMethodInheritedFromParentClassMismatchIsReported(): void
    {
        ...
        $parentStub->setMethods([$this->makeMethod('doWork')]);
        // stub: non-final → mismatch
    ...
    public function testFinalMethodInheritedFromParentClassMatchIsSuccess(): void
    {
        ...
        $parentStub = new PHPClass();
        $parentStub->setId($parentClassName);
        $parentStub->setMethods([$this->makeMethod('doWork', isFinal: \true)]);
        // matches
        $childStub = $this->createMockClassWithProperties($className);
        $childStub->setParentClass($parentStub);
        $provider = $this->createMockReflectionProvider([], [$reflClass]);
    ...
    public function testFinalMethodFromInterfaceMismatchIsReported(): void
    {
        ...
        $className = '\MyClass';
        $reflClass = $this->createMockClassWithProperties($className, null, null, null, [$this->makeMethod('interfaceMethod', isFinal: \true)]);
        $iface = $this->makeInterface('\MyInterface', [$this->makeMethod('interfaceMethod')]);
    ...
    public function testChildMethodOverridesParentForFinalCheck(): void
    {
        ...
        $parentStub->setId($parentClassName);
        $parentStub->setMethods([$this->makeMethod('doWork', isFinal: \true)]);
        // parent: final (should be overridden)
        $childStub = $this->createMockClassWithProperties($className, null, null, null, [$this->makeMethod('doWork')]);
        $childStub->setParentClass($parentStub);
        $provider = $this->createMockReflectionProvider([], [$reflClass]);
        $stubs = $this->createMockStorageManager();
        $stubs->method('getClasses')->willReturn([$childStub]);
