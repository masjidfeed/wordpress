...
class CheckVersionGatingConsistencyTest extends TestCase
{
    ...
    {
        self::assertNotEmpty(iterator_to_array($this->descriptorProvider()), 'Discovered no CheckDescriptors — the discovery logic is stale and these tests would pass vacuously.');
    }
    /**
    ...
    {
        $check = $descriptor->entityTypeConfig !== null ? new $descriptor->checkClass(entityTypeConfig: $descriptor->entityTypeConfig) : new $descriptor->checkClass();
        $unsupported = [];
        ...
        self::assertSame([], $unsupported, sprintf("%s registers %s for %s..%s, but %s::supports() rejects: %s.\n" . 'Those versions would fail in the validator suites. Narrow the descriptor range or widen supports().', $testClass, $key, $descriptor->fromVersion->value, $descriptor->toVersion->value, $descriptor->checkClass, implode(', ', $unsupported)));
    }
    /**
    ...
    {
        self::assertNotEmpty(self::versionsInRange($descriptor->fromVersion, $descriptor->toVersion), sprintf('%s registers %s for %s..%s, which matches no PhpVersions case, so the check never runs.', $testClass, $key, $descriptor->fromVersion->value, $descriptor->toVersion->value));
    }
    /**
     * @return string[]
     */
    private static function versionsInRange(PhpVersions $from, PhpVersions $to): array
    {
        ...
        foreach (PhpVersions::cases() as $case) {
            if (version_compare($case->value, $from->value, '>=') && version_compare($case->value, $to->value, '<=')) {
                $versions[] = $case->value;
            }
        }
        return $versions;
