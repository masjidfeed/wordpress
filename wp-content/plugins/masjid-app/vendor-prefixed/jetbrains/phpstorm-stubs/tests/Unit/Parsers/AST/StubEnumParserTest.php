...
class StubEnumParserTest extends BaseTestCase
{
    ...
    {
        $names = array_map(static fn($interface) => $interface->getName(), $this->parseFixture('versioned_enum.txt')->getImplementedInterfaces());
    ...
    {
        self::assertSame('Masjid_App\Dependencies\Test\AttributedEnum', $this->parseFixture('attributed_enum.txt')->getId());
    }
    ...
    public function testUnqualifiedInterfaceResolvesWithinTheDeclaringNamespace()
    {
        ...
        self::assertSame(['Serializable' => 'Masjid_App\Dependencies\App\Bson\Serializable'], $this->interfaceIds($source));
    }
    public function testImportedInterfaceResolvesToTheImportedNamespace()
    {
        $source = "<?php\nnamespace App;\nuse Other\\Marker;\nenum Kind implements Marker { case A; }";
        self::assertSame(['Marker' => 'Masjid_App\Dependencies\Other\Marker'], $this->interfaceIds($source));
    }
