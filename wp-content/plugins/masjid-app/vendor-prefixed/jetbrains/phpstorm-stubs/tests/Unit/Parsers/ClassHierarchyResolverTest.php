...
namespace Masjid_App\Dependencies\StubTests\Unit\Parsers;

use Masjid_App\Dependencies\PHPUnit\Framework\TestCase;
use Masjid_App\Dependencies\StubTests\Framework\Validator\Services\ClassAncestorNamesExtractor;
...
class ClassHierarchyResolverTest extends TestCase
{
    ...
    public function testSameNamespaceShortNameIsLinkedViaNsFallback(): void
    {
        ...
        $randomError = $this->makeClass('Masjid_App\Dependencies\Random\RandomError');
        $broken = $this->makeClass('Masjid_App\Dependencies\Random\BrokenRandomEngineError', 'RandomError');
    ...
    public function testShortNameCollisionResolvesToCorrectFqnClass(): void
    {
        ...
        $swooleException = $this->makeClass('Masjid_App\Dependencies\Swoole\Coroutine\Http2\Client\Exception', 'Exception');
        $errorException = $this->makeClass('\ErrorException', 'Exception');
        ...
        $this->assertSame($rootException, $errorException->getParentClass(), 'ErrorException.parentClass must link to \Exception, not a same-short-name class in another namespace');
    ...
    public function testNamespacedAncestorChainAfterResolution(): void
    {
        ...
        $this->resolver->resolve([$randomError, $broken]);
        $extractor = new ClassAncestorNamesExtractor();
        $this->assertEquals(['Masjid_App\Dependencies\Random\RandomError'], $extractor->extract($broken));
    }
    ...
    public function testInterfaceParentInSameNamespaceIsLinkedViaNsFallback(): void
    {
        ...
        $baseEngine = $this->makeInterface('Masjid_App\Dependencies\Random\BaseEngine');
        $cryptoEngine = $this->makeInterface('Masjid_App\Dependencies\Random\CryptoSafeEngine', 'BaseEngine');
