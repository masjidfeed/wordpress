...
class ClassAttributeTargetsCheckTest extends CheckTestCase
{
    ...
    private function runCheck(array $reflAttributes, array $stubAttributes, string $id = 'Override'): \Masjid_App\Dependencies\StubTests\Framework\Validator\Contracts\CheckResultSet
    {
    ...
    public function testOtherAttributesAreIgnored(): void
    {
        ...
        $reflAttrs = [['name' => 'SomeMarker', 'arguments' => []], ['name' => 'Attribute', 'arguments' => [0 => $flags]]];
     ...
     * running this suite.
     */
    private function attributeClassFor(string $version): \Masjid_App\Dependencies\StubTests\Framework\Model\PHPClass
    {
        $constants = [$this->makeClassConstant('TARGET_CLASS', 1), $this->makeClassConstant('TARGET_FUNCTION', 2), $this->makeClassConstant('TARGET_METHOD', 4), $this->makeClassConstant('TARGET_PROPERTY', 8), $this->makeClassConstant('TARGET_CLASS_CONSTANT', 16), $this->makeClassConstant('TARGET_PARAMETER', 32)];
    ...
    private function runCheckWithVersionMap(array $reflAttributes, array $stubAttributes, string $version, string $id = 'Override'): \Masjid_App\Dependencies\StubTests\Framework\Validator\Contracts\CheckResultSet
    {
        $reflClass = $this->makeClass($id);
        $reflClass->setAttributes($reflAttributes);
        $stubClass = $this->makeClass($id);
    ...
    {
        $result = $this->runCheckWithVersionMap($this->attribute(64), $this->symbolicAttribute('Attribute::TARGET_CONSTANT'), '8.6');
    ...
    {
        $result = $this->runCheckWithVersionMap($this->attribute(1 | 64), $this->symbolicAttribute('1|Attribute::TARGET_CONSTANT'), '8.6');
        $this->assertFalse($result->hasFailures(), implode('; ', $result->getFailures()));
    }
    public function testFullySymbolicBitwiseOrIsResolved(): void
    {
        $result = $this->runCheckWithVersionMap($this->attribute(4 | 8 | 64), $this->symbolicAttribute('Attribute::TARGET_METHOD|Attribute::TARGET_PROPERTY|Attribute::TARGET_CONSTANT'), '8.6');
        $this->assertFalse($result->hasFailures(), implode('; ', $result->getFailures()));
    }
    /**
    ...
    {
        $result = $this->runCheckWithVersionMap($this->attribute(1 | 64), $this->symbolicAttribute('Attribute::TARGET_CLASS'), '8.6');
        $this->assertTrue($result->hasFailures());
    ...
    {
        $result = $this->runCheckWithVersionMap($this->attribute(4), $this->symbolicAttribute('Attribute::TARGET_NOT_A_REAL_CONSTANT'), '8.6');
        $this->assertTrue($result->hasFailures());
