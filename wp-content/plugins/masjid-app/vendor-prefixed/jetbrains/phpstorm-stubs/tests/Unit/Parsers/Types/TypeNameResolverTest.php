...
namespace Masjid_App\Dependencies\StubTests\Unit\Parsers\Types;
...
use Masjid_App\Dependencies\StubTests\Framework\Parsers\Types\TypeNameResolver;
/**
...
class TypeNameResolverTest extends TestCase
{
    ...
    public function testBuiltInTypesAreReturnedUnchanged(string $name)
    {
        ...
        self::assertSame($name, $this->resolver->resolve($name, ['int' => 'Masjid_App\Dependencies\Some\Int_'], '\Dom'));
    }
    public static function builtInTypes(): array
    {
        return array_map(static fn(string $t): array => [$t], ['int', 'string', 'bool', 'float', 'array', 'object', 'mixed', 'void', 'never', 'null', 'false', 'true', 'callable', 'iterable', 'resource', 'self', 'parent', 'static']);
    }
    /**
    ...
    {
        self::assertSame('Masjid_App\Dependencies\Dom\Node', $this->resolver->resolve('Masjid_App\Dependencies\Dom\Node', [], '\Other'));
        self::assertSame('\Exception', $this->resolver->resolve('\Exception', [], '\Other'));
    }
    public function testImportAliasWins()
    {
        $imports = ['Exception' => 'Masjid_App\Dependencies\FFI\Exception'];
        self::assertSame('Masjid_App\Dependencies\FFI\Exception', $this->resolver->resolve('Exception', $imports, '\Dom'));
    }
    ...
    {
        $imports = ['Result' => 'Masjid_App\Dependencies\LDAP\Result'];
        self::assertSame('Masjid_App\Dependencies\LDAP\Result', $this->resolver->resolve('Result', $imports, '\Dom'));
    }
    public function testImportTakesPrecedenceOverTheCurrentNamespace()
    {
        $imports = ['Attr' => 'Masjid_App\Dependencies\Dom\Attr'];
        self::assertSame('Masjid_App\Dependencies\Dom\Attr', $this->resolver->resolve('Attr', $imports, '\SomewhereElse'));
    }
    /**
    ...
    {
        self::assertSame('Masjid_App\Dependencies\LDAP\Result', $this->resolver->resolve('Masjid_App\Dependencies\LDAP\Result', [], '\Dom'));
    }
    public function testUnqualifiedNameResolvesRelativeToTheCurrentNamespace()
    {
        self::assertSame('Masjid_App\Dependencies\Dom\Node', $this->resolver->resolve('Node', [], '\Dom'));
    }
    ...
    {
        self::assertSame('Masjid_App\Dependencies\Dom\Sub\Node', $this->resolver->resolve('Node', [], 'Masjid_App\Dependencies\Dom\Sub'));
    }
    /**
    ...
    public function testImportLookupIsCaseSensitive()
    {
        $imports = ['Result' => 'Masjid_App\Dependencies\LDAP\Result'];
        self::assertSame('Masjid_App\Dependencies\LDAP\Result', $this->resolver->resolve('Result', $imports, '\Dom'));
        self::assertSame('Masjid_App\Dependencies\Dom\result', $this->resolver->resolve('result', $imports, '\Dom'));
    }
}
