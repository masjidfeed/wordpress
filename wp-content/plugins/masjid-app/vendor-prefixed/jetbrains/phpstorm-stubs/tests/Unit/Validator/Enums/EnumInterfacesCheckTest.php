...
class EnumInterfacesCheckTest extends CheckTestCase
{
    ...
    public function testMissingInterfaceInStubsDoesNotFail(): void
    {
        ...
        $stubEnum = $this->makeEnum($enumId, interfaces: [$backedEnum]);
        // UnitEnum omitted (inherited transitively)
        $provider = $this->createMockReflectionProviderWithEnums([$reflEnum]);
        $stubs = $this->createMockStorageManager();
        $stubs->method('getEnums')->willReturn([$stubEnum]);
        $result = (new ClassInterfacesCheck(reflectionProvider: $provider, entityTypeConfig: EntityTypeConfig::forEnum()))->run($stubs, $enumId, '8.1');
    ...
    public function testEnumLevelKnownProblemSkipsInterfaceCheck(): void
    {
        ...
        $knownProblemsProvider->method('getProblems')->willReturn([new ProblemDefinition(entityType: EntityType::ENUM_TYPE, entityId: $enumId, type: ProblemType::INTERNAL_IMPLEMENTATION, affectedChecks: [CheckType::CLASS_INTERFACES], versionRange: new PhpVersionRange(PhpVersions::PHP_8_1, PhpVersions::LATEST), reason: 'Enum-level interface skip')]);
        KnownProblemsRegistry::reset();
