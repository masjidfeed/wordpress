...
namespace Masjid_App\Dependencies\StubTests\Unit\Validator\PhpDoc;
...
use Masjid_App\Dependencies\StubTests\Framework\Validator\PhpDoc\PhpDocTagsCheck;
class PhpDocTagsCheckTest extends PhpDocCheckTestCase
{
    ...
    private function runCheck(\Masjid_App\Dependencies\StubTests\Framework\Storage\StubDataQueryInterface $stubs, string $entityId): \Masjid_App\Dependencies\StubTests\Framework\Validator\Contracts\CheckResultSet
    {
    ...
    {
        return ['param' => ['param'], 'return' => ['return'], 'throws' => ['throws'], 'since' => ['since'], 'deprecated' => ['deprecated'], 'var' => ['var'], 'see' => ['see'], 'link' => ['link'], 'internal' => ['internal'], 'template' => ['template'], 'template-covariant' => ['template-covariant'], 'extends' => ['extends'], 'implements' => ['implements'], 'immutable' => ['immutable'], 'readonly' => ['readonly'], 'removed' => ['removed'], 'xglobal' => ['xglobal'], 'inheritdoc' => ['inheritdoc'], 'no-named-arguments' => ['no-named-arguments']];
    }
    ...
    {
        return ['phpstan-type' => ['phpstan-type'], 'phpstan-param' => ['phpstan-param'], 'phpstan-return' => ['phpstan-return'], 'phpstan-assert' => ['phpstan-assert'], 'phpstan-assert-if-true' => ['phpstan-assert-if-true'], 'phpstan-import-type' => ['phpstan-import-type'], 'phpstan-ignore' => ['phpstan-ignore'], 'phpstan-impure' => ['phpstan-impure'], 'psalm-pure' => ['psalm-pure'], 'psalm-assert' => ['psalm-assert'], 'psalm-immutable' => ['psalm-immutable'], 'psalm-param' => ['psalm-param'], 'psalm-return' => ['psalm-return'], 'psalm-var' => ['psalm-var'], 'phan-type' => ['phan-type'], 'unknown-tag' => ['unknown-tag']];
    }
    ...
    {
        KnownProblemsRegistry::reset();
        $provider = $this->createStub(\Masjid_App\Dependencies\StubTests\Framework\Validator\KnownProblems\KnownProblemsProvider::class);
        $provider->method('getProblems')->willReturn([new ProblemDefinition(entityType: EntityType::CLASS_TYPE, entityId: '\TestClass', type: ProblemType::INTERNAL_IMPLEMENTATION, affectedChecks: [CheckType::PHPDOC_TAGS], versionRange: new PhpVersionRange(PhpVersions::EARLIEST, PhpVersions::LATEST), reason: 'Uses tool-specific tags for internal implementation reasons')]);
        KnownProblemsRegistry::getInstance($provider);
        $check = new PhpDocTagsCheck();
        // must be created AFTER the custom registry is in place
        $phpDoc = "/**\n * @phpstan-type Foo = int\n */";
        $class = $this->makeClass('\TestClass', phpDoc: $phpDoc);
