...
class ReflectionImplementedInterfaceParserTest extends TestCase
{
    ...
    public function testNamespacedInterfaceStoresFullNameNotShortName(): void
    {
        ...
        $ref = new AdaptedReflectionClassReference('Masjid_App\Dependencies\Random\Engine');
        $result = $this->parser->parse($ref);
        $this->assertEquals('Masjid_App\Dependencies\Random\Engine', $result->getName());
    ...
    public function testNamespacedInterfaceIdIsLeadingBackslashPlusFullName(): void
    {
        $ref = new AdaptedReflectionClassReference('Masjid_App\Dependencies\Random\Engine');
        $result = $this->parser->parse($ref);
        $this->assertEquals('Masjid_App\Dependencies\Random\Engine', $result->getId());
    }
    ...
    public function testCryptoSafeEngineStoresFullNameNotShortName(): void
    {
        ...
        $ref = new AdaptedReflectionClassReference('Masjid_App\Dependencies\Random\CryptoSafeEngine');
        $result = $this->parser->parse($ref);
        $this->assertEquals('Masjid_App\Dependencies\Random\CryptoSafeEngine', $result->getName());
        $this->assertEquals('Masjid_App\Dependencies\Random\CryptoSafeEngine', $result->getId());
    }
    // ── Deep namespace ────────────────────────────────────────────────────────
    public function testDeepNamespacedInterface(): void
    {
        $ref = new AdaptedReflectionClassReference('Masjid_App\Dependencies\Foo\Bar\Baz\MyInterface');
        $result = $this->parser->parse($ref);
        $this->assertEquals('Masjid_App\Dependencies\Foo\Bar\Baz\MyInterface', $result->getName());
        $this->assertEquals('Masjid_App\Dependencies\Foo\Bar\Baz\MyInterface', $result->getId());
        $this->assertEquals('Masjid_App\Dependencies\Foo\Bar\Baz', $result->getNamespace());
    }
    // ── ID always starts with \ ───────────────────────────────────────────────
    public function testIdAlwaysStartsWithBackslash(): void
    {
        $cases = ['Throwable', 'Iterator', 'Masjid_App\Dependencies\Random\Engine', 'Masjid_App\Dependencies\Foo\Bar\MyInterface'];
        foreach ($cases as $name) {
            $ref = new AdaptedReflectionClassReference($name);
            $result = $this->parser->parse($ref);
            $this->assertStringStartsWith('\\', $result->getId(), "getId() for '{$name}' must start with '\\'");
        }
    }
    // ── getName() == getId() without leading \ ────────────────────────────────
    public function testNameEqualsIdWithoutLeadingBackslash(): void
    {
        $cases = ['Throwable', 'Masjid_App\Dependencies\Random\Engine', 'Masjid_App\Dependencies\Foo\Bar\Iface'];
        foreach ($cases as $name) {
            $ref = new AdaptedReflectionClassReference($name);
            $result = $this->parser->parse($ref);
            $this->assertEquals(ltrim($result->getId(), '\\'), $result->getName(), "getName() must equal getId() without leading \\ for '{$name}'");
        }
    }
}
