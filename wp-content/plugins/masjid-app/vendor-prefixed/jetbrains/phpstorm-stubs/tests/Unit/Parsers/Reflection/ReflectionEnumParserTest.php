...
use Masjid_App\Dependencies\StubTests\Framework\Parsers\Reflection\ReflectionEnumParser;
...
class ReflectionEnumParserTest extends TestCase
{
    ...
    {
        $reflectionClassMock = $this->getMockBuilder(AdaptedReflectionClass::class)->disableOriginalConstructor()->onlyMethods(['getMethods'])->getMock();
    ...
    {
        $reflectionClassMock = $this->getMockBuilder(AdaptedReflectionClass::class)->disableOriginalConstructor()->onlyMethods(['hasReflectionConstants', 'getReflectionConstants'])->getMock();
    ...
    {
        $case = $this->makeConstantMock('Ascending', null, \true);
    ...
    {
        $cases = [$this->makeConstantMock('First', null, \true), $this->makeConstantMock('Second', null, \true), $this->makeConstantMock('Third', null, \true)];
    ...
    {
        $enum = (new ReflectionEnumParser())->parse($this->makeEnumMockWithConstants([$this->makeConstantMock('FOO', 'BAR', \false)]));
        self::assertSame([], $enum->getCaseNames());
        self::assertCount(1, $enum->getConstants());
    }
    public function testCasesDefaultToEmptyArrayWhenConstantsCannotBeRead()
    {
        $enum = (new ReflectionEnumParser())->parse($this->getMockBuilder(AdaptedReflectionClass::class)->disableOriginalConstructor()->getMock());
        self::assertSame([], $enum->getCaseNames());
    }
    private function makeConstantMock(string $name, $value, bool $isEnumCase)
    {
        $mock = $this->getMockBuilder(AdaptedReflectionClassConstant::class)->disableOriginalConstructor()->onlyMethods(['getName', 'getValue', 'isEnumCase'])->getMock();
    ...
    {
        $mock = $this->getMockBuilder(AdaptedReflectionClass::class)->disableOriginalConstructor()->onlyMethods(['hasReflectionConstants', 'getReflectionConstants'])->getMock();
        $mock->method('hasReflectionConstants')->willReturn(\true);
