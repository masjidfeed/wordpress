...
use Masjid_App\Dependencies\StubTests\Framework\Validator\Functions\FunctionParametersCountCheck;
...
class FunctionParametersCountCheckTest extends CheckTestCase
{
    ...
    {
        $id = '\my_func';
        $provider = $this->createMockReflectionProvider([$this->makeFunction($id, [$this->makeParam('a'), $this->makeParam('b')])]);
        $stubs = $this->createMockStorageManager();
        $stubs->method('getFunctions')->willReturn([$this->makeFunction($id, [$this->makeParam('a')])]);
        $result = (new FunctionParametersCountCheck($provider))->run($stubs, $id, '8.0');
        $this->assertTrue($result->hasFailures());
    ...
    public function testParamNotYetAddedIsExcludedFromCount(): void
    {
        ...
        $id = '\my_func';
        $provider = $this->createMockReflectionProvider([$this->makeFunction($id, [$this->makeParam('a')])]);
        $stubs = $this->createMockStorageManager();
        $stubs->method('getFunctions')->willReturn([$this->makeFunction($id, [$this->makeParam('a'), $this->makeParam('b', sinceVersion: '8.1')])]);
        $result = (new FunctionParametersCountCheck($provider))->run($stubs, $id, '8.0');
        $this->assertFalse($result->hasFailures());
    }
    public function testParamAddedAtExactSinceVersionIsIncluded(): void
    {
        ...
        $provider = $this->createMockReflectionProvider([$this->makeFunction($id, [$this->makeParam('a'), $this->makeParam('b')])]);
        $stubs = $this->createMockStorageManager();
        $stubs->method('getFunctions')->willReturn([$this->makeFunction($id, [$this->makeParam('a'), $this->makeParam('b', sinceVersion: '8.0')])]);
        $result = (new FunctionParametersCountCheck($provider))->run($stubs, $id, '8.0');
        $this->assertFalse($result->hasFailures());
    }
    public function testParamAtExactRemovedVersionIsStillIncluded(): void
    {
        ...
        $provider = $this->createMockReflectionProvider([$this->makeFunction($id, [$this->makeParam('a'), $this->makeParam('b')])]);
        $stubs = $this->createMockStorageManager();
        $stubs->method('getFunctions')->willReturn([$this->makeFunction($id, [$this->makeParam('a'), $this->makeParam('b', removedVersion: '7.2')])]);
    ...
    public function testParamAfterRemovedVersionIsExcluded(): void
    {
        ...
        $provider = $this->createMockReflectionProvider([$this->makeFunction($id, [$this->makeParam('a')])]);
        $stubs = $this->createMockStorageManager();
        $stubs->method('getFunctions')->willReturn([$this->makeFunction($id, [$this->makeParam('a'), $this->makeParam('b', removedVersion: '7.1')])]);
    ...
    public function testPlaceholderAndVariadicWithSameNameCountAsOne(): void
    {
        ...
        $id = '\my_func';
        $provider = $this->createMockReflectionProvider([$this->makeFunction($id, [$this->makeParam('x'), $this->makeParam('vals')])]);
        $stubs = $this->createMockStorageManager();
        $stubs->method('getFunctions')->willReturn([$this->makeFunction($id, [
            $this->makeParam('x'),
    ...
    public function testVersionWindowParamCountedOnlyWithinRange(): void
    {
        ...
        $id = '\range_func';
        $stubParam = $this->makeParam('b', sinceVersion: '7.0', removedVersion: '7.2');
        // removedVersion='7.2': excluded from 7.2+
        foreach (['7.0', '7.1', '7.2'] as $version) {
            $expected = $version === '7.2' ? 1 : 2;
            $provider = $this->createMockReflectionProvider([$this->makeFunction($id, array_fill(0, $expected, $this->makeParam('_')))]);
            $stubs = $this->createMockStorageManager();
            $stubs->method('getFunctions')->willReturn([$this->makeFunction($id, [$this->makeParam('a'), $stubParam])]);
    ...
    public function testKnownProblemSkipsValidation(): void
    {
        ...
        $knownProblemsProvider->method('getProblems')->willReturn([new ProblemDefinition(entityType: EntityType::FUNCTION, entityId: $id, type: ProblemType::INTERNAL_IMPLEMENTATION, affectedChecks: [CheckType::PARAMETERS_COUNT], versionRange: new PhpVersionRange(PhpVersions::EARLIEST, PhpVersions::LATEST), reason: 'Test skip reason')]);
        KnownProblemsRegistry::reset();
