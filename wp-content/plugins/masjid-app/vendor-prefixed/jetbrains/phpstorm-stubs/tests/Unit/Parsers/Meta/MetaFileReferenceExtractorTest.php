...
namespace Masjid_App\Dependencies\StubTests\Unit\Parsers\Meta;
...
use Masjid_App\Dependencies\StubTests\Framework\MetaFile\MetaReferenceRole;
...
registerArgumentsSet('FFICTypeKind', \FFI\CType::TYPE_VOID, \FFI\CType::TYPE_FLOAT);
PHP;
        $refs = $this->extractFromCode($code);
        $this->assertContainsRef(MetaReferenceType::CLASS_CONST, 'Masjid_App\Dependencies\FFI\CType::TYPE_VOID', $refs);
        $this->assertContainsRef(MetaReferenceType::CLASS_CONST, 'Masjid_App\Dependencies\FFI\CType::TYPE_FLOAT', $refs);
...
registerArgumentsSet('test', \ReflectionClass::IS_FINAL | \ReflectionClass::IS_EXPLICIT_ABSTRACT);
PHP;
        $refs = $this->extractFromCode($code);
        $this->assertContainsRef(MetaReferenceType::CLASS_CONST, '\ReflectionClass::IS_FINAL', $refs);
...
override(\Masjid_App\Dependencies\PHPUnit\Framework\TestCase::createStub(0), map(["" => "\$0"]));
override(\Masjid_App\Dependencies\Mockery::mock(0), map(["" => "@"]));
expectedArguments(\Masjid_App\Dependencies\GuzzleHttp\Client::request(), 0, 'GET');
expectedArguments(\Masjid_App\Dependencies\Psr\Log\LoggerInterface::log(0), 0, 'info');
expectedArguments(\Masjid_App\Dependencies\Illuminate\Support\Facades\Http::send(), 0, 'GET');
PHP;
        $refs = $this->extractFromCode($code);
...
override(\Masjid_App\Dependencies\mock(0), map(["" => "@"]));
override(\Masjid_App\Dependencies\spy(0), map(["" => "@"]));
override(\Masjid_App\Dependencies\expect(0), type(0));
exitPoint(\Masjid_App\Dependencies\jexit());
exitPoint(\Masjid_App\Dependencies\wp_die());
exitPoint(\Masjid_App\Dependencies\dd());
PHP;
        $refs = $this->extractFromCode($code);
...
expectedArguments(\Masjid_App\Dependencies\test_func(), 0, \TEST_CONST);
expectedArguments(\Masjid_App\Dependencies\test_func(), 1, \TEST_CONST);
PHP;
        $file = $this->writeTempFile($code);
