...
use Masjid_App\Dependencies\StubTests\Framework\Validator\AbstractMemberFlagCheck;
use Masjid_App\Dependencies\StubTests\Framework\Validator\Contracts\DescribesMethodMismatch;
use Masjid_App\Dependencies\StubTests\Framework\Validator\Contracts\DescribesPropertyMismatch;
use Masjid_App\Dependencies\StubTests\Framework\Validator\Contracts\MemberKind;
...
{
    private function invokeDescribe(AbstractMemberFlagCheck $check, mixed $reflectionMember, mixed $stubMember): ?string
    {
    ...
    public function testMethodDescriberIsReachedWithTypedArguments()
    {
        ...
        self::assertSame('method describer saw \Foo::bar at 8.6', $this->invokeDescribe($check, new PHPMethod(), new PHPMethod()));
    }
    public function testPropertyDescriberIsReachedWithTypedArguments()
    {
        $check = new class extends AbstractMemberFlagCheck implements DescribesPropertyMismatch
        {
            ...
            public function describePropertyMismatch(string $propertyEntityId, PHPProperty $reflProperty, PHPProperty $stubProperty, string $phpVersion): ?string
            {
                return "property describer saw {$propertyEntityId}";
            }
        };
        self::assertSame('property describer saw \Foo::bar', $this->invokeDescribe($check, new PHPProperty(), new PHPProperty()));
    }
    ...
    public function testKindAndDescriberDisagreeingFailsLoudly()
    {
        $check = new class extends AbstractMemberFlagCheck implements DescribesMethodMismatch
        {
            ...
            {
                return MemberKind::PROPERTY;
                // deliberately inconsistent with the describer
            }
            public function describeMethodMismatch(string $methodEntityId, mixed $reflMethod, PHPMethod $stubMethod, string $phpVersion): ?string
            {
