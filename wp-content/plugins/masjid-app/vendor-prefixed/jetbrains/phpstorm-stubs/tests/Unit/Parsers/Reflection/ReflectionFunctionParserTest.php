...
namespace Masjid_App\Dependencies\StubTests\Unit\Parsers\Reflection;

use Masjid_App\Dependencies\PHPUnit\Framework\TestCase;
use Masjid_App\Dependencies\StubTests\Framework\Model\PHPParameter;
use Masjid_App\Dependencies\StubTests\Framework\Parsers\Reflection\ReflectionFunctionParser;
use Masjid_App\Dependencies\StubTests\Framework\Parsers\Reflection\Wrappers\AdaptedReflectionFunction;
use Masjid_App\Dependencies\StubTests\Framework\Parsers\Reflection\Wrappers\AdaptedReflectionParameter;
use Masjid_App\Dependencies\StubTests\Framework\Parsers\Reflection\Wrappers\AdaptedReflectionType;
class ReflectionFunctionParserTest extends TestCase
{
    ...
    {
        $functionMock = $this->getMockBuilder(AdaptedReflectionFunction::class)->disableOriginalConstructor()->onlyMethods([])->getMock();
    ...
    {
        $functionMock = $this->getMockBuilder(AdaptedReflectionFunction::class)->disableOriginalConstructor()->onlyMethods(['getShortName'])->getMock();
    ...
    {
        $functionMock = $this->getMockBuilder(AdaptedReflectionFunction::class)->disableOriginalConstructor()->onlyMethods(['getNamespaceName'])->getMock();
        $functionMock->method('getNamespaceName')->willReturn('Masjid_App\Dependencies\MyNameSpace\SubNameSpace');
        $basePHPElement = (new ReflectionFunctionParser())->parse($functionMock);
        self::assertEquals('Masjid_App\Dependencies\MyNameSpace\SubNameSpace', $basePHPElement->getNamespace());
    }
    ...
    {
        $functionMock = $this->getMockBuilder(AdaptedReflectionFunction::class)->disableOriginalConstructor()->onlyMethods(['getShortName', 'getNamespaceName'])->getMock();
        $functionMock->method('getShortName')->willReturn('foo');
        $functionMock->method('getNamespaceName')->willReturn('Masjid_App\Dependencies\SomeNamespace\SubNamespace');
        $basePHPElement = (new ReflectionFunctionParser())->parse($functionMock);
        self::assertEquals('Masjid_App\Dependencies\SomeNamespace\SubNamespace\foo', $basePHPElement->getId());
    }
    ...
    {
        $functionMock = $this->getMockBuilder(AdaptedReflectionFunction::class)->disableOriginalConstructor()->onlyMethods(['getReturnType'])->getMock();
    ...
    {
        $functionMock = $this->getMockBuilder(AdaptedReflectionFunction::class)->disableOriginalConstructor()->onlyMethods(['hasReturnType'])->getMock();
    ...
    {
        $functionMock = $this->getMockBuilder(AdaptedReflectionFunction::class)->disableOriginalConstructor()->onlyMethods(['getReturnType', 'hasReturnType'])->getMock();
        $returnTypeMock = $this->getMockBuilder(AdaptedReflectionType::class)->disableOriginalConstructor()->onlyMethods(['getName'])->getMock();
        $returnTypeMock->method('getName')->willReturn('string');
        $functionMock->method('getReturnType')->willReturn($returnTypeMock);
        $functionMock->method('hasReturnType')->willReturn(\true);
        $basePHPElement = (new ReflectionFunctionParser())->parse($functionMock);
    ...
    {
        $functionMock = $this->getMockBuilder(AdaptedReflectionFunction::class)->disableOriginalConstructor()->onlyMethods(['getReturnType', 'hasReturnType'])->getMock();
        $returnTypeMock = $this->getMockBuilder(AdaptedReflectionType::class)->disableOriginalConstructor()->onlyMethods(['getName', 'allowsNull'])->getMock();
        $returnTypeMock->method('getName')->willReturn('string');
        $returnTypeMock->method('allowsNull')->willReturn(\true);
        $functionMock->method('getReturnType')->willReturn($returnTypeMock);
        $functionMock->method('hasReturnType')->willReturn(\true);
        $basePHPElement = (new ReflectionFunctionParser())->parse($functionMock);
    ...
    {
        $functionMock = $this->getMockBuilder(AdaptedReflectionFunction::class)->disableOriginalConstructor()->onlyMethods(['getReturnType', 'hasReturnType'])->getMock();
        $returnTypeMock = $this->getMockBuilder(AdaptedReflectionType::class)->disableOriginalConstructor()->onlyMethods(['getTypes', 'isUnionType'])->getMock();
        $returnSubTypeMock = $this->getMockBuilder(AdaptedReflectionType::class)->disableOriginalConstructor()->onlyMethods(['getName'])->getMock();
        $returnSubTypeMock->method('getName')->willReturn('string');
        $returnSubTypeMock2 = $this->getMockBuilder(AdaptedReflectionType::class)->disableOriginalConstructor()->onlyMethods(['getName'])->getMock();
        ...
        $returnTypeMock->method('isUnionType')->willReturn(\true);
        $functionMock->method('getReturnType')->willReturn($returnTypeMock);
        $functionMock->method('hasReturnType')->willReturn(\true);
        $basePHPElement = (new ReflectionFunctionParser())->parse($functionMock);
    ...
    public function testItProperlyParsesAllTypesInUnionReturnTypeWithNullable()
    {
        ...
        $returnSubTypeMock2 = $this->getMockBuilder(AdaptedReflectionType::class)->disableOriginalConstructor()->onlyMethods(['getName'])->getMock();
        $returnSubTypeMock2->method('getName')->willReturn('int');
        $returnSubTypeMock3 = $this->getMockBuilder(AdaptedReflectionType::class)->disableOriginalConstructor()->onlyMethods(['getName'])->getMock();
    ...
    {
        $functionMock = $this->getMockBuilder(AdaptedReflectionFunction::class)->disableOriginalConstructor()->onlyMethods(['isDeprecated'])->getMock();
    ...
    {
        $functionMock = $this->getMockBuilder(AdaptedReflectionFunction::class)->disableOriginalConstructor()->onlyMethods(['isDeprecated'])->getMock();
        $functionMock->method('isDeprecated')->willReturn(\true);
        $basePHPElement = (new ReflectionFunctionParser())->parse($functionMock);
    ...
    {
        $functionMock = $this->getMockBuilder(AdaptedReflectionFunction::class)->disableOriginalConstructor()->onlyMethods(['getParameters'])->getMock();
    ...
    {
        $functionMock = $this->getMockBuilder(AdaptedReflectionFunction::class)->disableOriginalConstructor()->onlyMethods(['getParameters'])->getMock();
        $parameterMock = $this->getMockBuilder(AdaptedReflectionParameter::class)->disableOriginalConstructor()->onlyMethods(['getName', 'getPosition', 'isOptional', 'isVariadic', 'isPassedByReference', 'hasType', 'isDefaultValueAvailable'])->getMock();
    ...
    {
        $functionMock = $this->getMockBuilder(AdaptedReflectionFunction::class)->disableOriginalConstructor()->onlyMethods(['getParameters'])->getMock();
        $parameterMock = $this->getMockBuilder(AdaptedReflectionParameter::class)->disableOriginalConstructor()->onlyMethods(['getName', 'getPosition', 'hasType', 'getType', 'isOptional', 'getDefaultValue', 'isDefaultValueAvailable', 'isPassedByReference', 'isVariadic'])->getMock();
        $parameterMock->method('getName')->willReturn('foo');
        $parameterMock->method('getPosition')->willReturn(0);
        $parameterMock->method('hasType')->willReturn(\true);
        ...
        $parameterMock->method('isOptional')->willReturn(\true);
        $parameterMock->method('isDefaultValueAvailable')->willReturn(\true);
    ...
    {
        $functionMock = $this->getMockBuilder(AdaptedReflectionFunction::class)->disableOriginalConstructor()->onlyMethods(['getParameters'])->getMock();
        $parameterMock = $this->getMockBuilder(AdaptedReflectionParameter::class)->disableOriginalConstructor()->onlyMethods(['getName', 'getPosition', 'hasType', 'getType', 'isOptional', 'isDefaultValueAvailable', 'isPassedByReference', 'isVariadic'])->getMock();
        $parameterMock->method('getName')->willReturn('foo');
        $parameterMock->method('getPosition')->willReturn(0);
        $parameterMock->method('hasType')->willReturn(\false);
