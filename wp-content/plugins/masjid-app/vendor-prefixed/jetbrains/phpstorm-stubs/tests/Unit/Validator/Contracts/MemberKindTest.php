...
namespace Masjid_App\Dependencies\StubTests\Unit\Validator\Contracts;
...
class MemberKindTest extends TestCase
{
    ...
    {
        self::assertNotSame(MemberKind::METHOD->knownProblemEntityType(), MemberKind::PROPERTY->knownProblemEntityType());
    }
    ...
    public function testEveryCaseAnswersEveryAccessor()
    {
        foreach (MemberKind::cases() as $kind) {
            ...
            self::assertSame('\Foo', substr($kind->formatMemberId('\Foo', 'bar'), 0, 4), $kind->name . ' must prefix the owning entity id');
