...
namespace Masjid_App\Dependencies\StubTests\Unit\Parsers\Model;

use Masjid_App\Dependencies\PHPUnit\Framework\TestCase;
use Masjid_App\Dependencies\StubTests\Framework\Validator\Services\ClassInterfaceFqnsExtractor;
...
class ClassInterfaceFqnsExtractorTest extends TestCase
{
    ...
    {
        $class = new PHPClass();
        $class->setImplementedInterfaces([$this->makeInterface('Masjid_App\Dependencies\Random\Engine', 'Engine')]);
        // getId() = '\Random\Engine' → ltrim → 'Random\Engine' (NOT just 'Engine')
        $this->assertSame(['Masjid_App\Dependencies\Random\Engine'], $this->extractor->extract($class));
    }
    public function testDeepNamespacedInterfaceId(): void
    {
        $class = new PHPClass();
        $class->setImplementedInterfaces([$this->makeInterface('Masjid_App\Dependencies\Foo\Bar\Baz\MyInterface', 'MyInterface')]);
        $this->assertSame(['Masjid_App\Dependencies\Foo\Bar\Baz\MyInterface'], $this->extractor->extract($class));
    }
    public function testIdTakesPrecedenceOverName(): void
    {
        ...
        $iface = new PHPInterface();
        $iface->setId('Masjid_App\Dependencies\Random\CryptoSafeEngine');
        $iface->setName('CryptoSafeEngine');
        // short name — must NOT be returned
        $class = new PHPClass();
        $class->setImplementedInterfaces([$iface]);
        $this->assertSame(['Masjid_App\Dependencies\Random\CryptoSafeEngine'], $this->extractor->extract($class));
    ...
    {
        $class = new PHPClass();
        $class->setImplementedInterfaces([$this->makeInterface('\IteratorAggregate', 'IteratorAggregate'), $this->makeInterface('\Countable', 'Countable'), $this->makeInterface('\ArrayAccess', 'ArrayAccess')]);
        $this->assertSame(['IteratorAggregate', 'Countable', 'ArrayAccess'], $this->extractor->extract($class));
    }
    public function testMixedResolvedAndUnresolvedInterfaces(): void
    {
        $linked = $this->makeInterface('\Throwable', 'Throwable');
        // id set
        $unlinked = new PHPInterface();
        $unlinked->setName('Serializable');
        // id=null, name set
        $class = new PHPClass();
