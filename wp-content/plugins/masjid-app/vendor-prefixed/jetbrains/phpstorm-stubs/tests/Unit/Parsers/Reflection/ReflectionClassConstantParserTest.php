...
use function Masjid_App\Dependencies\PHPUnit\Framework\assertEquals;
class ReflectionClassConstantParserTest extends TestCase
{
    ...
    {
        $classConstantMock = $this->getMockBuilder(AdaptedReflectionClassConstant::class)->disableOriginalConstructor()->onlyMethods(['getName', 'getValue'])->getMock();
    ...
    {
        $classConstantMock = $this->getMockBuilder(AdaptedReflectionClassConstant::class)->disableOriginalConstructor()->onlyMethods([])->getMock();
    ...
    {
        $classConstantMock = $this->getMockBuilder(AdaptedReflectionClassConstant::class)->disableOriginalConstructor()->onlyMethods(['getName'])->getMock();
    ...
    {
        $classConstantMock = $this->getMockBuilder(AdaptedReflectionClassConstant::class)->disableOriginalConstructor()->onlyMethods(['getValue'])->getMock();
    ...
    {
        $classConstantMock = $this->getMockBuilder(AdaptedReflectionClassConstant::class)->disableOriginalConstructor()->onlyMethods(['getDeclaringClass'])->getMock();
    ...
    {
        $classConstantMock = $this->getMockBuilder(AdaptedReflectionClassConstant::class)->disableOriginalConstructor()->onlyMethods(['getDeclaringClass'])->getMock();
        $declaringClassMock = new AdaptedReflectionClassReference('Masjid_App\Dependencies\DummyNamespace\ParentClass');
        $classConstantMock->method('getDeclaringClass')->willReturn($declaringClassMock);
        $parsedConstant = (new ReflectionClassConstantParser())->parse($classConstantMock);
        assertEquals('Masjid_App\Dependencies\DummyNamespace\ParentClass', $parsedConstant->getParentId());
    }
    public function testItCanParseConstantPrivateVisibility()
    {
        $classConstantMock = $this->getMockBuilder(AdaptedReflectionClassConstant::class)->disableOriginalConstructor()->onlyMethods(['isPrivate'])->getMock();
        $classConstantMock->method('isPrivate')->willReturn(\true);
        $parsedConstant = (new ReflectionClassConstantParser())->parse($classConstantMock);
        self::assertSame(AccessModifier::PRIVATE, $parsedConstant->getAccess());
    }
    public function testItCanParseConstantProtectedVisibility()
    {
        $classConstantMock = $this->getMockBuilder(AdaptedReflectionClassConstant::class)->disableOriginalConstructor()->onlyMethods(['isProtected'])->getMock();
        $classConstantMock->method('isProtected')->willReturn(\true);
        $parsedConstant = (new ReflectionClassConstantParser())->parse($classConstantMock);
        self::assertSame(AccessModifier::PROTECTED, $parsedConstant->getAccess());
    }
    public function testItCanParseConstantPublicVisibility()
    {
        $classConstantMock = $this->getMockBuilder(AdaptedReflectionClassConstant::class)->disableOriginalConstructor()->onlyMethods(['isPublic'])->getMock();
        $classConstantMock->method('isPublic')->willReturn(\true);
        $parsedConstant = (new ReflectionClassConstantParser())->parse($classConstantMock);
    ...
    {
        $classConstantMock = $this->getMockBuilder(AdaptedReflectionClassConstant::class)->disableOriginalConstructor()->onlyMethods(['isPublic', 'isProtected', 'isPrivate'])->getMock();
    ...
    {
        return ['final constant stays final' => [\true, \true], 'non-final stays non-final' => [\false, \false]];
    }
    /**
