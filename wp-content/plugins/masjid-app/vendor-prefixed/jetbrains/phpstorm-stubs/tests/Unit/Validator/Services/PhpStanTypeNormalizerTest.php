...
use Masjid_App\Dependencies\StubTests\Framework\Validator\Services\PhpStanTypeNormalizer;
...
class PhpStanTypeNormalizerTest extends TestCase
{
    ...
    public function testEveryLeafTargetIsAPlainPhpType(string $leaf, string $target): void
    {
        foreach (explode('|', $target) as $component) {
            $isPrimitive = in_array($component, TypeResolver::PRIMITIVES, \true);
            ...
            self::assertTrue($isPrimitive || $isRealClass, "leaf '{$leaf}' maps to '{$target}', whose component '{$component}' is neither a PHP primitive nor an existing class");
        }
    }
    ...
    public static function leafTokens(): array
    {
        ...
        foreach (array_keys(self::leafMap()) as $leaf) {
            ...
            if (in_array($leaf, ['array', 'iterable'], \true)) {
                continue;
            }
            $cases[$leaf] = [$leaf];
        }
    ...
    public static function strippedForms(): array
    {
        return [
            ...
            'class constant' => ['Masjid_App\Dependencies\Foo\Bar::BAZ', 'mixed'],
    ...
    {
        return ['sorts a union' => ['string|int', 'int|string'], 'nullable shorthand sorts too' => ['?string', 'null|string'], 'generic then sort' => ['array<string, int>', 'array'], 'leaf then sort' => ['numeric-string|positive-int', 'int|string'], 'array-key expands to a union' => ['array-key', 'int|string']];
    }
}
