...
use Masjid_App\Dependencies\StubTests\Framework\Model\PHPClassLikeObject;
...
class ReflectionInterfaceParserTest extends TestCase
{
    ...
    public function testItCanParseConstants()
    {
        ...
        $reflectionClassConstantsMock = $this->getMockBuilder(AdaptedReflectionClassConstant::class)->disableOriginalConstructor()->onlyMethods(['getName', 'getValue'])->getMock();
    ...
    {
        $reflectionClassMock = $this->getMockBuilder(AdaptedReflectionClass::class)->disableOriginalConstructor()->getMock();
        $constantMock = $this->getMockBuilder(AdaptedReflectionClassConstant::class)->disableOriginalConstructor()->onlyMethods(['getName', 'getValue', 'isEnumCase'])->getMock();
    ...
    {
        $reflectionClassMock = $this->getMockBuilder(AdaptedReflectionClass::class)->disableOriginalConstructor()->getMock();
        $reflectionClassMock->method('getInterfaces')->willReturn([new AdaptedReflectionClassReference('Iterator'), new AdaptedReflectionClassReference('Traversable')]);
        $interface = (new ReflectionInterfaceParser())->parse($reflectionClassMock);
        $parents = $interface->getParentInterfaces();
        self::assertSame(['\Iterator', '\Traversable'], array_map(fn($parent) => $parent->getId(), $parents), 'Reflection reports the transitive ancestor set; every entry must keep its FQN id');
