...
namespace Masjid_App\Dependencies\StubTests\Unit\DataProviders;

use Masjid_App\Dependencies\PHPUnit\Framework\TestCase;
use Masjid_App\Dependencies\StubTests\Framework\DataProvider\CoreStubsDataProvider;
use Masjid_App\Dependencies\StubTests\Framework\DataProvider\StubCategory;
use Masjid_App\Dependencies\StubTests\Framework\DataProvider\StubsDataProvider;
class CoreStubsDataProviderTest extends TestCase
{
    public function testItReturnsOnlyCoreStubs()
    {
        ...
        foreach ($stubFiles as $file) {
            ...
            self::assertTrue(StubCategory::CORE->containsDirectory($topLevelDir), "File {$relativePath} is not from a CORE directory");
        }
    }
    public function testItReturnsOnlyBundledStubs()
    {
        ...
        foreach ($stubFiles as $file) {
            $relativePath = $this->getRelativePath($file, $provider->getStubsRootPath());
            $topLevelDir = $this->getTopLevelDirectory($relativePath);
            self::assertTrue(StubCategory::BUNDLED->containsDirectory($topLevelDir), "File {$relativePath} is not from a BUNDLED directory");
        }
    }
    public function testItReturnsOnlyExternalStubs()
    {
        ...
        foreach ($stubFiles as $file) {
            $relativePath = $this->getRelativePath($file, $provider->getStubsRootPath());
            $topLevelDir = $this->getTopLevelDirectory($relativePath);
            self::assertTrue(StubCategory::EXTERNAL->containsDirectory($topLevelDir), "File {$relativePath} is not from an EXTERNAL directory");
        }
    }
    public function testItReturnsOnlyPeclStubs()
    {
        ...
        foreach ($stubFiles as $file) {
            $relativePath = $this->getRelativePath($file, $provider->getStubsRootPath());
            $topLevelDir = $this->getTopLevelDirectory($relativePath);
            self::assertTrue(StubCategory::PECL->containsDirectory($topLevelDir), "File {$relativePath} is not from a PECL directory");
        }
    }
    public function testItReturnsOnlyOthersStubs()
    {
        ...
        foreach ($stubFiles as $file) {
            $relativePath = $this->getRelativePath($file, $provider->getStubsRootPath());
            $topLevelDir = $this->getTopLevelDirectory($relativePath);
            self::assertTrue(StubCategory::OTHERS->containsDirectory($topLevelDir), "File {$relativePath} is not from an OTHERS directory");
        }
    }
    public function testItReturnsMultipleCategories()
    {
        ...
        foreach ($stubFiles as $file) {
            $relativePath = $this->getRelativePath($file, $provider->getStubsRootPath());
            $topLevelDir = $this->getTopLevelDirectory($relativePath);
            $isValid = StubCategory::CORE->containsDirectory($topLevelDir) || StubCategory::BUNDLED->containsDirectory($topLevelDir);
            self::assertTrue($isValid, "File {$relativePath} is not from CORE or BUNDLED directory");
        }
    }
    ...
    public function testItScansTheInnerProviderOnlyOnceAndReusesTheResult()
    {
        $inner = new class implements StubsDataProvider
        {
            ...
            {
                $this->scanCount++;
                return ['/proj/stubs/Core/Core_c.php', '/proj/stubs/curl/curl.php'];
            }
    ...
    public function testCoreAndBundledHaveMoreFilesThanCoreAlone()
    {
        ...
        self::assertGreaterThan(count($coreFiles), count($coreAndBundledFiles), 'Core + Bundled should have more files than Core alone');
    }
    public function testItIncludesExpectedCoreDirectories()
    {
        ...
        foreach ($stubFiles as $file) {
            $relativePath = $this->getRelativePath($file, $provider->getStubsRootPath());
            $topLevelDir = $this->getTopLevelDirectory($relativePath);
            $foundDirectories[$topLevelDir] = \true;
        }
        // Check for some expected core directories
        $expectedDirs = ['Core', 'Reflection', 'SPL', 'standard'];
        foreach ($expectedDirs as $dir) {
            self::assertArrayHasKey($dir, $foundDirectories, "Expected core directory '{$dir}' not found in stub files");
        }
    }
    public function testItIncludesExpectedBundledDirectories()
    {
        ...
        foreach ($expectedDirs as $dir) {
            self::assertArrayHasKey($dir, $foundDirectories, "Expected bundled directory '{$dir}' not found in stub files");
        }
    }
    public function testItIncludesExpectedExternalDirectories()
    {
        ...
        foreach ($expectedDirs as $dir) {
            self::assertArrayHasKey($dir, $foundDirectories, "Expected external directory '{$dir}' not found in stub files");
        }
    }
    /**
    ...
    {
        return ['windows backslash paths' => ['C:\proj\stubs', ['C:\proj\stubs\Core\Core_c.php', 'C:\proj\stubs\curl\curl.php']], 'posix forward-slash paths' => ['/proj/stubs', ['/proj/stubs/Core/Core_c.php', '/proj/stubs/curl/curl.php']]];
    }
    #[\Masjid_App\Dependencies\PHPUnit\Framework\Attributes\DataProvider('separatorStyleProvider')]
