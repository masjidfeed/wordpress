...
namespace Masjid_App\Dependencies\StubTests\Unit\Runner;
...
{
    private const REQUIRED_FILES = ['StubsClasses.json', 'StubsFunctions.json', 'StubsInterfaces.json', 'StubsEnums.json', 'StubsConstants.json', 'StubsPhpDoc.json'];
    private string $cacheDir;
    protected function setUp(): void
    {
        parent::setUp();
        $this->cacheDir = sys_get_temp_dir() . '/runner_cache_test_' . uniqid('', \true);
        mkdir($this->cacheDir, 0777, \true);
    }
    ...
    public function testMissingPhpDocFileIsIncomplete(): void
    {
        ...
        $runner = new Runner(cacheDir: $this->cacheDir);
        $this->assertFalse($runner->isStubsCacheComplete(), 'A cache missing StubsPhpDoc.json must be considered incomplete.');
    }
    public function testMissingAnySingleRequiredFileIsIncomplete(): void
    {
        foreach (self::REQUIRED_FILES as $missing) {
            ...
            $runner = new Runner(cacheDir: $this->cacheDir);
            $this->assertFalse($runner->isStubsCacheComplete(), "Cache missing {$missing} must be considered incomplete.");
        }
    }
    /**
     * @param string[] $files
