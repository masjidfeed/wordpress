...
namespace Masjid_App\Dependencies\StubTests\Unit\Validator\Constants;
...
use Masjid_App\Dependencies\StubTests\Framework\Validator\Constants\ConstantExistsCheck;
...
class ConstantExistsCheckTest extends CheckTestCase
{
    ...
    public function testCorrectConstantAmongMultiplePasses(): void
    {
        ...
        $stubs = $this->createMockStorageManager();
        $stubs->method('getConstants')->willReturn([$this->makeGlobalConstant('\PHP_MINOR_VERSION'), $this->makeGlobalConstant($constantId), $this->makeGlobalConstant('\PHP_RELEASE_VERSION')]);
        $result = (new ConstantExistsCheck())->run($stubs, $constantId, PhpVersions::LATEST->value);
        $this->assertFalse($result->hasFailures());
    }
    // ── Known problem skip ────────────────────────────────────────────────────
    public function testKnownProblemSkipsCheck(): void
    {
        ...
        $knownProblemsProvider->method('getProblems')->willReturn([new ProblemDefinition(entityType: EntityType::GLOBAL_CONSTANT, entityId: $constantId, type: ProblemType::INTERNAL_IMPLEMENTATION, affectedChecks: [CheckType::CONSTANT_EXISTS], versionRange: new PhpVersionRange(PhpVersions::EARLIEST, PhpVersions::LATEST), reason: 'PHP language keyword reported as constant by reflection')]);
        KnownProblemsRegistry::reset();
    ...
    public function testKnownProblemOnlySkipsForVersionRange(): void
    {
        ...
        $knownProblemsProvider = $this->createStub(KnownProblemsProvider::class);
        $knownProblemsProvider->method('getProblems')->willReturn([new ProblemDefinition(entityType: EntityType::GLOBAL_CONSTANT, entityId: $constantId, type: ProblemType::INTERNAL_IMPLEMENTATION, affectedChecks: [CheckType::CONSTANT_EXISTS], versionRange: new PhpVersionRange(PhpVersions::PHP_8_0, PhpVersions::PHP_8_3), reason: 'Only present in 8.0-8.3')]);
        KnownProblemsRegistry::reset();
        $registry = KnownProblemsRegistry::getInstance($knownProblemsProvider);
        $stubs = $this->createMockStorageManager();
        $stubs->method('getConstants')->willReturn([]);
