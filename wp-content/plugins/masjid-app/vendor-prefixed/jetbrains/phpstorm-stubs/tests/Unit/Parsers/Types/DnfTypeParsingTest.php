...
use Masjid_App\Dependencies\StubTests\Framework\Parsers\Stubs\Types\TypeNodeConverter;
...
class DnfTypeParsingTest extends TestCase
{
    ...
    {
        return ['dnf int|(Foo&Bar)' => ['int|(\Foo&\Bar)', 'int|(\Foo&\Bar)'], 'dnf null|(Foo&Bar)' => ['null|(\Foo&\Bar)', 'null|(\Foo&\Bar)'], 'dnf int|(Foo&Bar)|null' => ['int|(\Foo&\Bar)|null', 'int|(\Foo&\Bar)|null'], 'pure intersection (Foo&Bar)' => ['(\Foo&\Bar)', '\Foo&\Bar'], 'plain union string|int' => ['string|int', 'string|int'], 'nullable string|null' => ['string|null', 'string|null'], 'standalone int' => ['int', 'int']];
    }
    ...
    public function testTypeNodeConverterRoundtrip(string $typeStr, string $expected): void
    {
        ...
        $typeNode = new class($typeStr) implements \Masjid_App\Dependencies\StubTests\Framework\Parsers\Stubs\Nodes\TypeNode
        {
    ...
    {
        $typeNode = new class('int|(\Foo&\Bar)') implements \Masjid_App\Dependencies\StubTests\Framework\Parsers\Stubs\Nodes\TypeNode
        {
            public function toString(): string
            {
                return 'int|(\Foo&\Bar)';
    ...
    {
        $typeNode = new class('(\Foo&\Bar)') implements \Masjid_App\Dependencies\StubTests\Framework\Parsers\Stubs\Nodes\TypeNode
        {
            public function toString(): string
            {
                return '(\Foo&\Bar)';
    ...
    public function testTypeNodeConverterNullableDnfIsUnionNotNullable(): void
    {
        ...
        $typeNode = new class('null|(\Foo&\Bar)') implements \Masjid_App\Dependencies\StubTests\Framework\Parsers\Stubs\Nodes\TypeNode
        {
            public function toString(): string
            {
                return 'null|(\Foo&\Bar)';
...
namespace Masjid_App\Dependencies\Test;
...
PHP;
        $parser = new \Masjid_App\Dependencies\StubTests\Framework\Parsers\Stubs\StubFunctionParser();
