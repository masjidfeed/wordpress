...
class AllStubsDataProviderTest extends TestCase
{
    ...
    {
        return ['posix root' => ['/usr/local/stubs/dom.php', \true], 'windows backslash drive' => ['C:\stubs\dom\dom_n.php', \true], 'windows forward-slash drive' => ['C:/stubs/dom/dom_n.php', \true], 'lowercase drive letter' => ['d:\projects\stubs.php', \true], 'windows UNC path' => ['\\\\server\share\stubs.php', \true], 'relative forward slash' => ['dom/dom_n.php', \false], 'relative backslash' => ['dom\dom_n.php', \false], 'bare filename' => ['dom_n.php', \false]];
    }
    ...
    public function testExclusionIsRelativeToTheStubsRootSoACheckoutUnderSuchADirectoryStillWorks()
    {
        ...
        foreach (['tests/phpstorm-stubs', '.git/worktrees/wt', 'vendor/x', '.idea/x'] as $suffix) {
            ...
            self::assertFalse($method->invoke($provider, $root . '/dom/dom_n.php'), "A stub in a checkout rooted at {$suffix} must not be excluded");
            // Exclusion still applies to those directory names *inside* the checkout.
            self::assertTrue($method->invoke($provider, $root . '/vendor/nikic/file.php'), "vendor/ inside a checkout rooted at {$suffix} must still be excluded");
        }
    }
    ...
    public function testItRecursivelyDiscoversNestedStubsAndPrunesExcludedDirectories()
    {
        ...
        foreach ($layout as $relative => $contents) {
            $full = $root . '/' . $relative;
            @mkdir(dirname($full), 0777, \true);
            file_put_contents($full, $contents);
        }
        try {
            $found = (new AllStubsDataProvider($root))->getAllStubFiles();
            $relative = array_map(fn($p) => ltrim(str_replace('\\', '/', substr($p, strlen($root))), '/'), $found);
            sort($relative);
