...
namespace Masjid_App\Dependencies\StubTests\Unit\Parsers\Serializers;

use Masjid_App\Dependencies\PHPUnit\Framework\TestCase;
use Masjid_App\Dependencies\StubTests\Framework\Model\PHPInterface;
use Masjid_App\Dependencies\StubTests\Framework\Serialization\Stubs\PHPInterfaceSerializer;
class PHPInterfaceSerializerTest extends TestCase
{
    ...
    {
        $iface = $this->makeInterface('\OuterIterator', '\\', [$this->makeParentStub('Iterator')]);
        $data = $this->serializer->serialize($iface);
        $this->assertSame(['Iterator'], $data['parentInterfaces']);
    }
    public function testSerializeInterfaceWithMultipleParentInterfaces(): void
    {
        $iface = $this->makeInterface('\BackedEnum', '\\', [$this->makeParentStub('UnitEnum'), $this->makeParentStub('Stringable')]);
        $data = $this->serializer->serialize($iface);
        $this->assertSame(['UnitEnum', 'Stringable'], $data['parentInterfaces']);
    }
    public function testSerializePreservesParentInterfaceOrder(): void
    {
        $iface = $this->makeInterface('\CombinedInterface', '\\', [$this->makeParentStub('Alpha'), $this->makeParentStub('Beta'), $this->makeParentStub('Gamma')]);
        $data = $this->serializer->serialize($iface);
    ...
    {
        $data = ['_type' => 'PHPInterface', 'name' => 'Iterator', 'id' => '\Iterator', 'namespace' => '\\'];
    ...
    {
        $data = ['name' => 'Iterator', 'id' => '\Iterator', 'namespace' => '\\', 'parentInterfaces' => []];
        $iface = $this->serializer->deserialize($data);
        $this->assertEmpty($iface->getParentInterfaces());
    }
    public function testDeserializeInterfaceRestoresParentInterfaceObjects(): void
    {
        $data = ['name' => 'OuterIterator', 'id' => '\OuterIterator', 'namespace' => '\\', 'parentInterfaces' => ['Iterator']];
        $iface = $this->serializer->deserialize($data);
    ...
    {
        $data = ['name' => 'BackedEnum', 'id' => '\BackedEnum', 'namespace' => '\\', 'parentInterfaces' => ['UnitEnum', 'Stringable']];
        $iface = $this->serializer->deserialize($data);
        $parents = $iface->getParentInterfaces();
    ...
    {
        $data = ['name' => 'SomeInterface', 'id' => '\SomeInterface', 'namespace' => '\\', 'parentInterfaces' => ['', 'ValidParent', '']];
        $iface = $this->serializer->deserialize($data);
        $parents = $iface->getParentInterfaces();
        $this->assertCount(1, $parents);
    ...
    {
        $original = $this->makeInterface('\SeekableIterator', '\\', [$this->makeParentStub('Iterator')]);
