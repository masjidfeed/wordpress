...
use Masjid_App\Dependencies\StubTests\Framework\Validator\Classes\Properties\ClassPropertyReadonlyCheck;
...
class ClassPropertyReadonlyCheckTest extends CheckTestCase
{
    ...
    {
        $className = '\MyClass';
        $reflClass = $this->createMockClassWithProperties($className, null, null, null, [], null, [], [$this->makeProperty('name', isReadonly: \false)]);
        $stubClass = $this->createMockClassWithProperties($className, null, null, null, [], null, [], [$this->makeProperty('name', isReadonly: \false)]);
        $provider = $this->createMockReflectionProvider([], [$reflClass]);
        $stubs = $this->createMockStorageManager();
        $stubs->method('getClasses')->willReturn([$stubClass]);
        $result = (new ClassPropertyReadonlyCheck($provider))->run($stubs, $className, '8.1');
    ...
    {
        $className = '\MyClass';
        $reflClass = $this->createMockClassWithProperties($className, null, null, null, [], null, [], [$this->makeProperty('name', isReadonly: \true)]);
        $stubClass = $this->createMockClassWithProperties($className, null, null, null, [], null, [], [$this->makeProperty('name', isReadonly: \true)]);
        $provider = $this->createMockReflectionProvider([], [$reflClass]);
        $stubs = $this->createMockStorageManager();
        $stubs->method('getClasses')->willReturn([$stubClass]);
        $result = (new ClassPropertyReadonlyCheck($provider))->run($stubs, $className, '8.1');
    ...
    public function testMultiplePropertiesMixedResultsAreAllReported(): void
    {
        ...
        $reflClass = $this->createMockClassWithProperties($className, null, null, null, [], null, [], [
            ...
            // matches
            $this->makeProperty('value', isReadonly: \true),
        ]);
        $stubClass = $this->createMockClassWithProperties($className, null, null, null, [], null, [], [$this->makeProperty('name', isReadonly: \false), $this->makeProperty('value', isReadonly: \false)]);
        $provider = $this->createMockReflectionProvider([], [$reflClass]);
        $stubs = $this->createMockStorageManager();
        $stubs->method('getClasses')->willReturn([$stubClass]);
        $result = (new ClassPropertyReadonlyCheck($provider))->run($stubs, $className, '8.1');
    ...
    {
        $className = '\MyClass';
        $reflClass = $this->createMockClassWithProperties($className, null, null, null, [], null, [], [$this->makeProperty('onlyInReflection', isReadonly: \true)]);
        $stubClass = $this->createMockClassWithProperties($className);
        // no properties
        $provider = $this->createMockReflectionProvider([], [$reflClass]);
        $stubs = $this->createMockStorageManager();
    ...
    public function testClassLevelKnownProblemSkipsAllProperties(): void
    {
        ...
        $knownProblemsProvider->method('getProblems')->willReturn([new ProblemDefinition(entityType: EntityType::CLASS_TYPE, entityId: $className, type: ProblemType::INTERNAL_IMPLEMENTATION, affectedChecks: [CheckType::CLASS_PROPERTIES_READONLY], versionRange: new PhpVersionRange(PhpVersions::EARLIEST, PhpVersions::LATEST), reason: 'Class-level skip')]);
        KnownProblemsRegistry::reset();
    ...
    public function testPropertyLevelKnownProblemSkipsSpecificMismatch(): void
    {
        ...
        $reflClass = $this->createMockClassWithProperties($className, null, null, null, [], null, [], [
            ...
            // matches
            $this->makeProperty('name', isReadonly: \true),
        ]);
        $stubClass = $this->createMockClassWithProperties($className, null, null, null, [], null, [], [$this->makeProperty('value', isReadonly: \false), $this->makeProperty('name', isReadonly: \false)]);
        $knownProblemsProvider = $this->createStub(\Masjid_App\Dependencies\StubTests\Framework\Validator\KnownProblems\KnownProblemsProvider::class);
        $knownProblemsProvider->method('getProblems')->willReturn([new ProblemDefinition(entityType: EntityType::PROPERTY, entityId: $mismatchedId, type: ProblemType::INTERNAL_IMPLEMENTATION, affectedChecks: [CheckType::CLASS_PROPERTIES_READONLY], versionRange: new PhpVersionRange(PhpVersions::EARLIEST, PhpVersions::LATEST), reason: 'Property-level skip')]);
        KnownProblemsRegistry::reset();
        $registry = KnownProblemsRegistry::getInstance($knownProblemsProvider);
        $provider = $this->createMockReflectionProvider([], [$reflClass]);
        $stubs = $this->createMockStorageManager();
    ...
    public function testPropertyLevelKnownProblemDoesNotSuppressOtherMismatches(): void
    {
        ...
        $stubClass = $this->createMockClassWithProperties($className, null, null, null, [], null, [], [$this->makeProperty('name', isReadonly: \false), $this->makeProperty('value', isReadonly: \false)]);
        $knownProblemsProvider = $this->createStub(\Masjid_App\Dependencies\StubTests\Framework\Validator\KnownProblems\KnownProblemsProvider::class);
        $knownProblemsProvider->method('getProblems')->willReturn([new ProblemDefinition(entityType: EntityType::PROPERTY, entityId: $className . '::$name', type: ProblemType::INTERNAL_IMPLEMENTATION, affectedChecks: [CheckType::CLASS_PROPERTIES_READONLY], versionRange: new PhpVersionRange(PhpVersions::EARLIEST, PhpVersions::LATEST), reason: 'Only name is known')]);
        KnownProblemsRegistry::reset();
        $registry = KnownProblemsRegistry::getInstance($knownProblemsProvider);
        $provider = $this->createMockReflectionProvider([], [$reflClass]);
        $stubs = $this->createMockStorageManager();
    ...
    public function testStubPropertyBelowSinceVersionIsExcluded(): void
    {
        $className = '\MyClass';
        $reflClass = $this->createMockClassWithProperties($className, null, null, null, [], null, [], [$this->makeProperty('name', isReadonly: \true)]);
        $stubClass = $this->createMockClassWithProperties($className, null, null, null, [], null, [], [$this->makeProperty('name', isReadonly: \false, sinceVersion: '8.2')]);
        $provider = $this->createMockReflectionProvider([], [$reflClass]);
        $stubs = $this->createMockStorageManager();
        $stubs->method('getClasses')->willReturn([$stubClass]);
        $result = (new ClassPropertyReadonlyCheck($provider))->run($stubs, $className, '8.1');
    ...
    public function testStubPropertyWithinVersionRangeIsIncluded(): void
    {
        $className = '\MyClass';
        $reflClass = $this->createMockClassWithProperties($className, null, null, null, [], null, [], [$this->makeProperty('name', isReadonly: \true)]);
        $stubClass = $this->createMockClassWithProperties($className, null, null, null, [], null, [], [$this->makeProperty('name', isReadonly: \false, sinceVersion: '8.1', removedVersion: '8.4')]);
        $provider = $this->createMockReflectionProvider([], [$reflClass]);
        $stubs = $this->createMockStorageManager();
        $stubs->method('getClasses')->willReturn([$stubClass]);
        $result = (new ClassPropertyReadonlyCheck($provider))->run($stubs, $className, '8.1');
    ...
    public function testReadonlyPropertyInheritedFromParentMismatchIsReported(): void
    {
        ...
        $parentStub->setProperties([$this->makeProperty('name', isReadonly: \false)]);
        // parent: non-readonly
    ...
    public function testReadonlyPropertyInheritedFromParentMatchIsSuccess(): void
    {
        ...
        $parentStub = new PHPClass();
        $parentStub->setId($parentClassName);
        $parentStub->setProperties([$this->makeProperty('name', isReadonly: \true)]);
        // matches
        $childStub = $this->createMockClassWithProperties($className);
        $childStub->setParentClass($parentStub);
        $provider = $this->createMockReflectionProvider([], [$reflClass]);
    ...
    public function testChildPropertyOverridesParentForReadonlyCheck(): void
    {
        ...
        $parentStub->setId($parentClassName);
        $parentStub->setProperties([$this->makeProperty('name', isReadonly: \true)]);
        // parent: readonly (must not win)
        $childStub = $this->createMockClassWithProperties($className, null, null, null, [], null, [], [$this->makeProperty('name', isReadonly: \false)]);
        $childStub->setParentClass($parentStub);
        $provider = $this->createMockReflectionProvider([], [$reflClass]);
        $stubs = $this->createMockStorageManager();
        $stubs->method('getClasses')->willReturn([$childStub]);
    ...
    {
        $className = '\MyClass';
        $reflClass = $this->createMockClassWithProperties($className, properties: [$this->makeProperty('id', isReadonly: \true)]);
        // The child declares nothing itself, so `id` can only come from the parent.
        $stubClass = $this->createMockClassWithProperties($className);
        $parent = $this->createMockClassWithProperties('\ParentClass', properties: [$this->makeProperty('id', isReadonly: \false)]);
        ...
        $stubs->method('getClasses')->willReturn([$stubClass]);
        $result = (new ClassPropertyReadonlyCheck($provider))->run($stubs, $className, '8.1');
        $this->assertSame(['\MyClass::$id' => 'Property \MyClass::$id is readonly in PHP 8.1 but non-readonly in stubs'], $result->getFailures(), 'The cyclic walk must terminate having visited the parent exactly once');
    }
}
