...
use Masjid_App\Dependencies\StubTests\Framework\Validator\Functions\FunctionOptionalParametersCheck;
...
class FunctionOptionalParametersCheckTest extends CheckTestCase
{
    ...
    {
        $id = '\my_func';
        $reflParams = [$this->makeParam('a', optional: \true), $this->makeParam('b', optional: \true), $this->makeParam('c', optional: \true)];
        $stubParams = [$this->makeParam('a'), $this->makeParam('b'), $this->makeParam('c')];
        $provider = $this->createMockReflectionProvider([$this->makeFunction($id, $reflParams)]);
        $stubs = $this->createMockStorageManager();
        $stubs->method('getFunctions')->willReturn([$this->makeFunction($id, $stubParams)]);
        $result = (new FunctionOptionalParametersCheck($provider))->run($stubs, $id, '8.0');
    ...
    public function testOptionalInStubsButNotInReflectionIsSuccess(): void
    {
        ...
        $id = '\my_func';
        $reflParams = [$this->makeParam('a')];
        // required in reflection
        $stubParams = [$this->makeParam('a', optional: \true)];
        // optional in stubs
        $provider = $this->createMockReflectionProvider([$this->makeFunction($id, $reflParams)]);
        $stubs = $this->createMockStorageManager();
        $stubs->method('getFunctions')->willReturn([$this->makeFunction($id, $stubParams)]);
    ...
    public function testVersionRemovedStubParamIsIgnored(): void
    {
        ...
        $id = '\my_func';
        $reflParams = [$this->makeParam('a'), $this->makeParam('b', optional: \true)];
        $stubParams = [$this->makeParam('a'), $this->makeParam('b', removedVersion: '7.4')];
        $provider = $this->createMockReflectionProvider([$this->makeFunction($id, $reflParams)]);
        $stubs = $this->createMockStorageManager();
        $stubs->method('getFunctions')->willReturn([$this->makeFunction($id, $stubParams)]);
        $result = (new FunctionOptionalParametersCheck($provider))->run($stubs, $id, '8.0');
    ...
    public function testKnownProblemSkipsValidation(): void
    {
        ...
        $knownProblemsProvider->method('getProblems')->willReturn([new ProblemDefinition(entityType: EntityType::FUNCTION, entityId: $id, type: ProblemType::INTERNAL_IMPLEMENTATION, affectedChecks: [CheckType::OPTIONAL_PARAMETERS], versionRange: new PhpVersionRange(PhpVersions::EARLIEST, PhpVersions::LATEST), reason: 'Optional params skip reason')]);
        KnownProblemsRegistry::reset();
        $registry = KnownProblemsRegistry::getInstance($knownProblemsProvider);
        // Mismatch: refl optional, stub not optional — would normally fail
        $provider = $this->createMockReflectionProvider([$this->makeFunction($id, [$this->makeParam('x', optional: \true)])]);
        $stubs = $this->createMockStorageManager();
        $stubs->method('getFunctions')->willReturn([$this->makeFunction($id, [$this->makeParam('x')])]);
