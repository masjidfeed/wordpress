...
class ReflectionParserReturnTypeContractTest extends TestCase
{
     ...
     */
    private const EXPECTED_RETURN_TYPES = ['ReflectionClassConstantParser' => PHPClassConstant::class, 'ReflectionClassParser' => PHPClass::class, 'ReflectionDefineConstantParser' => PHPConstant::class, 'ReflectionEnumParser' => PHPEnum::class, 'ReflectionFunctionParser' => PHPFunction::class, 'ReflectionImplementedInterfaceParser' => PHPInterface::class, 'ReflectionInterfaceParser' => PHPInterface::class, 'ReflectionMethodParser' => PHPMethod::class, 'ReflectionModernConstantParser' => PHPConstant::class, 'ReflectionParameterParser' => PHPParameter::class, 'ReflectionParentClassParser' => PHPClass::class, 'ReflectionPropertyParser' => PHPProperty::class];
    ...
    public function testParseDeclaresItsModelReturnTypeNonNullable(string $parser, string $expectedModel): void
    {
        ...
        self::assertInstanceOf(\ReflectionNamedType::class, $returnType, "{$parser}::parse() must keep a single named return type");
        self::assertSame($expectedModel, $returnType->getName(), "{$parser}::parse() must keep returning {$expectedModel}");
        self::assertFalse($returnType->allowsNull(), "{$parser}::parse() must stay non-nullable — callers dereference the result directly");
    }
    /**
    ...
    public function testEveryReflectionParserIsCovered(): void
    {
        ...
        sort($discovered);
        sort($accountedFor);
        self::assertSame($accountedFor, $discovered, 'Every Reflection*Parser must be listed in EXPECTED_RETURN_TYPES (or UNTYPED_BY_DESIGN).');
    }
}
