...
namespace Masjid_App\Dependencies\StubTests\Unit\Parsers\Storage;
...
use Masjid_App\Dependencies\StubTests\Framework\Runner\Runner;
use Masjid_App\Dependencies\StubTests\Framework\Serialization\PhpDocRepository;
use Masjid_App\Dependencies\StubTests\Framework\Storage\DefaultParsedDataStorageManager;
use Masjid_App\Dependencies\StubTests\Framework\Storage\ParsedDataPersistence;
use Masjid_App\Dependencies\StubTests\Framework\Storage\ParsedDataStorageManager;
use Masjid_App\Dependencies\StubTests\Framework\Storage\ParsedDataWriter;
...
class StorageInterfaceSegregationTest extends TestCase
{
    ...
    {
        self::assertFalse((new \ReflectionClass(StubDataQueryInterface::class))->hasMethod($method), "{$method}() is reachable from a read-only handle");
    }
    ...
    {
        $methods = array_map(static fn(\ReflectionMethod $m): string => $m->getName(), (new \ReflectionClass(StubDataQueryInterface::class))->getMethods());
        sort($methods);
        self::assertSame(['getClasses', 'getConstants', 'getEnums', 'getFunctions', 'getInterfaces', 'hasClass', 'hasEnum', 'hasInterface'], $methods);
    }
    /**
    ...
    {
        return ['stubs parser' => [AllStubsParser::class, 1], 'reflection parser' => [AllReflectionParser::class, 1]];
    }
    ...
    {
        self::assertTrue((new \ReflectionClass(DefaultParsedDataStorageManager::class))->implementsInterface(ParsedDataStorageManager::class));
    }
    /**
    ...
    {
        self::assertSame('Masjid_App\Dependencies\StubTests\Framework\Storage', (new \ReflectionClass(StubDataQueryInterface::class))->getNamespaceName());
    ...
    {
        $methods = array_map(static fn(\ReflectionMethod $m): string => $m->getName(), (new \ReflectionClass(PhpDocRepository::class))->getMethods());
        sort($methods);
        self::assertSame(['getPhpDoc', 'setPhpDoc'], $methods);
        self::assertTrue((new \ReflectionClass(PhpDocStorage::class))->implementsInterface(PhpDocRepository::class), 'PhpDocStorage must remain usable wherever a PhpDocRepository is expected');
    }
}
