...
use Masjid_App\Dependencies\StubTests\Framework\Storage\InMemoryParsedDataStorage;
/**
...
class CoreStubsDataProviderUsageExampleTest extends TestCase
{
    ...
    public function testParseOnlyCoreStubs()
    {
        ...
        $parser = new AllStubsParser($dataProvider, $storageManager, [new StubClassParser(), new StubFunctionParser()]);
        $parser->parseAll();
    ...
    {
        $dataProvider = new CoreStubsDataProvider([StubCategory::CORE, StubCategory::BUNDLED, StubCategory::EXTERNAL]);
        ...
        foreach ($stubFiles as $file) {
            ...
            self::assertFalse(StubCategory::PECL->containsDirectory($topLevelDir), "PECL directory {$topLevelDir} should not be included in type hint validation");
        }
    }
}
