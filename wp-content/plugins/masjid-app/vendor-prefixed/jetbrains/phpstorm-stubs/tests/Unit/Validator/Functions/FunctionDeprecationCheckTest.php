...
class FunctionDeprecationCheckTest extends CheckTestCase
{
    ...
    {
        $id = '\old_func';
        $provider = $this->createMockReflectionProvider([$this->makeFunction($id, isDeprecated: \true)]);
        $stubs = $this->createMockStorageManager();
        $stubs->method('getFunctions')->willReturn([$this->makeFunction($id, isDeprecated: \true)]);
    ...
    public function testStubDeprecationStartingLaterThanTheVersionUnderTestDoesNotSatisfyReflection(): void
    {
        ...
        $provider = $this->createMockReflectionProvider([$this->makeFunction($id, isDeprecated: \true)]);
        $stubs = $this->createMockStorageManager();
        $stubs->method('getFunctions')->willReturn([$this->makeFunction($id, isDeprecated: \true, deprecatedSinceVersion: '8.4')]);
        $result = (new FunctionDeprecationCheck($provider))->run($stubs, $id, '8.1');
        $this->assertTrue($result->hasFailures());
        $this->assertStringContainsString('not marked as deprecated in stubs', $result->getFailures()[$id]);
    }
    ...
    public function testKnownProblemSkipsValidation(): void
    {
        ...
        $knownProblemsProvider->method('getProblems')->willReturn([new ProblemDefinition(entityType: EntityType::FUNCTION, entityId: $id, type: ProblemType::INTERNAL_IMPLEMENTATION, affectedChecks: [CheckType::DEPRECATION], versionRange: new PhpVersionRange(PhpVersions::EARLIEST, PhpVersions::LATEST), reason: 'Test skip reason')]);
        KnownProblemsRegistry::reset();
