...
class ClassAncestorNamesExtractorTest extends TestCase
{
    ...
    {
        $parent = $this->makeClass('Masjid_App\Dependencies\Random\RandomError', 'RandomError');
        $class = new PHPClass();
        $class->setParentClass($parent);
        $this->assertSame(['Masjid_App\Dependencies\Random\RandomError'], $this->extractor->extract($class));
    }
    // ── Fallback to getName() when getId() is null ────────────────────────────
    public function testFallsBackToNameWhenIdIsNull(): void
    {
        $parent = new PHPClass();
        $parent->setName('BaseClass');
        // id not set → null
        $class = new PHPClass();
        $class->setParentClass($parent);
    ...
    {
        $root = $this->makeClass('Masjid_App\Dependencies\Random\RandomError', 'RandomError');
        $mid = $this->makeClass('Masjid_App\Dependencies\Random\BrokenRandomEngineError', 'BrokenRandomEngineError');
        $mid->setParentClass($root);
        $class = new PHPClass();
        $class->setParentClass($mid);
        $this->assertSame(['Masjid_App\Dependencies\Random\BrokenRandomEngineError', 'Masjid_App\Dependencies\Random\RandomError'], $this->extractor->extract($class));
    }
    // ── Cycle safety ──────────────────────────────────────────────────────────
    ...
    {
        $root = new PHPClass();
        // id=null, name=null — traversal must stop here
        $parent = $this->makeClass('\Exception');
