...
class AdaptedReflectionTypeTest extends TestCase
{
    ...
    {
        $typeMock = $this->getMockBuilder(ReflectionNamedType::class)->disableOriginalConstructor()->getMock();
        $typeMock->method('isBuiltin')->willReturn(\true);
        $adapted = new AdaptedReflectionType($typeMock);
    ...
    public function testItStoresTypesKeyCorrectly()
    {
        ...
        $innerTypeMock1 = $this->getMockBuilder(ReflectionNamedType::class)->disableOriginalConstructor()->getMock();
        $innerTypeMock1->method('getName')->willReturn('string');
        $innerTypeMock2 = $this->getMockBuilder(ReflectionNamedType::class)->disableOriginalConstructor()->getMock();
        $innerTypeMock2->method('getName')->willReturn('int');
        // Mock union type (PHP 8.0+)
        if (class_exists('ReflectionUnionType')) {
            $unionTypeMock = $this->getMockBuilder(\ReflectionUnionType::class)->disableOriginalConstructor()->getMock();
    ...
    {
        $innerTypeMock = $this->getMockBuilder(ReflectionNamedType::class)->disableOriginalConstructor()->getMock();
        ...
        if (class_exists('ReflectionUnionType')) {
            ...
            self::assertInstanceOf(\Masjid_App\Dependencies\StubTests\Framework\Parsers\Reflection\Wrappers\AdaptedReflectionNamedType::class, $types[0]);
