...
class ClassConstantsValueCheckTest extends CheckTestCase
{
    ...
    public function testClassLevelKnownProblemSkipsEntireCheck(): void
    {
        ...
        $knownProblemsProvider->method('getProblems')->willReturn([new ProblemDefinition(entityType: EntityType::CLASS_TYPE, entityId: $classId, type: ProblemType::INTERNAL_IMPLEMENTATION, affectedChecks: [CheckType::CLASS_CONSTANTS_VALUE], versionRange: new PhpVersionRange(PhpVersions::EARLIEST, PhpVersions::LATEST), reason: 'Value skip reason')]);
        KnownProblemsRegistry::reset();
    ...
    {
        $classId = '\SpecialClass';
        $reflClass = $this->makeClass($classId, constants: [$this->makeClassConstant('STABLE', 10), $this->makeClassConstant('ICU_DEPENDENT', 1)]);
        $stubClass = $this->makeClass($classId, constants: [$this->makeClassConstant('STABLE', 10), $this->makeClassConstant('ICU_DEPENDENT', 99)]);
        $knownProblemsProvider = $this->createStub(\Masjid_App\Dependencies\StubTests\Framework\Validator\KnownProblems\KnownProblemsProvider::class);
        $knownProblemsProvider->method('getProblems')->willReturn([new ProblemDefinition(entityType: EntityType::CLASS_CONSTANT, entityId: $classId . '::ICU_DEPENDENT', type: ProblemType::INTERNAL_IMPLEMENTATION, affectedChecks: [CheckType::CLASS_CONSTANTS_VALUE], versionRange: new PhpVersionRange(PhpVersions::EARLIEST, PhpVersions::LATEST), reason: 'ICU-version-dependent value')]);
        KnownProblemsRegistry::reset();
        $registry = KnownProblemsRegistry::getInstance($knownProblemsProvider);
        $provider = $this->createMockReflectionProviderWithClasses([$reflClass]);
        $stubs = $this->createMockStorageManager();
