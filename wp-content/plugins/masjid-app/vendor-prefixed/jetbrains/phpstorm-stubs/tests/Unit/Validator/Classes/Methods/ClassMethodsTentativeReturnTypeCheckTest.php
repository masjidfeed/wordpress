...
class ClassMethodsTentativeReturnTypeCheckTest extends CheckTestCase
{
    ...
    {
        $className = '\MyClass';
        $reflClass = $this->createMockClassWithProperties($className, null, null, null, [$this->makeMethod('current', isTentative: \true)]);
        $stubClass = $this->createMockClassWithProperties($className, null, null, null, [$this->makeMethod('current', isTentative: \true)]);
        $provider = $this->createMockReflectionProvider([], [$reflClass]);
        $stubs = $this->createMockStorageManager();
        $stubs->method('getClasses')->willReturn([$stubClass]);
        $result = (new ClassMethodsTentativeReturnTypeCheck($provider))->run($stubs, $className, '8.1');
    ...
    public function testReflectionTentativeStubNotTentativeIsFailure(): void
    {
        ...
        $stubClass = $this->createMockClassWithProperties($className, null, null, null, [$this->makeMethod('current')]);
        // stubs: not tentative
        $provider = $this->createMockReflectionProvider([], [$reflClass]);
        $stubs = $this->createMockStorageManager();
        $stubs->method('getClasses')->willReturn([$stubClass]);
    ...
    public function testStubTentativeReflectionNotTentativeIsFailure(): void
    {
        ...
        $stubClass = $this->createMockClassWithProperties($className, null, null, null, [$this->makeMethod('doWork', isTentative: \true)]);
    ...
    public function testMultipleMethodsMismatchesAreAllReported(): void
    {
        ...
        $reflClass = $this->createMockClassWithProperties($className, null, null, null, [
            $this->makeMethod('ok'),
            // matches
            $this->makeMethod('current', isTentative: \true),
        ]);
        $stubClass = $this->createMockClassWithProperties($className, null, null, null, [$this->makeMethod('ok'), $this->makeMethod('current')]);
        $provider = $this->createMockReflectionProvider([], [$reflClass]);
        $stubs = $this->createMockStorageManager();
        $stubs->method('getClasses')->willReturn([$stubClass]);
        $result = (new ClassMethodsTentativeReturnTypeCheck($provider))->run($stubs, $className, '8.1');
    ...
    public function testTentativeMethodFromInterfaceMismatchIsReported(): void
    {
        ...
        $reflClass = $this->createMockClassWithProperties($className, null, null, null, [$this->makeMethod('current', isTentative: \true)]);
        // reflection: tentative
        $iface = $this->makeInterface('\Iterator', [$this->makeMethod('current')]);
    ...
    public function testTentativeMethodFromInterfaceMatchIsSuccess(): void
    {
        $className = '\SplFixedArray';
        $reflClass = $this->createMockClassWithProperties($className, null, null, null, [$this->makeMethod('current', isTentative: \true)]);
        $iface = $this->makeInterface('\Iterator', [$this->makeMethod('current', isTentative: \true)]);
        $stubClass = $this->createMockClassWithProperties($className, null, null, null, [], null, [$iface]);
        $provider = $this->createMockReflectionProvider([], [$reflClass]);
        $stubs = $this->createMockStorageManager();
        $stubs->method('getClasses')->willReturn([$stubClass]);
    ...
    public function testClassLevelKnownProblemSkipsAllMethods(): void
    {
        ...
        $knownProblemsProvider->method('getProblems')->willReturn([new ProblemDefinition(entityType: EntityType::CLASS_TYPE, entityId: $className, type: ProblemType::INTERNAL_IMPLEMENTATION, affectedChecks: [CheckType::TENTATIVE_RETURN_TYPE], versionRange: new PhpVersionRange(PhpVersions::PHP_8_1, PhpVersions::LATEST), reason: 'Class-level skip')]);
        KnownProblemsRegistry::reset();
        $registry = KnownProblemsRegistry::getInstance($knownProblemsProvider);
        $provider = $this->createMockReflectionProvider([], [$reflClass]);
        $stubs = $this->createMockStorageManager();
        $stubs->method('getClasses')->willReturn([$stubClass]);
        $result = (new ClassMethodsTentativeReturnTypeCheck($provider, $registry))->run($stubs, $className, '8.1');
        $this->assertFalse($result->hasFailures());
    ...
    public function testMethodLevelKnownProblemSkipsSpecificMethod(): void
    {
        ...
        $reflClass = $this->createMockClassWithProperties($className, null, null, null, [$this->makeMethod('ok'), $this->makeMethod('current', isTentative: \true)]);
        $stubClass = $this->createMockClassWithProperties($className, null, null, null, [$this->makeMethod('ok'), $this->makeMethod('current')]);
        $knownProblemsProvider = $this->createStub(\Masjid_App\Dependencies\StubTests\Framework\Validator\KnownProblems\KnownProblemsProvider::class);
        $knownProblemsProvider->method('getProblems')->willReturn([new ProblemDefinition(entityType: EntityType::METHOD, entityId: $mismatchedId, type: ProblemType::INTERNAL_IMPLEMENTATION, affectedChecks: [CheckType::TENTATIVE_RETURN_TYPE], versionRange: new PhpVersionRange(PhpVersions::PHP_8_1, PhpVersions::LATEST), reason: 'Method-level skip')]);
        KnownProblemsRegistry::reset();
        $registry = KnownProblemsRegistry::getInstance($knownProblemsProvider);
        $provider = $this->createMockReflectionProvider([], [$reflClass]);
        $stubs = $this->createMockStorageManager();
