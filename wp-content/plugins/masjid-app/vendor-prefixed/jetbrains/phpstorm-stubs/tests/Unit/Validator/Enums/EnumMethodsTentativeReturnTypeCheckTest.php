...
class EnumMethodsTentativeReturnTypeCheckTest extends CheckTestCase
{
    ...
    private function makeReflection(array $enums): \Masjid_App\Dependencies\StubTests\Framework\Validator\Contracts\ReflectionProviderInterface
    {
        $provider = $this->createStub(\Masjid_App\Dependencies\StubTests\Framework\Validator\Contracts\ReflectionProviderInterface::class);
    ...
    public function testEnumNotFoundInReflectionIsFailure(): void
    {
        ...
        $result = (new ClassMethodsTentativeReturnTypeCheck(reflectionProvider: $this->makeReflection([]), entityTypeConfig: EntityTypeConfig::forEnum()))->run($stubs, $enumId, '8.1');
    ...
    public function testEnumNotFoundInStubsIsFailure(): void
    {
        ...
        $result = (new ClassMethodsTentativeReturnTypeCheck(reflectionProvider: $this->makeReflection([$this->makeEnumWithMethod($enumId, 'cases', \false)]), entityTypeConfig: EntityTypeConfig::forEnum()))->run($stubs, $enumId, '8.1');
        $this->assertTrue($result->hasFailures());
    ...
    public function testMatchingTentativeFlagsSucceed(): void
    {
        $enumId = '\MyEnum';
        $stubs = $this->createMockStorageManager();
        $stubs->method('getEnums')->willReturn([$this->makeEnumWithMethod($enumId, 'current', \true)]);
        $result = (new ClassMethodsTentativeReturnTypeCheck(reflectionProvider: $this->makeReflection([$this->makeEnumWithMethod($enumId, 'current', \true)]), entityTypeConfig: EntityTypeConfig::forEnum()))->run($stubs, $enumId, '8.1');
