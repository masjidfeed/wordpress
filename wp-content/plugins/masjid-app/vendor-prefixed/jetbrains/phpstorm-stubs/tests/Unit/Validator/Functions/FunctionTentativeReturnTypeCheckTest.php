...
class FunctionTentativeReturnTypeCheckTest extends CheckTestCase
{
    ...
    {
        $id = '\missing_func';
        $provider = $this->createMockReflectionProvider([$this->makeFunction($id, isTentative: \true)]);
        $stubs = $this->createMockStorageManager();
    ...
    public function testBothTentativeIsSuccess(): void
    {
        ...
        $provider = $this->createMockReflectionProvider([$this->makeFunction($id, isTentative: \true)]);
        $stubs = $this->createMockStorageManager();
        $stubs->method('getFunctions')->willReturn([$this->makeFunction($id, isTentative: \true)]);
        $result = (new FunctionTentativeReturnTypeCheck($provider))->run($stubs, $id, '8.1');
        $this->assertFalse($result->hasFailures());
        $this->assertEquals(1, $result->getSuccessCount());
    }
    ...
    public function testKnownProblemSkipsValidation(): void
    {
        ...
        $knownProblemsProvider->method('getProblems')->willReturn([new ProblemDefinition(entityType: EntityType::FUNCTION, entityId: $id, type: ProblemType::INTERNAL_IMPLEMENTATION, affectedChecks: [CheckType::TENTATIVE_RETURN_TYPE], versionRange: new PhpVersionRange(PhpVersions::PHP_8_1, PhpVersions::LATEST), reason: 'Known tentative mismatch')]);
        KnownProblemsRegistry::reset();
