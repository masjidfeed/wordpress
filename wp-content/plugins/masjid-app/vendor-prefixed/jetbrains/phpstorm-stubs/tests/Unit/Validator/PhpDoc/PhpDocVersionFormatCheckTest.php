...
use Masjid_App\Dependencies\StubTests\Framework\Validator\PhpDoc\PhpDocVersionFormatCheck;
class PhpDocVersionFormatCheckTest extends PhpDocCheckTestCase
{
    ...
    {
        return ['@since 5.6' => ['since', '5.6'], '@since 7.0' => ['since', '7.0'], '@since 8.0' => ['since', '8.0'], '@since 8.4' => ['since', '8.4'], '@since 0.4' => ['since', '0.4'], '@since 1.2' => ['since', '1.2'], '@deprecated 5.4' => ['deprecated', '5.4'], '@removed 8.0' => ['removed', '8.0']];
    }
    ...
    {
        return ['@since 5.1.2' => ['since', '5.1.2', '5.1'], '@since 7.0.7' => ['since', '7.0.7', '7.0'], '@since 8.0.1' => ['since', '8.0.1', '8.0'], '@since 5.6.1' => ['since', '5.6.1', '5.6'], '@deprecated 5.1.3' => ['deprecated', '5.1.3', '5.1'], '@removed 8.1.2' => ['removed', '8.1.2', '8.1'], '@since 1.2.3' => ['since', '1.2.3', '1.2'], '@since 4.4.5.35' => ['since', '4.4.5.35', '4.4']];
    }
    ...
    public function testKnownProblemAtEntityLevelSkipsCheck(): void
    {
        ...
        $provider->method('getProblems')->willReturn([new ProblemDefinition(entityType: EntityType::CLASS_TYPE, entityId: '\TestClass', type: ProblemType::INTERNAL_IMPLEMENTATION, affectedChecks: [CheckType::PHPDOC_VERSION_FORMAT], versionRange: new PhpVersionRange(PhpVersions::EARLIEST, PhpVersions::LATEST), reason: 'Library uses its own versioning scheme with patch components')]);
        KnownProblemsRegistry::getInstance($provider);
        $check = new PhpDocVersionFormatCheck();
        // must be created AFTER the custom registry
        $phpDoc = "/**\n * @since 5.1.2\n */";
        $class = $this->makeClass('\TestClass', phpDoc: $phpDoc);
