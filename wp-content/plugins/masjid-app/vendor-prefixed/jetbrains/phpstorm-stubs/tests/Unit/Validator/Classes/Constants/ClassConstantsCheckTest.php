...
namespace Masjid_App\Dependencies\StubTests\Unit\Validator\Classes\Constants;
...
class ClassConstantsCheckTest extends CheckTestCase
{
    ...
    public function testConstantInReflectionOnlyPasses(): void
    {
        ...
        $reflClass = $this->makeClass($classId, constants: [$this->makeClassConstant('ATOM', 'Y-m-d\TH:i:sP')]);
        $stubClass = $this->makeClass($classId);
        // stub doesn't re-declare inherited constant
        $provider = $this->createMockReflectionProviderWithClasses([$reflClass]);
        $stubs = $this->createMockStorageManager();
        $stubs->method('getClasses')->willReturn([$stubClass]);
        $result = (new ClassConstantsCheck($provider))->run($stubs, $classId, '8.0');
    ...
    public function testClassLevelKnownProblemSkipsEntireCheck(): void
    {
        ...
        $knownProblemsProvider->method('getProblems')->willReturn([new ProblemDefinition(entityType: EntityType::CLASS_TYPE, entityId: $classId, type: ProblemType::INTERNAL_IMPLEMENTATION, affectedChecks: [CheckType::CLASS_CONSTANTS], versionRange: new PhpVersionRange(PhpVersions::EARLIEST, PhpVersions::LATEST), reason: 'Test skip reason')]);
        KnownProblemsRegistry::reset();
    ...
    public function testConstantLevelKnownProblemSkipsSpecificConstant(): void
    {
        ...
        $stubClass = $this->makeClass($classId, constants: [
            $this->makeClassConstant('SKIPPED', 'wrong'),
            // not in reflection, but has known problem
            $this->makeClassConstant('GOOD', 42),
        ]);
        $knownProblemsProvider = $this->createStub(\Masjid_App\Dependencies\StubTests\Framework\Validator\KnownProblems\KnownProblemsProvider::class);
        $knownProblemsProvider->method('getProblems')->willReturn([new ProblemDefinition(entityType: EntityType::CLASS_CONSTANT, entityId: $classId . '::SKIPPED', type: ProblemType::INTERNAL_IMPLEMENTATION, affectedChecks: [CheckType::CLASS_CONSTANTS], versionRange: new PhpVersionRange(PhpVersions::EARLIEST, PhpVersions::LATEST), reason: 'Constant skip reason')]);
        KnownProblemsRegistry::reset();
        $registry = KnownProblemsRegistry::getInstance($knownProblemsProvider);
        $provider = $this->createMockReflectionProviderWithClasses([$reflClass]);
        $stubs = $this->createMockStorageManager();
    ...
    public function testConstantLevelKnownProblemViaEntityIdsList(): void
    {
        ...
        $reflClass = $this->makeClass($classId, constants: [$this->makeClassConstant('GOOD', 42)]);
        $stubClass = $this->makeClass($classId, constants: [
            $this->makeClassConstant('DRIVER_A', 1),
            // not in reflection → known problem via entityIds
            $this->makeClassConstant('DRIVER_B', 2),
            // not in reflection → known problem via entityIds
            $this->makeClassConstant('GOOD', 42),
        ]);
        $knownProblemsProvider = $this->createStub(\Masjid_App\Dependencies\StubTests\Framework\Validator\KnownProblems\KnownProblemsProvider::class);
        $knownProblemsProvider->method('getProblems')->willReturn([new ProblemDefinition(entityType: EntityType::CLASS_CONSTANT, entityId: $classId, type: ProblemType::INTERNAL_IMPLEMENTATION, affectedChecks: [CheckType::CLASS_CONSTANTS], versionRange: new PhpVersionRange(PhpVersions::EARLIEST, PhpVersions::LATEST), reason: 'Driver constants skip', entityIds: [$classId . '::DRIVER_A', $classId . '::DRIVER_B'])]);
        KnownProblemsRegistry::reset();
        $registry = KnownProblemsRegistry::getInstance($knownProblemsProvider);
        $provider = $this->createMockReflectionProviderWithClasses([$reflClass]);
        $stubs = $this->createMockStorageManager();
    ...
    public function testNonSkippedConstantStillFailsWhenOthersAreSkipped(): void
    {
        ...
        $stubClass = $this->makeClass($classId, constants: [
            $this->makeClassConstant('DRIVER_A', 1),
            // skipped via known problem
            $this->makeClassConstant('SPURIOUS', 99),
        ]);
        $knownProblemsProvider = $this->createStub(\Masjid_App\Dependencies\StubTests\Framework\Validator\KnownProblems\KnownProblemsProvider::class);
        $knownProblemsProvider->method('getProblems')->willReturn([new ProblemDefinition(entityType: EntityType::CLASS_CONSTANT, entityId: $classId . '::DRIVER_A', type: ProblemType::INTERNAL_IMPLEMENTATION, affectedChecks: [CheckType::CLASS_CONSTANTS], versionRange: new PhpVersionRange(PhpVersions::EARLIEST, PhpVersions::LATEST), reason: 'Driver constant')]);
        KnownProblemsRegistry::reset();
        $registry = KnownProblemsRegistry::getInstance($knownProblemsProvider);
        $provider = $this->createMockReflectionProviderWithClasses([$reflClass]);
        $stubs = $this->createMockStorageManager();
