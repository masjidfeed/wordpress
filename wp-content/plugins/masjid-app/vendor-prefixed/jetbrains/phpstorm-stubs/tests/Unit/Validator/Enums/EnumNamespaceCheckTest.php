...
class EnumNamespaceCheckTest extends CheckTestCase
{
    ...
    {
        $enum = $this->makeEnum('Masjid_App\Dependencies\Dom\AdjacentPosition', namespace: '\Dom');
        $stubs = $this->createMockStorageManager();
        $stubs->method('getEnums')->willReturn([$enum]);
        $result = $this->check->run($stubs, 'Masjid_App\Dependencies\Dom\AdjacentPosition', '8.4');
        $this->assertFalse($result->hasFailures());
    }
    public function testNamespacedEnumFailsWhenNamespaceMismatch(): void
    {
        $enum = $this->makeEnum('Masjid_App\Dependencies\Dom\AdjacentPosition', namespace: '\WrongNs');
        $stubs = $this->createMockStorageManager();
        $stubs->method('getEnums')->willReturn([$enum]);
        $result = $this->check->run($stubs, 'Masjid_App\Dependencies\Dom\AdjacentPosition', '8.4');
