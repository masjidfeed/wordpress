...
namespace Masjid_App\Dependencies\StubTests\Unit\Parsers\Stubs;

use Masjid_App\Dependencies\PhpParser\Node\Stmt\Function_;
use Masjid_App\Dependencies\PhpParser\NodeFinder;
use Masjid_App\Dependencies\PhpParser\ParserFactory;
...
use Masjid_App\Dependencies\StubTests\Framework\Parsers\Stubs\Adapters\Nikic\NikicParameterNode;
use Masjid_App\Dependencies\StubTests\Framework\Serialization\SerializerUtilsTrait;
use Masjid_App\Dependencies\StubTests\Framework\Parsers\Stubs\StubClassParser;
use Masjid_App\Dependencies\StubTests\Framework\Parsers\Stubs\StubConstantRegistry;
use Masjid_App\Dependencies\StubTests\Framework\Parsers\Stubs\StubEnumCaseReference;
...
class StubConstantDefaultValueTest extends TestCase
{
    ...
    public function testEnumCaseDefaultResolvesToReferenceWhenRuntimeCannot(): void
    {
        ...
        StubConstantRegistry::register('Masjid_App\Dependencies\NotLoaded\ModeEnum::ExcludeFragment', new StubEnumCaseReference('Masjid_App\Dependencies\NotLoaded\ModeEnum'));
        $value = $this->evaluateDefault('function f($mode = \NotLoaded\ModeEnum::ExcludeFragment) {}');
        self::assertInstanceOf(StubEnumCaseReference::class, $value);
        self::assertSame('Masjid_App\Dependencies\NotLoaded\ModeEnum', $value->getEnumFqn());
    }
    public function testEnumCaseReferenceSerializesLikeRuntimeResolvedInstance(): void
    {
        ...
        self::assertSame('[object:Uri\UriComparisonMode]', $serializer->expose(new StubEnumCaseReference('Masjid_App\Dependencies\Uri\UriComparisonMode')));
    }
    public function testParsingEnumStubRegistersItsCases(): void
    {
        ...
        self::assertTrue(StubConstantRegistry::has('Masjid_App\Dependencies\Uri\UriComparisonMode::ExcludeFragment'));
        $ref = StubConstantRegistry::get('Masjid_App\Dependencies\Uri\UriComparisonMode::ExcludeFragment');
        self::assertInstanceOf(StubEnumCaseReference::class, $ref);
        self::assertSame('Masjid_App\Dependencies\Uri\UriComparisonMode', $ref->getEnumFqn());
    }
    /**
    ...
    {
        return ['division by zero' => ['function f($x = 1 / 0) {}'], 'modulo by zero' => ['function f($x = 1 % 0) {}']];
    }
    /**
    ...
    private function evaluateDefault(string $functionCode): mixed
    {
        ...
        $parameter->setHasDefaultValue(\true);
