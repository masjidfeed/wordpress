...
class ClassMethodsParameterNamesCheckTest extends CheckTestCase
{
    ...
    public function testParameterNameMismatchIsFailure(): void
    {
        ...
        $stubParams = [$this->makeParam('fn')];
        // wrong name
        $reflClass = $this->createMockClassWithProperties($className, null, null, null, [$this->createMockMethod('invoke', $reflParams)]);
        $stubClass = $this->createMockClassWithProperties($className, null, null, null, [$this->createMockMethod('invoke', $stubParams)]);
        $provider = $this->createMockReflectionProvider([], [$reflClass]);
        $stubs = $this->createMockStorageManager();
        $stubs->method('getClasses')->willReturn([$stubClass]);
        $result = (new ClassMethodsParameterNamesCheck($provider))->run($stubs, $className, '8.0');
    ...
    public function testMultipleNameMismatchesReportedTogether(): void
    {
        ...
        $stubParams = [$this->makeParam('x'), $this->makeParam('y')];
        // both wrong
    ...
    public function testVersionFilteredParamIsExcluded(): void
    {
        ...
        $stubParams = [$this->makeParam('value'), $this->makeParam('extra', sinceVersion: '8.1')];
        $reflClass = $this->createMockClassWithProperties($className, null, null, null, [$this->createMockMethod('doWork', $reflParams)]);
        $stubClass = $this->createMockClassWithProperties($className, null, null, null, [$this->createMockMethod('doWork', $stubParams)]);
        $provider = $this->createMockReflectionProvider([], [$reflClass]);
        $stubs = $this->createMockStorageManager();
    ...
    public function testSameNamedVariadicDeduplication(): void
    {
        ...
        $className = '\MyClass';
        $reflParams = [$this->makeParam('values')];
        // single param in reflection
        $stubParams = [
            $this->makeParam('values', removedVersion: '7.4'),
            // removed after 7.4
            $this->makeParam('values', variadic: \true),
        ];
    ...
    public function testNameMismatchInheritedFromParentIsChecked(): void
    {
        ...
        $reflParams = [$this->makeParam('value')];
        $stubParams = [$this->makeParam('wrongName')];
        // wrong name in parent stub
    ...
    public function testClassLevelKnownProblemSkipsAllMethods(): void
    {
        ...
        $stubParams = [$this->makeParam('wrong')];
        // mismatch
        $reflClass = $this->createMockClassWithProperties($className, null, null, null, [$this->createMockMethod('doWork', $reflParams)]);
        $stubClass = $this->createMockClassWithProperties($className, null, null, null, [$this->createMockMethod('doWork', $stubParams)]);
        ...
        $knownProblemsProvider->method('getProblems')->willReturn([new ProblemDefinition(entityType: EntityType::CLASS_TYPE, entityId: $className, type: ProblemType::OVERLOADED_SIGNATURE, affectedChecks: [CheckType::PARAMETER_NAMES], versionRange: new PhpVersionRange(PhpVersions::PHP_8_0, PhpVersions::LATEST), reason: 'Class-level param names skip')]);
        KnownProblemsRegistry::reset();
    ...
    public function testMethodLevelKnownProblemSkipsSpecificMismatch(): void
    {
        ...
        $knownProblemsProvider = $this->createStub(\Masjid_App\Dependencies\StubTests\Framework\Validator\KnownProblems\KnownProblemsProvider::class);
        $knownProblemsProvider->method('getProblems')->willReturn([new ProblemDefinition(entityType: EntityType::METHOD, entityId: $mismatchedId, type: ProblemType::OVERLOADED_SIGNATURE, affectedChecks: [CheckType::PARAMETER_NAMES], versionRange: new PhpVersionRange(PhpVersions::PHP_8_0, PhpVersions::LATEST), reason: 'Method-level param names skip')]);
        KnownProblemsRegistry::reset();
        $registry = KnownProblemsRegistry::getInstance($knownProblemsProvider);
        $provider = $this->createMockReflectionProvider([], [$reflClass]);
        $stubs = $this->createMockStorageManager();
