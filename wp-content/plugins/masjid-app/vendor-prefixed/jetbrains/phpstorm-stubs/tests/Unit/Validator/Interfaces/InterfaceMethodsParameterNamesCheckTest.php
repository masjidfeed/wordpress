...
class InterfaceMethodsParameterNamesCheckTest extends CheckTestCase
{
    ...
    public function testParameterNameMismatchFails(): void
    {
        ...
        $reflIface = $this->makeInterface($interfaceId, [$this->makeMethod('offsetGet', parameters: [$this->makeParam('offset')])]);
        $stubIface = $this->makeInterface($interfaceId, [$this->makeMethod('offsetGet', parameters: [$this->makeParam('key')])]);
        $provider = $this->createMockReflectionProviderWithInterfaces([$reflIface]);
        $stubs = $this->createMockStorageManager();
        $stubs->method('getInterfaces')->willReturn([$stubIface]);
        $result = (new ClassMethodsParameterNamesCheck(reflectionProvider: $provider, entityTypeConfig: EntityTypeConfig::forInterface()))->run($stubs, $interfaceId, '8.0');
    ...
    public function testParameterNamesFromParentInterfaceAreChecked(): void
    {
        ...
        $reflIface = $this->makeInterface($childId, [$this->makeMethod('doWork', parameters: [$this->makeParam('value')])]);
        $parentStub = $this->makeInterface('\ParentInterface', [$this->makeMethod('doWork', parameters: [$this->makeParam('value')])]);
    ...
    public function testInterfaceLevelKnownProblemSkipsAllMethods(): void
    {
        ...
        $reflIface = $this->makeInterface($interfaceId, [$this->makeMethod('doWork', parameters: [$this->makeParam('correct')])]);
        $stubIface = $this->makeInterface($interfaceId, [$this->makeMethod('doWork', parameters: [$this->makeParam('wrong')])]);
