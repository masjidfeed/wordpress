...
namespace Masjid_App\Dependencies\StubTests\Unit\Parsers\Storage\Managers;
...
class ParsedDataJsonStorageManagerTest extends TestCase
{
    ...
    public function testManagerCanWriteAndReadReflectionDataToJson()
    {
        ...
        self::assertEquals('Masjid_App\Dependencies\Test\Namespace', $loadedClass->getNamespace());
    ...
    public function testJsonRoundTripPreservesEntityData()
    {
        ...
        $originalClass->setNamespace('Masjid_App\Dependencies\Complete\Namespace');
        $originalClass->setId('complete_id');
        $originalClass->setIsFinal(\true);
    ...
    public function testHasClassMethodWorksWithJsonStorage()
    {
        ...
        $class->setId('Masjid_App\Dependencies\Search\SearchableClass');
        $manager->addClass($class);
        $manager->save();
        // Load and test hasClass
        $newJsonStorage = new JsonParsedDataStorage($jsonFilePath, new StubsEntitySerializer());
        $newManager = new DefaultParsedDataStorageManager($newJsonStorage);
        self::assertTrue($newManager->hasClass('Masjid_App\Dependencies\Search\SearchableClass'));
        self::assertFalse($newManager->hasClass('Masjid_App\Dependencies\NonExistent\NonExistentClass'));
    }
    ...
    public function testSaveFailsLoudlyOnAValueJsonCannotRepresent(): void
    {
        ...
        $class = new PHPClass();
        $class->setName("Broken\xb11Name");
        // invalid UTF-8, no JSON representation
        ...
        try {
            ...
            self::assertStringNotContainsString('"name": null', file_exists($jsonFilePath) ? file_get_contents($jsonFilePath) : '');
        }
    }
    public function testAddEntityStoresDuplicatesWithSameId(): void
    {
        ...
        $entity1->setId('Masjid_App\Dependencies\Couchbase\SearchFacet');
        $entity1->setName('SearchFacet');
        $entity2 = new PHPClass();
        $entity2->setId('Masjid_App\Dependencies\Couchbase\SearchFacet');
        // same ID, different object (from different directory)
