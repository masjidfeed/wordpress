...
class ClassMethodsOptionalParametersCheckTest extends CheckTestCase
{
    ...
    public function testMethodWithNoOptionalParamsSucceeds(): void
    {
        ...
        $reflClass = $this->createMockClassWithProperties($className, null, null, null, [$this->createMockMethod('doWork', $params)]);
        $stubClass = $this->createMockClassWithProperties($className, null, null, null, [$this->createMockMethod('doWork', $params)]);
        $provider = $this->createMockReflectionProvider([], [$reflClass]);
        $stubs = $this->createMockStorageManager();
        $stubs->method('getClasses')->willReturn([$stubClass]);
        $result = (new ClassMethodsOptionalParametersCheck($provider))->run($stubs, $className, '8.0');
    ...
    {
        $className = '\MyClass';
        $reflParams = [$this->makeParam('a'), $this->makeParam('b', optional: \true)];
        $stubParams = [$this->makeParam('a'), $this->makeParam('b', optional: \true)];
    ...
    public function testOptionalParamInReflectionButNotStubsIsFailure(): void
    {
        $className = '\MyClass';
        $reflParams = [$this->makeParam('a'), $this->makeParam('b', optional: \true)];
        $stubParams = [$this->makeParam('a'), $this->makeParam('b')];
        // not optional
        $reflClass = $this->createMockClassWithProperties($className, null, null, null, [$this->createMockMethod('compute', $reflParams)]);
        $stubClass = $this->createMockClassWithProperties($className, null, null, null, [$this->createMockMethod('compute', $stubParams)]);
        $provider = $this->createMockReflectionProvider([], [$reflClass]);
    ...
    public function testOptionalInStubsButNotReflectionIsSuccess(): void
    {
        ...
        $stubParams = [$this->makeParam('x', optional: \true)];
        // optional in stubs only
    ...
    public function testMethodMissingFromStubsIsNotReportedAsOptionalMismatch(): void
    {
        ...
        $className = '\MyClass';
        $reflParams = [$this->makeParam('b', optional: \true)];
    ...
    public function testVersionExcludedStubParamIsIgnored(): void
    {
        $className = '\MyClass';
        $reflParams = [$this->makeParam('a'), $this->makeParam('b', optional: \true)];
        $stubParams = [$this->makeParam('a'), $this->makeParam('b', sinceVersion: '8.1')];
        $reflClass = $this->createMockClassWithProperties($className, null, null, null, [$this->createMockMethod('doWork', $reflParams)]);
        $stubClass = $this->createMockClassWithProperties($className, null, null, null, [$this->createMockMethod('doWork', $stubParams)]);
        $provider = $this->createMockReflectionProvider([], [$reflClass]);
        $stubs = $this->createMockStorageManager();
    ...
    public function testOptionalParamMismatchInheritedFromParentIsChecked(): void
    {
        ...
        $reflParams = [$this->makeParam('x', optional: \true)];
        // optional in reflection
        $stubParams = [$this->makeParam('x')];
        // not optional in stubs
    ...
    public function testClassLevelKnownProblemSkipsAllMethods(): void
    {
        ...
        $knownProblemsProvider->method('getProblems')->willReturn([new ProblemDefinition(entityType: EntityType::CLASS_TYPE, entityId: $className, type: ProblemType::INTERNAL_IMPLEMENTATION, affectedChecks: [CheckType::OPTIONAL_PARAMETERS], versionRange: new PhpVersionRange(PhpVersions::EARLIEST, PhpVersions::LATEST), reason: 'Class-level optional params skip')]);
        KnownProblemsRegistry::reset();
    ...
    public function testMethodLevelKnownProblemSkipsSpecificMismatch(): void
    {
        ...
        $knownProblemsProvider = $this->createStub(\Masjid_App\Dependencies\StubTests\Framework\Validator\KnownProblems\KnownProblemsProvider::class);
        $knownProblemsProvider->method('getProblems')->willReturn([new ProblemDefinition(entityType: EntityType::METHOD, entityId: $mismatchedId, type: ProblemType::INTERNAL_IMPLEMENTATION, affectedChecks: [CheckType::OPTIONAL_PARAMETERS], versionRange: new PhpVersionRange(PhpVersions::EARLIEST, PhpVersions::LATEST), reason: 'Method-level optional params skip')]);
        KnownProblemsRegistry::reset();
        $registry = KnownProblemsRegistry::getInstance($knownProblemsProvider);
        $provider = $this->createMockReflectionProvider([], [$reflClass]);
        $stubs = $this->createMockStorageManager();
