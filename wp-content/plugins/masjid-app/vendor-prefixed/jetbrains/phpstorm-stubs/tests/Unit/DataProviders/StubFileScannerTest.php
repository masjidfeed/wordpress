...
use Masjid_App\Dependencies\StubTests\Framework\DataProvider\StubFileScanner;
class StubFileScannerTest extends TestCase
{
    ...
    {
        $found = StubFileScanner::collect($this->root, fn(string $path, string $name): bool => str_ends_with($name, '.php'), fn(string $path, string $name): bool => \true);
        self::assertSame(['ext1/a.php', 'ext2/sub/deep/b.php', 'meta/.phpstorm.meta.php', 'vendor/dep/c.php'], $this->relative($found));
    }
    ...
    {
        $found = StubFileScanner::collect($this->root, fn(string $path, string $name): bool => $name === '.phpstorm.meta.php', fn(string $path, string $name): bool => \true);
        self::assertSame(['meta/.phpstorm.meta.php'], $this->relative($found));
    }
    public function testMissingDirectoryYieldsNoFiles(): void
    {
        $found = StubFileScanner::collect($this->root . '/does-not-exist', fn(string $path, string $name): bool => \true, fn(string $path, string $name): bool => \true);
    ...
    private function relative(array $paths): array
    {
        ...
        $relative = array_map(fn(string $p) => ltrim(str_replace($normalizedRoot, '', str_replace('\\', '/', $p)), '/'), $paths);
        sort($relative);
        return $relative;
    }
}
