...
use Masjid_App\Dependencies\StubTests\Framework\Validator\Constants\ConstantValueCheck;
...
class ConstantValueCheckTest extends CheckTestCase
{
    ...
    public function testNonLatestVersionSkipsComparison(): void
    {
        ...
        $stubConstant = $this->makeGlobalConstant($constantId, 99);
        // different — would fail at LATEST
    ...
    public function testKnownProblemOnlySkipsForVersionRange(): void
    {
        ...
        // mismatch
        $knownProblemsProvider = $this->createStub(KnownProblemsProvider::class);
        $knownProblemsProvider->method('getProblems')->willReturn([new ProblemDefinition(entityType: EntityType::GLOBAL_CONSTANT, entityId: $constantId, type: ProblemType::INTERNAL_IMPLEMENTATION, affectedChecks: [CheckType::CONSTANT_VALUE], versionRange: new PhpVersionRange(PhpVersions::PHP_8_0, PhpVersions::PHP_8_3), reason: 'Only a known issue in 8.0-8.3')]);
        KnownProblemsRegistry::reset();
        $registry = KnownProblemsRegistry::getInstance($knownProblemsProvider);
        $provider = $this->createMockReflectionProviderWithConstants([$reflConstant]);
        $stubs = $this->createMockStorageManager();
