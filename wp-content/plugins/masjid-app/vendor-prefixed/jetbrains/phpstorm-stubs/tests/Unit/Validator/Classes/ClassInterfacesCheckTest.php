...
class ClassInterfacesCheckTest extends CheckTestCase
{
    ...
    public function testClassWithMultipleMatchingInterfaces(): void
    {
        ...
        $interfaces = [$this->makeLinkedInterface('\IteratorAggregate'), $this->makeLinkedInterface('\ArrayAccess'), $this->makeLinkedInterface('\Serializable'), $this->makeLinkedInterface('\Countable')];
        ...
        $stubInterfaces = [$this->makeLinkedInterface('\IteratorAggregate'), $this->makeLinkedInterface('\ArrayAccess'), $this->makeLinkedInterface('\Serializable'), $this->makeLinkedInterface('\Countable')];
    ...
    public function testKnownProblemSkipsValidation(): void
    {
        ...
        $knownProblemsProvider->method('getProblems')->willReturn([new ProblemDefinition(entityType: EntityType::CLASS_TYPE, entityId: $className, type: ProblemType::OVERLOADED_SIGNATURE, affectedChecks: [CheckType::CLASS_INTERFACES], versionRange: new PhpVersionRange(PhpVersions::EARLIEST, PhpVersions::LATEST), reason: 'Known interface issue')]);
        KnownProblemsRegistry::reset();
    ...
    public function testLinkedInterfacesFqnsUsedForComparison(): void
    {
        ...
        $className = 'Masjid_App\Dependencies\Random\Randomizer';
        $reflectionIface = new PHPInterface();
        $reflectionIface->setName('RandomEngine');
        // short name (as getName() returns after linking)
        $reflectionIface->setId('Masjid_App\Dependencies\Random\RandomEngine');
        // full id
        $reflectionClass = $this->createMockClassWithProperties($className, null, null, null, [], null, [$reflectionIface]);
        $stubIface = new PHPInterface();
        $stubIface->setName('RandomEngine');
        $stubIface->setId('Masjid_App\Dependencies\Random\RandomEngine');
        $stubClass = $this->createMockClassWithProperties($className, null, null, null, [], null, [$stubIface]);
        $reflectionProvider = $this->createMockReflectionProvider([], [$reflectionClass]);
        $stubsManager = $this->createMockStorageManager();
        $stubsManager->method('getClasses')->willReturn([$stubClass]);
        $check = new ClassInterfacesCheck($reflectionProvider);
