...
{
    // ── Helper ───────────────────────────────────────────────────────────────
    private function makeParam(string $name, ?string $sinceVersion = null, ?string $removedVersion = null, bool $isVariadic = \false): PHPParameter
    {
    ...
    public function testMixedVersionFilteringKeepsOnlyAvailable(): void
    {
        ...
        $result = ParameterFilterHelper::filterAndDeduplicate([$oldParam, $newParam, $alwaysParam], '8.0');
    ...
    public function testMergesConsecutiveSameNamedParamsWhereSecondIsVariadic(): void
    {
        ...
        $variadic = $this->makeParam('values', isVariadic: \true);
        $result = ParameterFilterHelper::filterAndDeduplicate([$plain, $variadic], PhpVersions::LATEST->value);
    ...
    public function testDoesNotMergeNonVariadicSameNamedParams(): void
    {
        ...
        $second = $this->makeParam('dup');
        // not variadic
        $result = ParameterFilterHelper::filterAndDeduplicate([$first, $second], PhpVersions::LATEST->value);
    ...
    public function testDoesNotMergeVariadicWhenNamesAreDifferent(): void
    {
        ...
        $variadic = $this->makeParam('beta', isVariadic: \true);
        $result = ParameterFilterHelper::filterAndDeduplicate([$first, $variadic], PhpVersions::LATEST->value);
        $this->assertCount(2, $result);
    ...
    public function testFilteringAndDeduplicationWorkTogether(): void
    {
        ...
        $variadic = $this->makeParam('data', sinceVersion: '8.0', isVariadic: \true);
        $result = ParameterFilterHelper::filterAndDeduplicate([$placeholder, $variadic], '8.4');
        $this->assertCount(1, $result);
        $this->assertTrue($result[0]->isVariadic());
    }
    public function testDeduplicationAfterFilteringMergesCorrectly(): void
    {
        ...
        $variadic = $this->makeParam('args', sinceVersion: '5.6', isVariadic: \true);
        $result = ParameterFilterHelper::filterAndDeduplicate([$plain, $variadic], '7.0');
        $this->assertCount(1, $result);
        $this->assertTrue($result[0]->isVariadic());
    }
}
