...
use Masjid_App\Dependencies\StubTests\Framework\Validator\PhpDoc\PhpDocLinksCheck;
class PhpDocLinksCheckTest extends PhpDocCheckTestCase
{
    ...
    {
        return new PhpDocLinksCheck(reflectionProvider: null, knownProblemsRegistry: null, urlFetcher: $urlFetcher, checkLiveness: $checkLiveness);
    }
    private function runCheck(PhpDocLinksCheck $check, \Masjid_App\Dependencies\StubTests\Framework\Storage\StubDataQueryInterface $stubs, string $entityId): \Masjid_App\Dependencies\StubTests\Framework\Validator\Contracts\CheckResultSet
    {
    ...
    public function testHttpLinkIsFailure(): void
    {
        ...
        $this->assertStringContainsString('must use https', implode(' ', $result->getFailures()), 'Failure message should mention the https requirement');
    }
    ...
    public function testLivenessCheckPassesFor200(): void
    {
        ...
        $phpDoc = "/**\n * @link https://php.net/example\n */";
        $class = $this->makeClass('\TestClass', phpDoc: $phpDoc);
        $check = new PhpDocLinksCheck(null, null, $fetcher, checkLiveness: \true);
        $result = $check->run($this->makeStubsWithClass($class), '\TestClass', PhpVersions::LATEST->value);
    ...
    {
        $fetcher = fn(string $url): int => 0;
        // 0 = curl connection failure
        $phpDoc = "/**\n * @link https://php.net/example\n */";
        $class = $this->makeClass('\TestClass', phpDoc: $phpDoc);
        $check = new PhpDocLinksCheck(null, null, $fetcher, checkLiveness: \true);
        $result = $check->run($this->makeStubsWithClass($class), '\TestClass', PhpVersions::LATEST->value);
    ...
    public function testKnownProblemAtEntityLevelSkipsCheck(): void
    {
        ...
        $provider->method('getProblems')->willReturn([new ProblemDefinition(entityType: EntityType::CLASS_TYPE, entityId: '\TestClass', type: ProblemType::INTERNAL_IMPLEMENTATION, affectedChecks: [CheckType::PHPDOC_LINKS], versionRange: new PhpVersionRange(PhpVersions::EARLIEST, PhpVersions::LATEST), reason: 'Known dead link in legacy phpDoc')]);
