...
class InterfaceMethodsTentativeReturnTypeCheckTest extends CheckTestCase
{
    ...
    public function testInterfaceNotFoundInReflectionIsFailure(): void
    {
        ...
        $result = (new ClassMethodsTentativeReturnTypeCheck(reflectionProvider: $this->createMockReflectionProviderWithInterfaces([]), entityTypeConfig: EntityTypeConfig::forInterface()))->run($stubs, $ifaceId, '8.1');
    ...
    public function testInterfaceNotFoundInStubsIsFailure(): void
    {
        ...
        $result = (new ClassMethodsTentativeReturnTypeCheck(reflectionProvider: $this->createMockReflectionProviderWithInterfaces([$this->makeInterface($ifaceId, [$this->makeMethod('current', isTentative: \true)])]), entityTypeConfig: EntityTypeConfig::forInterface()))->run($stubs, $ifaceId, '8.1');
        $this->assertTrue($result->hasFailures());
    ...
    public function testMatchingTentativeFlagsSucceed(): void
    {
        $ifaceId = '\Iterator';
        $stubs = $this->createMockStorageManager();
        $stubs->method('getInterfaces')->willReturn([$this->makeInterface($ifaceId, [$this->makeMethod('current', isTentative: \true)])]);
        $result = (new ClassMethodsTentativeReturnTypeCheck(reflectionProvider: $this->createMockReflectionProviderWithInterfaces([$this->makeInterface($ifaceId, [$this->makeMethod('current', isTentative: \true)])]), entityTypeConfig: EntityTypeConfig::forInterface()))->run($stubs, $ifaceId, '8.1');
    ...
    public function testParentInterfaceTentativeMethodMismatchIsReported(): void
    {
        ...
        $reflChild = $this->makeInterface($childId, [$this->makeMethod('current', isTentative: \true)]);
        ...
        $result = (new ClassMethodsTentativeReturnTypeCheck(reflectionProvider: $this->createMockReflectionProviderWithInterfaces([$reflChild]), entityTypeConfig: EntityTypeConfig::forInterface()))->run($stubs, $childId, '8.1');
        $this->assertTrue($result->hasFailures());
