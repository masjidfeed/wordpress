...
use Masjid_App\Dependencies\StubTests\Framework\Parsers\Types\TypeStringParser;
/**
...
class TypeStringParserTest extends TestCase
{
    ...
    {
        return ['empty string is NoType' => ['', NoType::class, ''], 'plain name' => ['Foo', StandaloneType::class, 'Foo'], 'builtin scalar' => ['int', StandaloneType::class, 'int'], 'nullable shorthand' => ['?Foo', NullableType::class, 'Foo|null'], 'parenthesised intersection' => ['(A&B)', IntersectionType::class, 'A&B'], 'bare intersection' => ['A&B', IntersectionType::class, 'A&B'], 'three-way intersection' => ['A&B&C', IntersectionType::class, 'A&B&C'], 'union of two' => ['A|B', UnionType::class, 'A|B'], 'union of three' => ['A|B|C', UnionType::class, 'A|B|C'], 'type|null is nullable' => ['Foo|null', NullableType::class, 'Foo|null'], 'null|type is nullable regardless of order' => ['null|Foo', NullableType::class, 'Foo|null']];
    }
    /**
    ...
    {
        return ['standalone' => ['Foo', '<Foo>'], 'nullable shorthand' => ['?Foo', '<Foo>|null'], 'union members' => ['A|B', '<A>|<B>'], 'intersection members' => ['A&B', '<A>&<B>'], 'nullable via null union' => ['Foo|null', '<Foo>|null'], 'dnf leaves' => ['int|(A&B)', '<int>|(<A>&<B>)']];
    }
    /**
    ...
    public function testUnionSplittingRespectsParenthesisDepth()
    {
        ...
        self::assertInstanceOf(UnionType::class, $result);
        self::assertFalse($result->containsTypes('(A'), 'the group was split mid-parenthesis, so depth tracking was ignored');
    ...
    {
        return [['Foo'], ['int'], ['?Foo'], ['Foo|null'], ['A|B'], ['A|B|C'], ['A&B'], ['(A&B)'], ['int|(A&B)'], ['(A&B)|(C&D)'], ['(A&B)|null'], ['int|(A&B)|null']];
    }
}
