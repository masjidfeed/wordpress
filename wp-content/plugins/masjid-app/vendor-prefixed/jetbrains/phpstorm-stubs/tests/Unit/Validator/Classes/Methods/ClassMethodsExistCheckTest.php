...
use Masjid_App\Dependencies\StubTests\Framework\Validator\Classes\Methods\ClassMethodsExistCheck;
...
class ClassMethodsExistCheckTest extends CheckTestCase
{
    ...
    {
        $className = '\MyClass';
        $reflectionMethods = [$this->createMockMethod('method1'), $this->createMockMethod('method2'), $this->createMockMethod('__construct')];
        $reflectionClass = $this->createMockClassWithProperties($className, null, null, null, $reflectionMethods);
        $stubMethods = [$this->makeMethod('method1'), $this->makeMethod('method2'), $this->makeMethod('__construct')];
    ...
    {
        $className = '\MyClass';
        $reflectionMethods = [$this->createMockMethod('existingMethod'), $this->createMockMethod('missingMethod')];
        $reflectionClass = $this->createMockClassWithProperties($className, null, null, null, $reflectionMethods);
        $stubMethods = [$this->makeMethod('existingMethod')];
        $stubClass = $this->createMockClassWithProperties($className, null, null, null, $stubMethods);
        $reflectionProvider = $this->createMockReflectionProvider([], [$reflectionClass]);
        $stubsManager = $this->createMockStorageManager();
        $stubsManager->method('getClasses')->willReturn([$stubClass]);
    ...
    public function testMethodNotYetAvailableIsNotCountedInStubs(): void
    {
        ...
        $className = '\MyClass';
        $reflectionMethods = [$this->createMockMethod('oldMethod')];
        $reflectionClass = $this->createMockClassWithProperties($className, null, null, null, $reflectionMethods);
        $stubMethods = [$this->makeMethod('oldMethod'), $this->makeMethod('newMethod', sinceVersion: '8.1')];
        $stubClass = $this->createMockClassWithProperties($className, null, null, null, $stubMethods);
        $reflectionProvider = $this->createMockReflectionProvider([], [$reflectionClass]);
        $stubsManager = $this->createMockStorageManager();
        $stubsManager->method('getClasses')->willReturn([$stubClass]);
    ...
    public function testRemovedMethodIsNotCountedInStubs(): void
    {
        ...
        $className = '\MyClass';
        $reflectionMethods = [$this->createMockMethod('currentMethod')];
        $reflectionClass = $this->createMockClassWithProperties($className, null, null, null, $reflectionMethods);
        $stubMethods = [$this->makeMethod('currentMethod'), $this->makeMethod('legacyMethod', removedVersion: '7.4')];
        $stubClass = $this->createMockClassWithProperties($className, null, null, null, $stubMethods);
        $reflectionProvider = $this->createMockReflectionProvider([], [$reflectionClass]);
        $stubsManager = $this->createMockStorageManager();
        $stubsManager->method('getClasses')->willReturn([$stubClass]);
    ...
    public function testMethodInheritedFromParentClassIsCounted(): void
    {
        ...
        $reflectionMethods = [$this->createMockMethod('childMethod'), $this->createMockMethod('parentMethod')];
        $reflectionClass = $this->createMockClassWithProperties($className, null, null, null, $reflectionMethods);
        // Stub: child only has childMethod; parent has parentMethod
        $parentStubMethods = [$this->makeMethod('parentMethod')];
        $parentStubClass = $this->getMockBuilder(PHPClass::class)->disableOriginalConstructor()->onlyMethods(['getId', 'getName', 'getNamespace', 'getMethods'])->getMock();
        ...
        $childStubClass = $this->getMockBuilder(PHPClass::class)->disableOriginalConstructor()->onlyMethods(['getId', 'getName', 'getNamespace', 'getMethods'])->getMock();
    ...
    public function testPsUnreservePrefixMethodMatchesRealName(): void
    {
        ...
        $className = '\Generator';
        $reflectionMethods = [$this->createMockMethod('throw')];
        $reflectionClass = $this->createMockClassWithProperties($className, null, null, null, $reflectionMethods);
        $stubMethods = [$this->makeMethod('PS_UNRESERVE_PREFIX_throw')];
        $stubClass = $this->createMockClassWithProperties($className, null, null, null, $stubMethods);
        $reflectionProvider = $this->createMockReflectionProvider([], [$reflectionClass]);
        $stubsManager = $this->createMockStorageManager();
        $stubsManager->method('getClasses')->willReturn([$stubClass]);
    ...
    public function testClassLevelKnownProblemSkipsAllMethodChecks(): void
    {
        ...
        $reflectionClass = $this->createMockClassWithProperties($className, null, null, null, $reflectionMethods);
        $stubClass = $this->createMockClassWithProperties($className);
        // no methods, but class-level skip
        ...
        $knownProblemsProvider->method('getProblems')->willReturn([new ProblemDefinition(entityType: EntityType::CLASS_TYPE, entityId: $className, type: ProblemType::INTERNAL_IMPLEMENTATION, affectedChecks: [CheckType::CLASS_METHODS_EXIST], versionRange: new PhpVersionRange(PhpVersions::EARLIEST, PhpVersions::LATEST), reason: 'Class-level skip reason')]);
        KnownProblemsRegistry::reset();
    ...
    public function testMethodLevelKnownProblemSkipsSpecificMethod(): void
    {
        ...
        $reflectionMethods = [$this->createMockMethod('presentMethod'), $this->createMockMethod('missingMethod')];
        $reflectionClass = $this->createMockClassWithProperties($className, null, null, null, $reflectionMethods);
        $stubMethods = [$this->makeMethod('presentMethod')];
        $stubClass = $this->createMockClassWithProperties($className, null, null, null, $stubMethods);
        $knownProblemsProvider = $this->createStub(\Masjid_App\Dependencies\StubTests\Framework\Validator\KnownProblems\KnownProblemsProvider::class);
        $knownProblemsProvider->method('getProblems')->willReturn([new ProblemDefinition(entityType: EntityType::METHOD, entityId: $missingMethodId, type: ProblemType::INTERNAL_IMPLEMENTATION, affectedChecks: [CheckType::CLASS_METHODS_EXIST], versionRange: new PhpVersionRange(PhpVersions::EARLIEST, PhpVersions::LATEST), reason: 'Method-level skip reason')]);
        KnownProblemsRegistry::reset();
        $registry = KnownProblemsRegistry::getInstance($knownProblemsProvider);
        $reflectionProvider = $this->createMockReflectionProvider([], [$reflectionClass]);
        $stubsManager = $this->createMockStorageManager();
    ...
    public function testVersionBoundarySinceVersionEqualsPhpVersion(): void
    {
        ...
        $reflectionClass = $this->createMockClassWithProperties($className, null, null, null, $reflectionMethods);
        $stubMethods = [$this->makeMethod('justAddedMethod', sinceVersion: '8.0')];
        // since exactly 8.0
        $stubClass = $this->createMockClassWithProperties($className, null, null, null, $stubMethods);
        $reflectionProvider = $this->createMockReflectionProvider([], [$reflectionClass]);
        $stubsManager = $this->createMockStorageManager();
    ...
    public function testVersionBoundaryRemovedVersionEqualsPhpVersion(): void
    {
        ...
        $reflectionClass = $this->createMockClassWithProperties($className, null, null, null, $reflectionMethods);
        $stubMethods = [$this->makeMethod('lastVersionMethod', removedVersion: '7.5')];
        // excluded from 7.5+, so available in 7.4
        $stubClass = $this->createMockClassWithProperties($className, null, null, null, $stubMethods);
        $reflectionProvider = $this->createMockReflectionProvider([], [$reflectionClass]);
        $stubsManager = $this->createMockStorageManager();
        $stubsManager->method('getClasses')->willReturn([$stubClass]);
    ...
    public function testMethodInheritedFromInterfaceIsCounted(): void
    {
        ...
        $className = '\MyClass';
        $reflectionMethods = [$this->createMockMethod('classMethod'), $this->createMockMethod('interfaceMethod')];
        $reflectionClass = $this->createMockClassWithProperties($className, null, null, null, $reflectionMethods);
        $interface = $this->makeInterface('\MyInterface', [$this->makeMethod('interfaceMethod')]);
        $stubClass = $this->createMockClassWithProperties($className, null, null, null, [$this->makeMethod('classMethod')], null, [$interface]);
        $reflectionProvider = $this->createMockReflectionProvider([], [$reflectionClass]);
        $stubsManager = $this->createMockStorageManager();
        $stubsManager->method('getClasses')->willReturn([$stubClass]);
        $check = new ClassMethodsExistCheck($reflectionProvider);
    ...
    public function testPsUnreservePrefixInInterfaceMethodMatchesRealName(): void
    {
        ...
        $reflectionClass = $this->createMockClassWithProperties($className, null, null, null, $reflectionMethods);
        $interface = $this->makeInterface('\CalendarInterface', [$this->makeMethod('PS_UNRESERVE_PREFIX_isSet')]);
        $stubClass = $this->createMockClassWithProperties($className, null, null, null, [], null, [$interface]);
        $reflectionProvider = $this->createMockReflectionProvider([], [$reflectionClass]);
        $stubsManager = $this->createMockStorageManager();
        $stubsManager->method('getClasses')->willReturn([$stubClass]);
        $check = new ClassMethodsExistCheck($reflectionProvider);
    ...
    public function testMethodInheritedFromParentInterfaceChainIsCounted(): void
    {
        ...
        $stubClass = $this->createMockClassWithProperties($className, null, null, null, [], null, [$childIface]);
        $reflectionProvider = $this->createMockReflectionProvider([], [$reflectionClass]);
        $stubsManager = $this->createMockStorageManager();
        $stubsManager->method('getClasses')->willReturn([$stubClass]);
        $check = new ClassMethodsExistCheck($reflectionProvider);
    ...
    public function testDeepThreeLevelClassHierarchyGrandparentMethodIsCounted(): void
    {
        ...
        $className = '\ChildClass';
        $reflectionMethods = [$this->createMockMethod('childMethod'), $this->createMockMethod('parentMethod'), $this->createMockMethod('grandparentMethod')];
        $reflectionClass = $this->createMockClassWithProperties($className, null, null, null, $reflectionMethods);
        ...
        $stubClass = $this->createMockClassWithProperties($className, null, null, null, [$this->makeMethod('childMethod')]);
    ...
    public function testCycleInClassHierarchyDoesNotCauseInfiniteLoop(): void
    {
        ...
        $className = '\ClassA';
        $reflectionMethods = [$this->createMockMethod('methodA'), $this->createMockMethod('methodB')];
        $reflectionClass = $this->createMockClassWithProperties($className, null, null, null, $reflectionMethods);
        $classA = new PHPClass();
        $classA->setId('\ClassA');
    ...
    public function testCycleInInterfaceHierarchyDoesNotCauseInfiniteLoop(): void
    {
        ...
        $className = '\MyClass';
        $reflectionMethods = [$this->createMockMethod('method1'), $this->createMockMethod('method2')];
        $reflectionClass = $this->createMockClassWithProperties($className, null, null, null, $reflectionMethods);
        ...
        $iface2->addParentInterface($iface1);
        // cycle!
        $stubClass = $this->createMockClassWithProperties($className, null, null, null, [], null, [$iface1]);
        $reflectionProvider = $this->createMockReflectionProvider([], [$reflectionClass]);
        $stubsManager = $this->createMockStorageManager();
        $stubsManager->method('getClasses')->willReturn([$stubClass]);
        $check = new ClassMethodsExistCheck($reflectionProvider);
    ...
    {
        $className = '\MyClass';
        $reflectionMethods = [$this->createMockMethod('gammaMethod'), $this->createMockMethod('alphaMethod'), $this->createMockMethod('betaMethod')];
        $reflectionClass = $this->createMockClassWithProperties($className, null, null, null, $reflectionMethods);
        $stubClass = $this->createMockClassWithProperties($className);
        // no methods
        $reflectionProvider = $this->createMockReflectionProvider([], [$reflectionClass]);
    ...
    public function testMethodWithNullNameIsSkippedGracefully(): void
    {
        ...
        $reflectionClass = $this->createMockClassWithProperties($className, null, null, null, $reflectionMethods);
        $methodWithNullName = new PHPMethod();
        // name never set → getName() returns null
        $stubClass = $this->createMockClassWithProperties($className, null, null, null, [$this->makeMethod('realMethod'), $methodWithNullName]);
        $reflectionProvider = $this->createMockReflectionProvider([], [$reflectionClass]);
        $stubsManager = $this->createMockStorageManager();
        $stubsManager->method('getClasses')->willReturn([$stubClass]);
        $check = new ClassMethodsExistCheck($reflectionProvider);
    ...
    public function testKnownProblemVersionRangeOutsideBoundaryStillReportsFailure(): void
    {
        ...
        // no methods
        $knownProblemsProvider = $this->createStub(\Masjid_App\Dependencies\StubTests\Framework\Validator\KnownProblems\KnownProblemsProvider::class);
        $knownProblemsProvider->method('getProblems')->willReturn([new ProblemDefinition(entityType: EntityType::METHOD, entityId: $missingMethodId, type: ProblemType::INTERNAL_IMPLEMENTATION, affectedChecks: [CheckType::CLASS_METHODS_EXIST], versionRange: new PhpVersionRange(PhpVersions::PHP_8_0, PhpVersions::PHP_8_2), reason: 'Partial version skip')]);
        KnownProblemsRegistry::reset();
        $registry = KnownProblemsRegistry::getInstance($knownProblemsProvider);
        $reflectionProvider = $this->createMockReflectionProvider([], [$reflectionClass]);
        $stubsManager = $this->createMockStorageManager();
