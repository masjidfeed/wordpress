...
namespace Masjid_App\Dependencies\StubTests\Unit\Validator\Functions;
...
use Masjid_App\Dependencies\StubTests\Framework\Validator\Functions\FunctionParameterDeprecationCheck;
...
class FunctionParameterDeprecationCheckTest extends CheckTestCase
{
    ...
    {
        $id = '\missing_func';
        $reflFn = $this->makeFunction($id, [$this->makeParam('p', deprecated: \true)]);
    ...
    {
        $id = '\my_func';
        $provider = $this->createMockReflectionProvider([$this->makeFunction($id, [$this->makeParam('value')])]);
        $stubs = $this->createMockStorageManager();
        $stubs->method('getFunctions')->willReturn([$this->makeFunction($id, [$this->makeParam('value')])]);
        $result = (new FunctionParameterDeprecationCheck($provider))->run($stubs, $id, '8.0');
        $this->assertFalse($result->hasFailures());
        $this->assertEquals(1, $result->getSuccessCount());
    }
    ...
    {
        $id = '\define';
        $provider = $this->createMockReflectionProvider([$this->makeFunction($id, [$this->makeParam('case_insensitive', deprecated: \true)])]);
        $stubs = $this->createMockStorageManager();
        $stubs->method('getFunctions')->willReturn([$this->makeFunction($id, [$this->makeParam('case_insensitive', deprecated: \true)])]);
        $result = (new FunctionParameterDeprecationCheck($provider))->run($stubs, $id, '8.0');
        $this->assertFalse($result->hasFailures());
        $this->assertEquals(1, $result->getSuccessCount());
    }
    ...
    {
        $id = '\define';
        $provider = $this->createMockReflectionProvider([$this->makeFunction($id, [$this->makeParam('constant_name'), $this->makeParam('value'), $this->makeParam('case_insensitive', deprecated: \true)])]);
        $stubs = $this->createMockStorageManager();
        $stubs->method('getFunctions')->willReturn([$this->makeFunction($id, [$this->makeParam('constant_name'), $this->makeParam('value'), $this->makeParam('case_insensitive')])]);
        $result = (new FunctionParameterDeprecationCheck($provider))->run($stubs, $id, '8.0');
        $this->assertTrue($result->hasFailures());
    ...
    public function testStubParameterDeprecationStartingLaterDoesNotSatisfyReflection(): void
    {
        ...
        $provider = $this->createMockReflectionProvider([$this->makeFunction($id, [$this->makeParam('case_insensitive', deprecated: \true)])]);
        $stubs = $this->createMockStorageManager();
        $stubs->method('getFunctions')->willReturn([$this->makeFunction($id, [$this->makeParam('case_insensitive', deprecated: \true, deprecatedSinceVersion: '8.4')])]);
        $result = (new FunctionParameterDeprecationCheck($provider))->run($stubs, $id, '8.0');
        $this->assertTrue($result->hasFailures());
        $this->assertStringContainsString('$case_insensitive', $result->getFailures()[$id]);
    }
    ...
    public function testDeprecatedInStubsButNotInReflectionIsSuccess(): void
    {
        ...
        $id = '\my_func';
        $provider = $this->createMockReflectionProvider([$this->makeFunction($id, [$this->makeParam('p')])]);
        $stubs = $this->createMockStorageManager();
        $stubs->method('getFunctions')->willReturn([$this->makeFunction($id, [$this->makeParam('p', deprecated: \true)])]);
        $result = (new FunctionParameterDeprecationCheck($provider))->run($stubs, $id, '8.0');
        $this->assertFalse($result->hasFailures());
        $this->assertEquals(1, $result->getSuccessCount());
    }
    ...
    {
        $id = '\my_func';
        $provider = $this->createMockReflectionProvider([$this->makeFunction($id, [$this->makeParam('a', deprecated: \true), $this->makeParam('b'), $this->makeParam('c', deprecated: \true)])]);
        $stubs = $this->createMockStorageManager();
        $stubs->method('getFunctions')->willReturn([$this->makeFunction($id, [$this->makeParam('a'), $this->makeParam('b'), $this->makeParam('c')])]);
        $result = (new FunctionParameterDeprecationCheck($provider))->run($stubs, $id, '8.0');
        $this->assertTrue($result->hasFailures());
    ...
    {
        $id = '\my_func';
        $provider = $this->createMockReflectionProvider([$this->makeFunction($id, [$this->makeParam('kept'), $this->makeParam('removed', deprecated: \true)])]);
        $stubs = $this->createMockStorageManager();
        $stubs->method('getFunctions')->willReturn([$this->makeFunction($id, [$this->makeParam('kept')])]);
        $result = (new FunctionParameterDeprecationCheck($provider))->run($stubs, $id, '8.0');
        $this->assertFalse($result->hasFailures());
    }
    // ── Known problems ────────────────────────────────────────────────────────
    public function testKnownProblemSkipsValidation(): void
    {
        ...
        $knownProblemsProvider->method('getProblems')->willReturn([new ProblemDefinition(entityType: EntityType::FUNCTION, entityId: $id, type: ProblemType::INTERNAL_IMPLEMENTATION, affectedChecks: [CheckType::PARAMETER_DEPRECATION], versionRange: new PhpVersionRange(PhpVersions::EARLIEST, PhpVersions::LATEST), reason: 'Test skip reason')]);
        KnownProblemsRegistry::reset();
        $registry = KnownProblemsRegistry::getInstance($knownProblemsProvider);
        $provider = $this->createMockReflectionProvider([$this->makeFunction($id, [$this->makeParam('p', deprecated: \true)])]);
        $stubs = $this->createMockStorageManager();
        $stubs->method('getFunctions')->willReturn([$this->makeFunction($id, [$this->makeParam('p')])]);
