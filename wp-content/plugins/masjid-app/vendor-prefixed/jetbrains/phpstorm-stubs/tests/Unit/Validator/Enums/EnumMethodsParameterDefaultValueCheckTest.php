...
class EnumMethodsParameterDefaultValueCheckTest extends CheckTestCase
{
    ...
    {
        $enumId = '\MyEnum';
        $stubEnum = $this->getMockBuilder(PHPEnum::class)->disableOriginalConstructor()->onlyMethods(['getId', 'getName', 'getMethods', 'getImplementedInterfaces'])->getMock();
    ...
    {
        $enumId = '\MyEnum';
        $reflParams = [$this->makeParam('flags', optional: \true, hasDefaultValue: \true, defaultValue: 0)];
        $stubParams = [$this->makeParam('flags', optional: \true, hasDefaultValue: \true, defaultValue: 0)];
        $reflMethod = $this->createMockMethod('process', $reflParams);
        $stubMethod = $this->createMockMethod('process', $stubParams);
        $reflEnum = $this->getMockBuilder(PHPEnum::class)->disableOriginalConstructor()->onlyMethods(['getId', 'getName', 'getMethods', 'getImplementedInterfaces'])->getMock();
    ...
    public function testMismatchedDefaultIsFailure(): void
    {
        $enumId = '\MyEnum';
        $reflParams = [$this->makeParam('flags', optional: \true, hasDefaultValue: \true, defaultValue: 0)];
        $stubParams = [$this->makeParam('flags', optional: \true, hasDefaultValue: \true, defaultValue: 5)];
        // wrong
        $reflMethod = $this->createMockMethod('process', $reflParams);
        $stubMethod = $this->createMockMethod('process', $stubParams);
        $reflEnum = $this->getMockBuilder(PHPEnum::class)->disableOriginalConstructor()->onlyMethods(['getId', 'getName', 'getMethods', 'getImplementedInterfaces'])->getMock();
