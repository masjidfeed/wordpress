...
class InterfaceNamespaceCheckTest extends CheckTestCase
{
    ...
    {
        $iface = $this->makeInterface('Masjid_App\Dependencies\Random\Engine', namespace: '\Random');
        $stubs = $this->createMockStorageManager();
        $stubs->method('getInterfaces')->willReturn([$iface]);
        $result = $this->check->run($stubs, 'Masjid_App\Dependencies\Random\Engine', '8.2');
        $this->assertFalse($result->hasFailures());
    }
    public function testDeeplyNamespacedInterface(): void
    {
        $iface = $this->makeInterface('Masjid_App\Dependencies\Foo\Bar\Baz\MyInterface', namespace: 'Masjid_App\Dependencies\Foo\Bar\Baz');
        $stubs = $this->createMockStorageManager();
        $stubs->method('getInterfaces')->willReturn([$iface]);
        $result = $this->check->run($stubs, 'Masjid_App\Dependencies\Foo\Bar\Baz\MyInterface', '8.0');
        $this->assertFalse($result->hasFailures());
    }
    // ── Namespace mismatch ────────────────────────────────────────────────────
    public function testNamespaceMismatchIsFailure(): void
    {
        $iface = $this->makeInterface('Masjid_App\Dependencies\Random\Engine', namespace: '\WrongNamespace');
        $stubs = $this->createMockStorageManager();
        $stubs->method('getInterfaces')->willReturn([$iface]);
        $result = $this->check->run($stubs, 'Masjid_App\Dependencies\Random\Engine', '8.2');
        $this->assertTrue($result->hasFailures());
        $failures = $result->getFailures();
        $this->assertArrayHasKey('Masjid_App\Dependencies\Random\Engine', $failures);
