...
class ClassConstantsVisibilityCheckTest extends CheckTestCase
{
    ...
    {
        $classId = '\MyClass';
        $reflClass = $this->makeClass($classId, constants: [$this->makeClassConstant('OK', visibility: AccessModifier::PUBLIC), $this->makeClassConstant('BAD', visibility: AccessModifier::PUBLIC)]);
        $stubClass = $this->makeClass($classId, constants: [
            $this->makeClassConstant('OK', visibility: AccessModifier::PUBLIC),
            // matches
            $this->makeClassConstant('BAD', visibility: AccessModifier::PROTECTED),
        ]);
        $provider = $this->createMockReflectionProviderWithClasses([$reflClass]);
        $stubs = $this->createMockStorageManager();
        $stubs->method('getClasses')->willReturn([$stubClass]);
    ...
    public function testClassLevelKnownProblemSkipsEntireCheck(): void
    {
        ...
        $knownProblemsProvider->method('getProblems')->willReturn([new ProblemDefinition(entityType: EntityType::CLASS_TYPE, entityId: $classId, type: ProblemType::INTERNAL_IMPLEMENTATION, affectedChecks: [CheckType::CLASS_CONSTANTS_VISIBILITY], versionRange: new PhpVersionRange(PhpVersions::EARLIEST, PhpVersions::LATEST), reason: 'Class-level visibility skip')]);
        KnownProblemsRegistry::reset();
    ...
    {
        $classId = '\SpecialClass';
        $reflClass = $this->makeClass($classId, constants: [$this->makeClassConstant('GOOD', visibility: AccessModifier::PUBLIC), $this->makeClassConstant('LEGACY', visibility: AccessModifier::PUBLIC)]);
        $stubClass = $this->makeClass($classId, constants: [
            $this->makeClassConstant('GOOD', visibility: AccessModifier::PUBLIC),
            // matches → passes
            $this->makeClassConstant('LEGACY', visibility: AccessModifier::PROTECTED),
        ]);
        $knownProblemsProvider = $this->createStub(\Masjid_App\Dependencies\StubTests\Framework\Validator\KnownProblems\KnownProblemsProvider::class);
        $knownProblemsProvider->method('getProblems')->willReturn([new ProblemDefinition(entityType: EntityType::CLASS_CONSTANT, entityId: $classId . '::LEGACY', type: ProblemType::INTERNAL_IMPLEMENTATION, affectedChecks: [CheckType::CLASS_CONSTANTS_VISIBILITY], versionRange: new PhpVersionRange(PhpVersions::EARLIEST, PhpVersions::LATEST), reason: 'Constant-level visibility skip')]);
        KnownProblemsRegistry::reset();
        $registry = KnownProblemsRegistry::getInstance($knownProblemsProvider);
        $provider = $this->createMockReflectionProviderWithClasses([$reflClass]);
        $stubs = $this->createMockStorageManager();
