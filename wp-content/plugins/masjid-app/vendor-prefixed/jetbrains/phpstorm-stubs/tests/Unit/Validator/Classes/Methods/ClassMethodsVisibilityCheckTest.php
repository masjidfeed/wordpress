...
class ClassMethodsVisibilityCheckTest extends CheckTestCase
{
    ...
    {
        $className = '\MyClass';
        $reflClass = $this->createMockClassWithProperties($className, null, null, null, [$this->makeMethod('doWork', access: 'public')]);
        $stubClass = $this->createMockClassWithProperties($className, null, null, null, [$this->makeMethod('doWork', access: 'public')]);
        $provider = $this->createMockReflectionProvider([], [$reflClass]);
        $stubs = $this->createMockStorageManager();
        $stubs->method('getClasses')->willReturn([$stubClass]);
        $result = (new ClassMethodsVisibilityCheck($provider))->run($stubs, $className, '8.0');
    ...
    {
        $className = '\MyClass';
        $reflClass = $this->createMockClassWithProperties($className, null, null, null, [$this->makeMethod('doWork', access: 'protected')]);
        $stubClass = $this->createMockClassWithProperties($className, null, null, null, [$this->makeMethod('doWork', access: 'protected')]);
        $provider = $this->createMockReflectionProvider([], [$reflClass]);
        $stubs = $this->createMockStorageManager();
        $stubs->method('getClasses')->willReturn([$stubClass]);
        $result = (new ClassMethodsVisibilityCheck($provider))->run($stubs, $className, '8.0');
    ...
    {
        $className = '\MyClass';
        $reflClass = $this->createMockClassWithProperties($className, null, null, null, [$this->makeMethod('doWork', access: 'private')]);
        $stubClass = $this->createMockClassWithProperties($className, null, null, null, [$this->makeMethod('doWork', access: 'private')]);
        $provider = $this->createMockReflectionProvider([], [$reflClass]);
        $stubs = $this->createMockStorageManager();
        $stubs->method('getClasses')->willReturn([$stubClass]);
        $result = (new ClassMethodsVisibilityCheck($provider))->run($stubs, $className, '8.0');
    ...
    public function testMultipleMethodsMixedResultsAreAllReported(): void
    {
        ...
        $reflClass = $this->createMockClassWithProperties($className, null, null, null, [
            ...
            // matches
            $this->makeMethod('badMethod', access: 'protected'),
        ]);
        $stubClass = $this->createMockClassWithProperties($className, null, null, null, [$this->makeMethod('okMethod', access: 'public'), $this->makeMethod('badMethod', access: 'public')]);
        $provider = $this->createMockReflectionProvider([], [$reflClass]);
        $stubs = $this->createMockStorageManager();
        $stubs->method('getClasses')->willReturn([$stubClass]);
        $result = (new ClassMethodsVisibilityCheck($provider))->run($stubs, $className, '8.0');
    ...
    {
        $className = '\MyClass';
        $reflClass = $this->createMockClassWithProperties($className, null, null, null, [$this->makeMethod('onlyInReflection', access: 'public')]);
        $stubClass = $this->createMockClassWithProperties($className);
        // no methods
        $provider = $this->createMockReflectionProvider([], [$reflClass]);
        $stubs = $this->createMockStorageManager();
    ...
    public function testClassLevelKnownProblemSkipsAllMethods(): void
    {
        ...
        $knownProblemsProvider->method('getProblems')->willReturn([new ProblemDefinition(entityType: EntityType::CLASS_TYPE, entityId: $className, type: ProblemType::INTERNAL_IMPLEMENTATION, affectedChecks: [CheckType::CLASS_METHODS_VISIBILITY], versionRange: new PhpVersionRange(PhpVersions::EARLIEST, PhpVersions::LATEST), reason: 'Class-level skip')]);
        KnownProblemsRegistry::reset();
    ...
    public function testMethodLevelKnownProblemSkipsSpecificMismatch(): void
    {
        ...
        $stubClass = $this->createMockClassWithProperties($className, null, null, null, [$this->makeMethod('okMethod', access: 'public'), $this->makeMethod('doWork', access: 'public')]);
        $knownProblemsProvider = $this->createStub(\Masjid_App\Dependencies\StubTests\Framework\Validator\KnownProblems\KnownProblemsProvider::class);
        $knownProblemsProvider->method('getProblems')->willReturn([new ProblemDefinition(entityType: EntityType::METHOD, entityId: $mismatchedId, type: ProblemType::INTERNAL_IMPLEMENTATION, affectedChecks: [CheckType::CLASS_METHODS_VISIBILITY], versionRange: new PhpVersionRange(PhpVersions::EARLIEST, PhpVersions::LATEST), reason: 'Method-level skip')]);
        KnownProblemsRegistry::reset();
        $registry = KnownProblemsRegistry::getInstance($knownProblemsProvider);
        $provider = $this->createMockReflectionProvider([], [$reflClass]);
        $stubs = $this->createMockStorageManager();
    ...
    {
        $className = '\MyClass';
        $reflClass = $this->createMockClassWithProperties($className, null, null, null, [$this->makeMethod('newMethod', access: 'protected')]);
        $stubClass = $this->createMockClassWithProperties($className, null, null, null, [$this->makeMethod('newMethod', sinceVersion: '8.1', access: 'public')]);
        $provider = $this->createMockReflectionProvider([], [$reflClass]);
        $stubs = $this->createMockStorageManager();
        $stubs->method('getClasses')->willReturn([$stubClass]);
        $result = (new ClassMethodsVisibilityCheck($provider))->run($stubs, $className, '8.0');
    ...
    {
        $className = '\Generator';
        $reflClass = $this->createMockClassWithProperties($className, null, null, null, [$this->makeMethod('throw', access: 'public')]);
        $stubClass = $this->createMockClassWithProperties($className, null, null, null, [$this->makeMethod('PS_UNRESERVE_PREFIX_throw', access: 'protected')]);
        $provider = $this->createMockReflectionProvider([], [$reflClass]);
        $stubs = $this->createMockStorageManager();
        $stubs->method('getClasses')->willReturn([$stubClass]);
        $result = (new ClassMethodsVisibilityCheck($provider))->run($stubs, $className, '8.0');
    ...
    public function testPsUnreservePrefixMethodVisibilityMatchIsSuccess(): void
    {
        $className = '\Generator';
        $reflClass = $this->createMockClassWithProperties($className, null, null, null, [$this->makeMethod('throw', access: 'public')]);
        $stubClass = $this->createMockClassWithProperties($className, null, null, null, [$this->makeMethod('PS_UNRESERVE_PREFIX_throw', access: 'public')]);
        $provider = $this->createMockReflectionProvider([], [$reflClass]);
        $stubs = $this->createMockStorageManager();
        $stubs->method('getClasses')->willReturn([$stubClass]);
        $result = (new ClassMethodsVisibilityCheck($provider))->run($stubs, $className, '8.0');
    ...
    public function testVisibilityFromParentClassMismatchIsReported(): void
    {
        ...
        $parentStub->setMethods([$this->makeMethod('doWork', access: 'public')]);
        // stub parent: public → mismatch
    ...
    public function testVisibilityFromParentClassMatchIsSuccess(): void
    {
        ...
        $parentStub = new PHPClass();
        $parentStub->setId($parentClassName);
        $parentStub->setMethods([$this->makeMethod('doWork', access: 'protected')]);
        // matches
        $childStub = $this->createMockClassWithProperties($className);
        $childStub->setParentClass($parentStub);
        $provider = $this->createMockReflectionProvider([], [$reflClass]);
    ...
    {
        $className = '\MyClass';
        $reflClass = $this->createMockClassWithProperties($className, null, null, null, [$this->makeMethod('interfaceMethod', access: 'public')]);
        $iface = $this->makeInterface('\MyInterface', [$this->makeMethod('interfaceMethod', access: 'protected')]);
    ...
    public function testChildMethodOverridesParentVisibility(): void
    {
        ...
        $parentStub->setMethods([$this->makeMethod('doWork', access: 'protected')]);
        // parent: protected
        $childStub = $this->createMockClassWithProperties($className, null, null, null, [$this->makeMethod('doWork', access: 'public')]);
        $childStub->setParentClass($parentStub);
        $provider = $this->createMockReflectionProvider([], [$reflClass]);
        $stubs = $this->createMockStorageManager();
        $stubs->method('getClasses')->willReturn([$childStub]);
