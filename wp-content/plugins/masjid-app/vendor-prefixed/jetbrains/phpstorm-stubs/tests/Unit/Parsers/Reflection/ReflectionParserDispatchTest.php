...
use Masjid_App\Dependencies\StubTests\Framework\Parsers\Reflection\ReflectionClassConstantParser;
use Masjid_App\Dependencies\StubTests\Framework\Parsers\Reflection\ReflectionClassParser;
use Masjid_App\Dependencies\StubTests\Framework\Parsers\Reflection\ReflectionDefineConstantParser;
...
use Masjid_App\Dependencies\StubTests\Framework\Parsers\Reflection\ReflectionImplementedInterfaceParser;
use Masjid_App\Dependencies\StubTests\Framework\Parsers\Reflection\ReflectionInterfaceParser;
use Masjid_App\Dependencies\StubTests\Framework\Parsers\Reflection\ReflectionMethodParser;
use Masjid_App\Dependencies\StubTests\Framework\Parsers\Reflection\ReflectionParameterParser;
use Masjid_App\Dependencies\StubTests\Framework\Parsers\Reflection\ReflectionParentClassParser;
use Masjid_App\Dependencies\StubTests\Framework\Parsers\Reflection\ReflectionPropertyParser;
...
class ReflectionParserDispatchTest extends TestCase
{
    ...
    public function testExactlyTheExpectedParserClaimsEachClassLikeShape(bool $internal, bool $interface, bool $enum, array $expected)
    {
    ...
    {
        return [
            'internal plain class' => [\true, \false, \false, ['ReflectionClassParser']],
            'internal interface' => [\true, \true, \false, ['ReflectionInterfaceParser']],
            'internal enum' => [\true, \false, \true, ['ReflectionEnumParser']],
            // userland entities are not reflected into the caches at all
            'userland class' => [\false, \false, \false, []],
            'userland interface' => [\false, \true, \false, []],
            'userland enum' => [\false, \false, \true, []],
        ];
    }
    /**
    ...
    {
        $accepted = $this->acceptedBy($this->adaptedClass($internal, $interface, $enum));
        self::assertLessThanOrEqual(1, count($accepted), sprintf('ambiguous dispatch for internal=%s interface=%s enum=%s: %s', var_export($internal, \true), var_export($interface, \true), var_export($enum, \true), implode(', ', $accepted)));
    }
    public static function allFlagCombinations(): array
    {
        $cases = [];
        foreach ([\true, \false] as $internal) {
            foreach ([\true, \false] as $interface) {
                foreach ([\true, \false] as $enum) {
                    if ($interface && $enum) {
                        continue;
                        // not producible by PHP; see the dedicated test below
                    }
                    $cases[sprintf('internal=%d interface=%d enum=%d', $internal, $interface, $enum)] = [$internal, $interface, $enum];
                }
            }
        }
        return $cases;
    ...
    {
        $contradictory = $this->adaptedClass(\true, \true, \true);
        self::assertSame(['ReflectionEnumParser', 'ReflectionInterfaceParser'], $this->acceptedBy($contradictory), 'documented overlap; unreachable via the reflection API');
    }
    ...
    {
        self::assertCount(1, $this->acceptedBy($this->adaptedClass(\true, $interface, $enum)));
    }
    public static function realisticInternalShapes(): array
    {
        return ['plain class' => [\false, \false], 'interface' => [\true, \false], 'enum' => [\false, \true]];
    }
    ...
    {
        $internalClass = $this->adaptedClass(\true, \false, \false);
    ...
    public function testSubParsersNeverClaimAnything(string $parserClass)
    {
        ...
        foreach ([$this->adaptedClass(\true, \false, \false), $this->createStub(AdaptedReflectionFunction::class), $this->createStub(AdaptedReflectionParameter::class), ['SOME_CONST' => 1], 'string', null] as $candidate) {
            self::assertFalse($parser->canParse($candidate), $parserClass . ' claimed something');
        }
    }
    public static function subParsers(): array
    {
        return ['class constants' => [ReflectionClassConstantParser::class], 'methods' => [ReflectionMethodParser::class], 'properties' => [ReflectionPropertyParser::class], 'parent class' => [ReflectionParentClassParser::class], 'implemented interfaces' => [ReflectionImplementedInterfaceParser::class], 'define constants' => [ReflectionDefineConstantParser::class]];
    }
}
