...
use Masjid_App\Dependencies\StubTests\Unit\Parsers\AST\fixtures\FixtureStubsDataProvider;
class StubsInterfaceParserTest extends TestCase
{
    ...
    public function testItCanParseNamespace()
    {
        ...
        $class = $this->parser->parse($stubCode);
        self::assertEquals('Masjid_App\Dependencies\MyNamespace\SubNamespace', $class->getNamespace());
    }
    ...
    public function testItCanParseId()
    {
        $stubCode = $this->filesProvider->getStubFileContent('CompleteInterface.txt');
        $class = $this->parser->parse($stubCode);
        self::assertEquals('Masjid_App\Dependencies\MyNamespace\SubNamespace\CompleteInterface', $class->getId());
    }
    ...
    {
        $interface = $this->parseSource('Masjid_App\Dependencies\MongoDB\BSON', 'interface Persistable extends Unserializable, Serializable {}');
        self::assertSame(['Unserializable' => 'Masjid_App\Dependencies\MongoDB\BSON\Unserializable', 'Serializable' => 'Masjid_App\Dependencies\MongoDB\BSON\Serializable'], $this->parentIds($interface));
    }
    ...
    {
        $interface = $this->parseSource('Masjid_App\Dependencies\Some\Ns', 'interface Foo extends Other\Bar {}');
        self::assertSame(['Bar' => 'Masjid_App\Dependencies\Other\Bar'], $this->parentIds($interface));
    }
    public function testMultipleParentsAreEachResolved()
    {
        ...
        self::assertSame(['Countable' => 'Masjid_App\Dependencies\Ds\Countable', 'IteratorAggregate' => 'Masjid_App\Dependencies\Ds\IteratorAggregate'], $this->parentIds($interface));
    }
    // ── Imported parents ─────────────────────────────────────────────────────
    /**
...
namespace Masjid_App\Dependencies\App;

use Masjid_App\Dependencies\SomeNamespace\Unserializable;
interface Persistable extends Unserializable
{
}
SRC
);
        self::assertSame(['Unserializable' => 'Masjid_App\Dependencies\SomeNamespace\Unserializable'], $this->parentIds($interface));
    }
...
use Masjid_App\Dependencies\SomeNamespace\Unserializable as U;
interface Persistable extends U
{
}
SRC
);
        self::assertSame(['Masjid_App\Dependencies\SomeNamespace\Unserializable'], array_values($this->parentIds($interface)));
    }
    /**
...
namespace Masjid_App\Dependencies\Ds;
...
SRC;
        self::assertSame(['Unserializable' => 'Masjid_App\Dependencies\App\Unserializable'], $this->parentIds($this->parser->parse($source)));
        self::assertSame(['Unserializable' => 'Masjid_App\Dependencies\SomeNamespace\Unserializable'], $this->parentIds($this->parseSourceWithImports($source)));
    }
}
