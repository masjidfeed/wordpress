...
class StubsCacheIntegrityTest extends TestCase
{
     ...
     */
    private const STUB_FLOORS = ['classes' => [500, 1416], 'functions' => [1000, 5177], 'interfaces' => [50, 151], 'enums' => [5, 18], 'constants' => [1000, 8255]];
    /**
    ...
    public function testStubsCacheHoldsAPlausibleNumberOf(string $type, int $floor, int $observed): void
    {
        ...
        self::assertGreaterThanOrEqual($floor, $count, sprintf("The stubs cache loaded only %d %s (expected at least %d; there were %d when this " . "floor was set).\nAn undersized cache does not fail any validator — the checks " . "simply iterate nothing and pass — so this is very likely a truncated or " . "partially written tests/cache/Stubs*.json.\nRegenerate with: docker compose -f " . "docker-compose.yml run --rm test_runner php tests/run-stubs-parser.php", $count, $type, $floor, $observed));
    }
    /**
    ...
    public function testReflectionCacheHoldsAPlausibleNumberOfEntities(string $phpVersion): void
    {
        ...
        self::assertGreaterThanOrEqual(100, count($reflection->getClasses()), sprintf('Reflection cache for PHP %s loaded implausibly few classes (smallest real cache ' . 'is 5.6 with 156). Regenerate with tests/run-all-reflection-parsers.sh', $phpVersion));
        self::assertGreaterThanOrEqual(1000, count($reflection->getFunctions()), sprintf('Reflection cache for PHP %s loaded implausibly few functions (smallest real ' . 'cache is 5.6 with 1641).', $phpVersion));
    }
}
