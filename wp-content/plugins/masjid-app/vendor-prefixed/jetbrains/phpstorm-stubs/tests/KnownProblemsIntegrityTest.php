...
use Masjid_App\Dependencies\StubTests\Framework\Validator\KnownProblems\DefaultKnownProblemsProvider;
...
class KnownProblemsIntegrityTest extends TestCase
{
    ...
    public function testKnownProblemEntityStillExistsInStubs(ProblemDefinition $problem, string $entityId): void
    {
        ...
        if ($exists === null) {
            ...
            self::markTestSkipped(sprintf("Cannot resolve %s '%s': PHPInterface and PHPEnum do not model properties, and " . "StubInterfaceParser does not parse them. PHP 8.4 permits property declarations " . "in interfaces, so this is a modelling gap, not a stale entry.", $problem->entityType->value, $entityId));
        }
        self::assertTrue($exists, sprintf("Known problem references %s '%s', which no longer exists in the stubs.\n" . "The suppression can never fire, so it is dead config. Either the entity was " . "renamed/removed and the entry should follow it, or the entry should be deleted.\n" . "Reason recorded on the entry: %s", $problem->entityType->value, $entityId, $problem->reason));
    }
    /**
    ...
    private static function entityExistsInStubs(EntityType $entityType, string $entityId): ?bool
    {
        ...
        return match ($entityType) {
            ...
            EntityType::CLASS_CONSTANT, EntityType::INTERFACE_CONSTANT, EntityType::ENUM_CONSTANT => self::hasMember($entityId, 'constant'),
        };
    }
    /**
    ...
    private static function hasMember(string $entityId, string $kind): ?bool
    {
        ...
        foreach (self::selfAndAncestors($owner) as $type) {
            $found = match ($kind) {
                ...
                'method' => self::hasNamed($type->getMethods(), $memberName, caseInsensitive: \true),
                ...
                default => $type instanceof PHPClass ? self::hasNamed($type->getProperties(), ltrim($memberName, '$')) : self::note($sawNonClassOwner),
            };
            if ($found) {
                return \true;
            }
    ...
    private static function selfAndAncestors(PHPClassLikeObject $type): array
    {
        ...
        while ($queue !== []) {
            ...
            if (isset($seen[$id])) {
                continue;
                // guards against a malformed cyclic hierarchy
            }
            $seen[$id] = $current;
    ...
    private static function hasNamed(array $members, string $name, bool $caseInsensitive = \false): bool
    {
        foreach ($members as $member) {
            $matches = $caseInsensitive ? strcasecmp($member->getName(), $name) === 0 : $member->getName() === $name;
            if ($matches) {
                return \true;
            }
        }
