...
use Masjid_App\Dependencies\StubTests\Framework\Validator\RunnerStubsProvider;
/**
...
abstract class ValidatorTestBase extends TestCase
{
    ...
    public function testEntity(string $methodName, string $entityId, string $phpVersion): void
    {
        ...
        if (isset($descriptors[$methodName])) {
            ...
            $check = $descriptor->entityTypeConfig !== null ? new $checkClass(entityTypeConfig: $descriptor->entityTypeConfig) : new $checkClass();
            $message = str_replace(['{entityId}', '{phpVersion}'], [$entityId, $phpVersion], $descriptor->messageTemplate);
     ...
     */
    protected function executeCheck(CheckInterface $check, string $entityId, string $phpVersion, ?string $customMessage = null): void
    {
        ...
        if (!$check->supports($phpVersion)) {
            $this->fail(sprintf('%s::supports(%s) is false, but the check is registered for that version. ' . 'Narrow the registered version range to match the check, or widen supports().', get_class($check), $phpVersion));
        }
        ...
        $this->assertEmpty($failures, $message . "\n" . implode("\n", $failures));
    }
}
