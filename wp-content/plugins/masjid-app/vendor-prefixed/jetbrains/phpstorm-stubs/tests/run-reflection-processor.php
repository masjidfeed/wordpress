...
use Masjid_App\Dependencies\StubTests\Framework\Parsers\Reflection\ReflectionModernConstantParser;
...
try {
    ...
    if (\preg_match('/Reflection(\d+\.\d+)\.json$/', $outputFile, $targetMatch) === 1) {
        ...
        if ($runtimeVersion !== 'unknown' && $actualMajorMinor !== $targetVersion) {
            $message = "Refusing to write {$outputFile}: the input data was extracted on PHP " . "{$runtimeVersion}, not PHP {$targetVersion}. Rebuild the php_under_test image for " . "{$targetVersion} (drop --skip-build) so Stage 1 runs on the intended runtime.";
    ...
    $storageManager = new DefaultParsedDataStorageManager($storage, EntityProcessingPipelineFactory::forReflection());
    ...
    $constantParser = \class_exists('\ReflectionConstant') ? new ReflectionModernConstantParser() : new ReflectionDefineConstantParser();
    ...
    if (\file_exists($outputFile)) {
        ...
        $fileSizeFormatted = \number_format($fileSize / 1024 / 1024, 2);
