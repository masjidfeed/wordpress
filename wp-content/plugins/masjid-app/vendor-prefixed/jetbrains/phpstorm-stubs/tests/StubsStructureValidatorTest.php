...
use Masjid_App\Dependencies\PHPUnit\Framework\Attributes\Test;
...
class StubsStructureValidatorTest extends TestCase
{
    ...
    public function checkStubsDirectoriesExistsInMap(string $directoryName): void
    {
        ...
        self::assertContains($directoryName, $allCategoryDirs, "Directory '{$directoryName}' is not listed in any StubCategory::getDirectories(). " . "Add it to the appropriate category (CORE, BUNDLED, EXTERNAL, PECL, or OTHERS) in " . "tests/Framework/DataProvider/StubCategory.php.");
    }
    /**
    ...
    public function checkGenerateStubsMapScript(): void
    {
        ...
        self::assertSame(realpath($existingMap), $scriptDefaultOutput, 'The default output path inside generate-stubs-map.php must resolve to PhpStormStubsMap.php in the project root.');
        exec(sprintf('php %s %s 2>&1', escapeshellarg($scriptPath), escapeshellarg($tempOutput)), $execOutput, $exitCode);
        try {
            self::assertSame(0, $exitCode, "Generator script exited with code {$exitCode}:\n" . implode("\n", $execOutput));
            ...
            self::assertStringNotContainsString($projectRoot, $generated, 'PhpStormStubsMap.php must store relative paths, not absolute ones.');
        } finally {
    ...
    public function checkStubMapIsUpToDate(): void
    {
        ...
        exec(sprintf('php %s %s 2>&1', escapeshellarg($scriptPath), escapeshellarg($tempOutput)), $execOutput, $exitCode);
        try {
            self::assertSame(0, $exitCode, "Generator script failed with exit code {$exitCode}:\n" . implode("\n", $execOutput));
            self::assertSame(file_get_contents($existingMap), file_get_contents($tempOutput), 'PhpStormStubsMap.php is out of date. ' . 'Regenerate it by running: php tests/Framework/Tools/generate-stubs-map.php');
        } finally {
            if (file_exists($tempOutput)) {
                unlink($tempOutput);
            }
