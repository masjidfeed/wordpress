...
try {
    ...
    // Package all data
    $extractedData = ['phpVersion' => $phpVersion, 'runtimeVersion' => \PHP_VERSION, 'classes' => $wrappedClasses, 'interfaces' => $wrappedInterfaces, 'enums' => $wrappedEnums, 'functions' => $wrappedFunctions, 'constants' => $constants];
    // Save extracted data
    echo "[7/7] Saving wrapped reflection data...\n";
    // Ensure directory exists
    $outputDir = \dirname($outputFile);
    if (!\file_exists($outputDir)) {
        \mkdir($outputDir, 0777, \true);
    }
    // Serialize data
