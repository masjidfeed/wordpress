...
class PhpVersionsSyncTest extends TestCase
{
    ...
    public function testEachPhpVersionHasReflectionCacheFile(): void
    {
        foreach (PhpVersions::cases() as $version) {
            self::assertFileExists($this->cacheDir . '/Reflection' . $version->value . '.json', "Reflection cache missing for PHP {$version->value}. " . "Run tests/run-all-reflection-parsers.sh to regenerate, " . "then commit the new cache file.");
        }
    }
    /**
    ...
    public function testEarliestAndLatestPointAtTheBoundaryCases(): void
    {
        ...
        self::assertSame($cases[0], PhpVersions::EARLIEST, 'PhpVersions::EARLIEST must be the first enum case, otherwise every ' . 'CheckDescriptor ranged from EARLIEST skips the versions before it.');
        self::assertSame($cases[count($cases) - 1], PhpVersions::LATEST, 'PhpVersions::LATEST must be the last enum case, otherwise every ' . 'CheckDescriptor ranged to LATEST silently skips the newest version.');
    }
    public function testEachPhpVersionHasAPinnedPatchInTheManifest(): void
    {
        $manifestPath = $this->cacheDir . '/php-versions.json';
        self::assertFileExists($manifestPath);
        $manifest = json_decode(file_get_contents($manifestPath), \true, 512, \JSON_THROW_ON_ERROR);
        foreach (PhpVersions::cases() as $version) {
            self::assertArrayHasKey($version->value, $manifest, "tests/cache/php-versions.json has no pinned patch for PHP {$version->value}. " . "run-all-reflection-parsers.sh reads it to pin the base image, so an unpinned " . "version regenerates its cache from a floating patch release.");
        }
        foreach (array_keys($manifest) as $minor) {
            self::assertNotNull(PhpVersions::tryFrom((string) $minor), "tests/cache/php-versions.json pins PHP {$minor}, which has no PhpVersions case. " . "Either add the case or drop the manifest entry.");
        }
    }
    public function testEachPhpVersionHasADockerImage(): void
    {
        foreach (PhpVersions::cases() as $version) {
            self::assertFileExists(__DIR__ . '/DockerImages/' . $version->value . '/Dockerfile', "tests/DockerImages/{$version->value}/Dockerfile is missing, so the reflection " . "cache for PHP {$version->value} cannot be regenerated.");
        }
    }
    /**
        ...
        foreach (PhpVersions::cases() as $version) {
            ...
            foreach (self::EXTENSION_SENTINELS as $extension => [$symbol, $removedIn]) {
                ...
                if ($isPresent) {
                    ...
                    if ($isAfterRemoval) {
                        $problems[] = "{$extension}: declared removed in PHP {$removedIn}, but {$symbol} " . "is still in the PHP {$version->value} cache — correct the 'removedIn' entry, " . "a wrong one hides real coverage loss after it";
                    }
                    continue;
                }
                ...
                $problems[] = "{$extension}: missing from the PHP {$version->value} cache " . "({$symbol} is gone) but present from PHP {$firstSeenIn[$extension]} onwards";
            }
        }
        self::assertSame([], $problems, "Reflection cache coverage regressed:\n  - " . implode("\n  - ", $problems) . "\n\n" . "Either the version's tests/DockerImages/<version>/Dockerfile stopped building the " . "extension — restore it and regenerate with tests/run-all-reflection-parsers.sh — or the " . "extension really left core, in which case declare the release in " . "PhpVersionsSyncTest::EXTENSION_SENTINELS.");
    }
    /**
    ...
    private function symbolNamesInReflectionCache(PhpVersions $version): array
    {
        ...
        $entities = json_decode(file_get_contents($path), \true, 512, \JSON_THROW_ON_ERROR);
        $names = [];
        foreach ($entities as $entity) {
            if (isset($entity['name'])) {
                $names[strtolower($entity['name'])] = \true;
            }
        }
        return $names;
    }
    public function testNoOrphanedReflectionCacheFiles(): void
    {
        ...
        foreach ($cacheFiles as $file) {
            ...
            self::assertContains($version, $enumVersions, "Orphaned cache file Reflection{$version}.json has no matching PhpVersions case. " . "Either add the case to PhpVersions.php or delete the stale cache file.");
        }
    }
}
