...
use Masjid_App\Dependencies\StubTests\Framework\Parsers\Registries\EntityReflectionObjectParsersRegistry;
/**
...
class AllReflectionParser
{
    ...
    public function __construct(CurrentRuntimeReflectionRawDataProvider $dataProvider, ParsedDataWriter $storageManager, ?EntityReflectionObjectParsersRegistry $parsersRegistry = null)
    {
    ...
    public function parseAll(): void
    {
        ...
        $this->parseEntities(array_merge($this->dataProvider->getReflectionClasses(), $this->dataProvider->getReflectionInterfaces(), $this->dataProvider->getReflectionEnums()), function ($name) {
            return new AdaptedReflectionClass(new \ReflectionClass($name));
        });
        // Parse functions
        $this->parseEntities($this->dataProvider->getReflectionFunctions(), function ($name) {
    ...
    private function parseConstants(): void
    {
        ...
        if (class_exists('\ReflectionConstant')) {
            ...
            $this->parseEntities($constantNames, function ($name) {
                return new \ReflectionConstant($name);
            });
        } else {
            // For PHP < 8.1, use array format
            // Convert associative array to list of [name => value] entries
            $constantEntries = array_map(function ($name, $value) {
                return [$name => $value];
            }, array_keys($reflectionConstants), array_values($reflectionConstants));
            $this->parseEntities($constantEntries, function ($entry) {
                return $entry;
                // Already in array format expected by parser
            });
        }
    }
}
