...
use Masjid_App\Dependencies\StubTests\Framework\Validator\Services\ParameterNamesComparator;
/**
...
class ClassMethodsParameterNamesCheck extends AbstractMemberFlagCheck implements DescribesMethodMismatch
{
    ...
    public function describeMethodMismatch(string $methodEntityId, PHPMethod $reflMethod, PHPMethod $stubMethod, string $phpVersion): ?string
    {
        ...
        return "Method {$methodEntityId}: parameter name mismatch(es) in PHP {$phpVersion}: " . implode('; ', $mismatches);
    }
}
