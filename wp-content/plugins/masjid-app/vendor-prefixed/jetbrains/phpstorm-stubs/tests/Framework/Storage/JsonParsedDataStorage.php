...
use Masjid_App\Dependencies\StubTests\Framework\Serialization\EntitySerializerInterface;
/**
...
class JsonParsedDataStorage implements ParsedDataPersistentStorageProvider
{
    ...
    private bool $ownsPhpDocStorage = \true;
    /**
     ...
     */
    public function __construct(string $pathToJsonFile, EntitySerializerInterface $serializer, bool $loadExisting = \true, ?PhpDocStorage $phpDocStorage = null, bool $ownsPhpDocStorage = \true)
    {
    ...
    public function save(): void
    {
        ...
        $json = json_encode($serializedData, \JSON_PRETTY_PRINT | \JSON_UNESCAPED_SLASHES | \JSON_THROW_ON_ERROR);
    ...
    public function load(): void
    {
        ...
        $data = json_decode($jsonContent, \true);
        if (!is_array($data)) {
    ...
    private function corruptCacheMessage(string $problem): string
    {
        ...
        return sprintf("Cache file %s %s (%s bytes).\n" . "It exists, so nothing will regenerate it automatically, and treating it as empty " . "would let checks pass while validating nothing.\n" . "Regenerate with: docker compose -f docker-compose.yml run --rm test_runner php " . "tests/run-stubs-parser.php\n" . "(for a Reflection*.json file, use tests/run-all-reflection-parsers.sh instead)", $this->pathToJsonFile, $problem, $size === \false ? 'unknown' : $size);
    }
}
