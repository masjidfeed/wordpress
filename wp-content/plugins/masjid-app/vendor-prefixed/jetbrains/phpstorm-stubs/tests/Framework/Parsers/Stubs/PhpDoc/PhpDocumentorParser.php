...
namespace Masjid_App\Dependencies\StubTests\Framework\Parsers\Stubs\PhpDoc;

use Masjid_App\Dependencies\phpDocumentor\Reflection\DocBlock;
use Masjid_App\Dependencies\phpDocumentor\Reflection\DocBlock\Tags\Deprecated;
use Masjid_App\Dependencies\phpDocumentor\Reflection\DocBlock\Tags\Generic;
use Masjid_App\Dependencies\phpDocumentor\Reflection\DocBlock\Tags\Param;
use Masjid_App\Dependencies\phpDocumentor\Reflection\DocBlock\Tags\Return_;
use Masjid_App\Dependencies\phpDocumentor\Reflection\DocBlock\Tags\Since;
use Masjid_App\Dependencies\phpDocumentor\Reflection\DocBlock\Tags\Var_;
...
class PhpDocumentorParser implements PhpDocParserInterface
{
            ...
            switch ($m[1]) {
                ...
                case 'param':
                    if (preg_match('/\$(\w+)/', $rest, $vm)) {
                        $parsed->paramTypes[$vm[1]] = $this->preferFaithfulType($parsed->paramTypes[$vm[1]] ?? null, $type);
                    }
                    break;
            }
        }
    ...
    private function preferFaithfulType(?string $current, string $verbatim): string
    {
        ...
        if ($resolved !== null && $raw !== null && $resolved[0] === $raw[0] && $raw[1] > $resolved[1]) {
            return $verbatim;
        }
        return $current;
    }
        ...
        for (; $i < $len; $i++) {
            ...
            if ($c === '<' || $c === '{' || $c === '(') {
                ...
                if ($c === ' ') {
                    ...
                    $next = $j < $len ? $s[$j] : '';
                    if (in_array($prev, ['|', '&', ':'], \true) || in_array($next, ['|', '&', ':'], \true)) {
                        continue;
                    }
                    break;
                }
