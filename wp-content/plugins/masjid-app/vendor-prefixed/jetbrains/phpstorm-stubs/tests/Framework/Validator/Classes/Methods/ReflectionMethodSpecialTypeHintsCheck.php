...
namespace Masjid_App\Dependencies\StubTests\Framework\Validator\Classes\Methods;
...
use Masjid_App\Dependencies\StubTests\Framework\Validator\Services\TypeResolver;
/**
...
class ReflectionMethodSpecialTypeHintsCheck extends AbstractClassCheck
{
     ...
     */
    private const REQUIRED_HINTS = ['\ReflectionFunctionAbstract' => ['getReturnType' => ['7.1' => 'ReflectionNamedType|null', '8.0' => 'ReflectionNamedType|ReflectionUnionType|null', '8.1' => 'ReflectionNamedType|ReflectionUnionType|ReflectionIntersectionType|null']], '\ReflectionParameter' => ['getType' => ['7.1' => 'ReflectionNamedType|null', '8.0' => 'ReflectionNamedType|ReflectionUnionType|null', '8.1' => 'ReflectionNamedType|ReflectionUnionType|ReflectionIntersectionType|null']], '\ReflectionProperty' => ['getType' => ['8.0' => 'ReflectionNamedType|ReflectionUnionType|null', '8.1' => 'ReflectionNamedType|ReflectionUnionType|ReflectionIntersectionType|null']], '\ReflectionEnum' => ['getBackingType' => ['8.2' => 'ReflectionNamedType|null']]];
    ...
    public function run(StubDataQueryInterface $stubs, string $entityId, string $phpVersion): CheckResultSet
    {
        ...
        foreach (self::REQUIRED_HINTS[$entityId] as $methodName => $requiredVersions) {
            ...
            if ($actualVersionMap === null) {
                $hasFailure = \true;
                $results->addFailure($methodEntityId, "{$methodEntityId} is missing LanguageLevelTypeAware version-specific entries");
                continue;
            }
            foreach ($requiredVersions as $requiredVersion => $requiredType) {
                if (!array_key_exists($requiredVersion, $actualVersionMap)) {
                    $hasFailure = \true;
                    $results->addFailure("{$methodEntityId}@{$requiredVersion}", "{$methodEntityId} LanguageLevelTypeAware is missing entry for PHP {$requiredVersion}: expected '{$requiredType}'");
                    continue;
                }
                ...
                if ($normalizedActual !== $normalizedExpected) {
                    $hasFailure = \true;
                    $results->addFailure("{$methodEntityId}@{$requiredVersion}", "{$methodEntityId} LanguageLevelTypeAware entry for PHP {$requiredVersion}: expected '{$requiredType}', found '{$actualType}'");
                }
            }
        }
        if (!$hasFailure) {
