...
namespace Masjid_App\Dependencies\StubTests\Framework\Parsers\Stubs\Versions;

use Masjid_App\Dependencies\StubTests\Framework\Parsers\Stubs\PhpDoc\ParsedPhpDoc;
use Masjid_App\Dependencies\StubTests\Framework\Parsers\Stubs\Nodes\AttributeNode;
...
class DefaultAvailableVersionParser implements AvailableVersionParserInterface
{
    ...
    public function parseAvailableVersion(ParsedPhpDoc $phpDoc, array $attributes, array $imports = []): array
    {
        ...
        return ['sinceVersion' => $sinceVersion, 'removedVersion' => $removedVersion];
    }
    /**
    ...
    private function parsePhpStormStubsElementAvailable(array $attributes, array $imports): array
    {
        ...
        foreach ($attributes as $attribute) {
            ...
            if ($this->isPhpStormStubsElementAvailable($fullName)) {
                ...
                break;
                // Only process the first matching attribute
            }
        }
        return $result;
    }
    ...
    private function nextPhpVersion(string $version): string
    {
        ...
        $idx = array_search($version, $versions, \true);
    ...
    private function isPhpStormStubsElementAvailable(string $fullName): bool
    {
        ...
        return $fullName === 'JetBrains\PhpStorm\Internal\PhpStormStubsElementAvailable' || $fullName === 'PhpStormStubsElementAvailable' || str_ends_with($fullName, '\PhpStormStubsElementAvailable');
    }
}
