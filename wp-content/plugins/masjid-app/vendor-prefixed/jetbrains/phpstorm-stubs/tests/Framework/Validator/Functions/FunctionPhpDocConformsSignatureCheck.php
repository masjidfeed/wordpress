...
class FunctionPhpDocConformsSignatureCheck extends AbstractCallableCheck
{
    ...
    public function __construct(?ReflectionProviderInterface $reflectionProvider = null, ?KnownProblemsRegistry $knownProblemsRegistry = null, ?PhpDocConformanceService $conformanceService = null)
    {
    ...
    public function run(StubDataQueryInterface $stubs, string $entityId, string $phpVersion): CheckResultSet
    {
        ...
        $templateNames = $this->conformanceService->extractTemplateNames($stubFunction->getStubsMetadata()?->getPhpDoc());
        $mismatches = [];
        // Check return type
        ...
        if (!empty($mismatches)) {
            $results->addFailure($entityId, "Function {$entityId} PhpDoc/signature type mismatch in PHP {$phpVersion}: " . implode('; ', $mismatches));
            return $results;
        }
        $results->addSuccess($entityId);
        return $results;
