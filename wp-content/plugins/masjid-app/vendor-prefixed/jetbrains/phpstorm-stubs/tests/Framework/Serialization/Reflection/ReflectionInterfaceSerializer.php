...
namespace Masjid_App\Dependencies\StubTests\Framework\Serialization\Reflection;

use Masjid_App\Dependencies\StubTests\Framework\Serialization\EntityTypeSerializerInterface;
use Masjid_App\Dependencies\StubTests\Framework\Serialization\SubEntitySerializerTrait;
...
class ReflectionInterfaceSerializer implements EntityTypeSerializerInterface
{
    ...
    {
        $data = ['_type' => 'PHPInterface', 'name' => $this->toJsonSafe($entity->getName()), 'id' => $this->toJsonSafe($entity->getId())];
        ...
        foreach ($entity->getParentInterfaces() as $parentInterface) {
            $data['parentInterfaces'][] = $this->toJsonSafe($parentInterface->getId() ?? $parentInterface->getName());
        }
        return $data;
    }
