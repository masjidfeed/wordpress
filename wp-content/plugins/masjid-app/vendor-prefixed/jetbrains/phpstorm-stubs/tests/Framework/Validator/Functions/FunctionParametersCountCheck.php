...
class FunctionParametersCountCheck extends AbstractCallableCheck
{
    ...
    public function run(StubDataQueryInterface $stubs, string $entityId, string $phpVersion): CheckResultSet
    {
        ...
        $mismatch = ParameterCountComparator::compare($reflFunction->getParameters(), $stubFunction->getParameters(), $phpVersion);
        if ($mismatch !== null) {
            $results->addFailure($entityId, "Function {$entityId} parameter count mismatch in PHP {$phpVersion}: {$mismatch}");
            return $results;
        }
        $results->addSuccess($entityId);
        return $results;
