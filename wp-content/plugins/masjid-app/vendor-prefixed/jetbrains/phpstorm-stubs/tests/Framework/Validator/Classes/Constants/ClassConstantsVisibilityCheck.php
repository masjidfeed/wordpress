...
namespace Masjid_App\Dependencies\StubTests\Framework\Validator\Classes\Constants;

use Masjid_App\Dependencies\StubTests\Framework\Model\PHPClassConstant;
use Masjid_App\Dependencies\StubTests\Framework\Validator\AbstractConstantFlagCheck;
...
class ClassConstantsVisibilityCheck extends AbstractConstantFlagCheck
{
    ...
    protected function describeMismatch(string $constantEntityId, PHPClassConstant $reflConstant, PHPClassConstant $stubConstant, string $phpVersion): ?string
    {
        if ($reflConstant->getAccess() === $stubConstant->getAccess()) {
            return null;
        }
        return "Constant {$constantEntityId} has visibility '{$reflConstant->getAccess()->value}' in reflection " . "but '{$stubConstant->getAccess()->value}' in stubs";
    }
}
