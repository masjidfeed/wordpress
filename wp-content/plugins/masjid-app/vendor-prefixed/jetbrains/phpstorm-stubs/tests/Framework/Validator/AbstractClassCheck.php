...
use Masjid_App\Dependencies\StubTests\Framework\Validator\Contracts\LookupKind;
...
{
    private const FUNCTION_LOOKUP_UNSUPPORTED = ' operates on class-like entities but was' . ' configured with LookupKind::FUNCTION. Function checks must extend' . ' AbstractReflectionCheck (see EntityExistsCheck), which handles all four kinds.';
    ...
    public function __construct(?ReflectionProviderInterface $reflectionProvider = null, ?KnownProblemsRegistry $knownProblemsRegistry = null, ?EntityLookupService $entityLookup = null, ?MethodCollectionService $methodCollection = null, ?EntityTypeConfig $entityTypeConfig = null)
    {
    ...
    {
        if (($this->entityTypeConfig === null || $this->entityTypeConfig->lookupKind === LookupKind::CLASS_TYPE) && $entity instanceof PHPClass) {
