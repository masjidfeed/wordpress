...
class ReflectionTypeParser
{
    ...
    public function parse($reflectionType)
    {
        ...
        $isUnion = $reflectionType instanceof \ReflectionUnionType || method_exists($reflectionType, 'isUnionType') && $reflectionType->isUnionType();
        // Use duck typing to check for intersection types (supports both real Reflection and wrappers)
        $isIntersection = class_exists('\ReflectionIntersectionType') && $reflectionType instanceof \ReflectionIntersectionType || method_exists($reflectionType, 'isIntersectionType') && $reflectionType->isIntersectionType();
        if ($isUnion) {
    ...
    {
        $isIntersection = class_exists('\ReflectionIntersectionType') && $item instanceof \ReflectionIntersectionType || method_exists($item, 'isIntersectionType') && $item->isIntersectionType();
