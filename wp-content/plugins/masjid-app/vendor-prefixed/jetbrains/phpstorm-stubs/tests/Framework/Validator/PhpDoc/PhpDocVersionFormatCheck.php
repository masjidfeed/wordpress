...
class PhpDocVersionFormatCheck extends AbstractReflectionCheck
{
    ...
    public function run(StubDataQueryInterface $stubs, string $entityId, string $phpVersion): CheckResultSet
    {
        ...
        foreach ($violationsByLocation as $location => $versions) {
            $results->addFailure($location, "{$location} uses patch-level version in phpDoc in PHP {$phpVersion}: " . implode(', ', $versions));
        }
        return $results;
    }
    /**
    ...
    {
        return $this->collectByPhpDocLocation($entity, $entityId, fn(string $phpDoc): array => $this->findPatchVersions($phpDoc));
    }
    /**
