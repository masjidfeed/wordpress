...
namespace Masjid_App\Dependencies\StubTests\Framework\Validator\Classes\TypeForbidden;
...
use Masjid_App\Dependencies\StubTests\Framework\Validator\AbstractTypeForbiddenCheck;
...
class ClassMethodsNullableTypeForbiddenCheck extends AbstractTypeForbiddenCheck
{
    ...
    protected function collectMethodIssues(string $entityId, string $methodName, PHPMethod $method, string $phpVersion): array
    {
        ...
        if ($signatureReturnType instanceof NullableType) {
            ...
            $issues[$methodEntityId] = "{$this->getEntityLabel()} method {$methodEntityId} has nullable return type '{$returnType}' " . "but is available before PHP 7.1 (nullable type hints were introduced in PHP 7.1). " . "Use #[LanguageLevelTypeAware(['7.1' => '?...'], default: '...')] to restrict the nullable hint to PHP 7.1+.";
        }
        ...
        foreach ($this->methodCollection->filterAndDeduplicateParams($method->getParameters(), $phpVersion) as $param) {
            ...
            if ($declaredType instanceof NullableType) {
                ...
                $issues[$paramEntityId] = "{$this->getEntityLabel()} method {$methodEntityId} parameter \${$param->getName()} " . "has nullable type hint '{$paramType}' but the method is available before PHP 7.1 " . "(nullable type hints were introduced in PHP 7.1). " . "Use #[LanguageLevelTypeAware(['7.1' => '?...'], default: '...')] to restrict the nullable hint to PHP 7.1+.";
            }
        }
        return $issues;
    }
