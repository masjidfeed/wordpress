...
class ClassMethodsTentativeReturnTypeCheck extends AbstractMemberFlagCheck implements DescribesMethodMismatch
{
    ...
    public function describeMethodMismatch(string $methodEntityId, PHPMethod $reflMethod, PHPMethod $stubMethod, string $phpVersion): ?string
    {
        ...
        if ($reflTentative && !$stubTentative) {
            return "Method {$methodEntityId} has a tentative return type in PHP {$phpVersion}" . " but is not marked with #[TentativeType] in stubs";
        }
        return "Method {$methodEntityId} is marked with #[TentativeType] in stubs" . " but does not have a tentative return type in PHP {$phpVersion}";
    }
}
