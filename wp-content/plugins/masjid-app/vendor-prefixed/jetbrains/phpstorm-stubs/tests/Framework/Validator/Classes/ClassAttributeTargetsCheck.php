...
namespace Masjid_App\Dependencies\StubTests\Framework\Validator\Classes;
...
class ClassAttributeTargetsCheck extends AbstractClassCheck
{
     ...
     */
    private const ATOMIC_TARGET_NAMES = ['TARGET_CLASS', 'TARGET_FUNCTION', 'TARGET_METHOD', 'TARGET_PROPERTY', 'TARGET_CLASS_CONSTANT', 'TARGET_PARAMETER', 'TARGET_CONSTANT'];
    ...
    public function run(StubDataQueryInterface $stubs, string $entityId, string $phpVersion): CheckResultSet
    {
        ...
        foreach ([['reflection', $refl], ['stubs', $stub]] as [$side, $extracted]) {
            if ($extracted !== null && $extracted['flags'] === null) {
                $results->addFailure($entityId, sprintf('%s %s: could not resolve #[Attribute] flags from %s (%s). ' . 'Add the missing target constant to the map or fix the stub declaration.', $label, $entityId, $side, var_export($extracted['raw'], \true)));
                return $results;
            }
        }
        ...
        if ($reflTargets === null) {
            $results->addFailure($entityId, "{$label} {$entityId} is marked #[Attribute] in stubs but is not an attribute in PHP {$phpVersion}");
            return $results;
        }
        if ($stubTargets === null) {
            $results->addFailure($entityId, "{$label} {$entityId} is an attribute in PHP {$phpVersion} but is not marked #[Attribute] in stubs");
            return $results;
        }
        ...
        $results->addFailure($entityId, sprintf('%s %s #[Attribute] targets mismatch in PHP %s (%s). Reflection allows %s, stubs declare %s', $label, $entityId, $phpVersion, implode('; ', $detail), $this->describeTargets($reflTargets, $targetMap), $this->describeTargets($stubTargets, $targetMap)));
        return $results;
    }
    /**
        ...
        foreach ($attributes as $attribute) {
            ...
            foreach ([0, 'flags'] as $key) {
                if (array_key_exists($key, $arguments)) {
                    return ['flags' => $this->resolveFlags($arguments[$key], $targetMap), 'raw' => $arguments[$key]];
                }
            }
