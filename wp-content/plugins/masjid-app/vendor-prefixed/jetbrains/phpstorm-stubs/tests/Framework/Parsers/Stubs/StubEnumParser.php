...
use Masjid_App\Dependencies\StubTests\Framework\Parsers\Stubs\Nodes\EnumNode;
/**
...
class StubEnumParser implements MultiEntityStubParserInterface
{
    ...
    public function __construct(?EnumNodeExtractorInterface $nodeExtractor = null, ?PhpDocParserInterface $phpDocParser = null, ?AvailableVersionParserInterface $versionParser = null)
    {
    ...
    public function parseNode(EnumNode $node, array $imports = []): PHPEnum
    {
        ...
        $phpEnum->setIsFinal($node->isFinal());
        // Always true for enums
        $phpEnum->setIsReadonly(\false);
        // Enums are not readonly
        ...
        foreach ($node->getMethods() as $methodNode) {
            $phpEnum->addMethod($this->methodParser->parseNode($methodNode, $imports, $phpEnum->getNamespace(), $classTemplateNames));
        }
        // Cases
        ...
        foreach ($caseNames as $caseName) {
            StubConstantRegistry::register($phpEnum->getId() . '::' . $caseName, new StubEnumCaseReference($phpEnum->getId()));
        }
        return $phpEnum;
    }
    /**
