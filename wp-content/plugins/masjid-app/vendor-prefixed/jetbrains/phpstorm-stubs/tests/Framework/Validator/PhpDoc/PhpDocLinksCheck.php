...
namespace Masjid_App\Dependencies\StubTests\Framework\Validator\PhpDoc;
...
class PhpDocLinksCheck extends AbstractReflectionCheck
{
    ...
    public function __construct(?ReflectionProviderInterface $reflectionProvider = null, ?KnownProblemsRegistry $knownProblemsRegistry = null, ?callable $urlFetcher = null, ?bool $checkLiveness = null, ?EntityLookupService $entityLookup = null)
    {
    ...
    public static function fetchUrl(string $url): int
    {
        ...
        $ch = curl_init($url);
        curl_setopt_array($ch, [\CURLOPT_NOBODY => \true, \CURLOPT_RETURNTRANSFER => \true, \CURLOPT_FOLLOWLOCATION => \true, \CURLOPT_MAXREDIRS => 5, \CURLOPT_TIMEOUT => 10, \CURLOPT_CONNECTTIMEOUT => 5, \CURLOPT_USERAGENT => 'phpstorm-stubs-link-checker/1.0 (+https://github.com/JetBrains/phpstorm-stubs)', \CURLOPT_SSL_VERIFYPEER => \true, \CURLOPT_SSL_VERIFYHOST => 2]);
        curl_exec($ch);
        $code = curl_errno($ch) === \CURLE_OK ? (int) curl_getinfo($ch, \CURLINFO_HTTP_CODE) : 0;
    ...
    public function run(StubDataQueryInterface $stubs, string $entityId, string $phpVersion): CheckResultSet
    {
        ...
        foreach ($violationsByLocation as $location => $violations) {
            $results->addFailure($location, "{$location} has @link issues in PHP {$phpVersion}: " . implode('; ', $violations));
        }
        return $results;
    }
    /**
    ...
    {
        return $this->collectByPhpDocLocation($entity, $entityId, fn(string $phpDoc): array => $this->checkLinks($phpDoc, $checkLiveness));
    }
    /**
