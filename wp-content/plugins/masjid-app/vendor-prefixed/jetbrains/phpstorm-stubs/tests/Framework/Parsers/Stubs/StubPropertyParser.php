...
use Masjid_App\Dependencies\StubTests\Framework\Parsers\Stubs\Nodes\PropertyNode;
/**
...
class StubPropertyParser
{
    ...
    public function parseNode(PropertyNode $node, array $imports = [], string $namespace = '\\', array $classTemplateNames = []): PHPProperty
    {
        ...
        $parsedType = $this->typeParser->parseType($node->getType(), $parsedPhpDoc->varType, $node->getAttributes(), $imports, $namespace);
        ...
        if ($templateNames !== []) {
            ...
            $metadata?->setTypeFromPhpDoc(TemplateTypeNormalizer::unqualify($metadata->getTypeFromPhpDoc(), $templateNames));
        }
        return $property;
    }
    /**
