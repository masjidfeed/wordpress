...
class ClassParentClassCheck extends AbstractClassCheck
{
    ...
    public function __construct(?ReflectionProviderInterface $reflectionProvider = null, ?KnownProblemsRegistry $knownProblemsRegistry = null, ?ClassAncestorNamesExtractor $ancestorExtractor = null)
    {
    ...
    public function run(StubDataQueryInterface $stubs, string $entityId, string $phpVersion): CheckResultSet
    {
        ...
        if ($reflectionParentName === null) {
            ...
            $results->addFailure($entityId, "Parent class mismatch for {$entityId}: reflection has no parent, stubs have '{$stubDirectParentName}'");
            return $results;
        }
        ...
        if (in_array($reflectionParentName, $stubAncestors, \true)) {
            $results->addSuccess($entityId);
            return $results;
        }
        $stubDirectParentName = $stubClass->getParentClass() !== null ? $stubClass->getParentClass()->getName() : '(none)';
        $results->addFailure($entityId, "Parent class mismatch for {$entityId}: reflection has '{$reflectionParentName}', " . "stubs have '{$stubDirectParentName}'");
        return $results;
    }
}
