...
class ClassInterfacesCheck extends AbstractClassCheck
{
    ...
    public function __construct(?ReflectionProviderInterface $reflectionProvider = null, ?KnownProblemsRegistry $knownProblemsRegistry = null, ?ClassInterfaceFqnsExtractor $fqnsExtractor = null, ?EntityTypeConfig $entityTypeConfig = null)
    {
        parent::__construct(reflectionProvider: $reflectionProvider, knownProblemsRegistry: $knownProblemsRegistry, entityTypeConfig: $entityTypeConfig);
    ...
    public function run(StubDataQueryInterface $stubs, string $entityId, string $phpVersion): CheckResultSet
    {
        ...
        if (!empty($spurious)) {
            sort($spurious);
            $results->addFailure($entityId, "Interface mismatch for {$entityId}: stubs declare interface(s) not in reflection: " . implode(', ', $spurious));
            return $results;
        }
