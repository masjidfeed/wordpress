    ...
    protected function collectMethodIssues(string $entityId, string $methodName, PHPMethod $method, string $phpVersion): array
    {
        ...
        if ($signatureReturnType !== null && !$signatureReturnType instanceof NullableType) {
            ...
            if ($returnType !== '' && str_contains($returnType, '|')) {
                $issues[$methodEntityId] = "{$this->getEntityLabel()} method {$methodEntityId} has union return type '{$returnType}' " . "but is available before PHP 8.0 (union type hints were introduced in PHP 8.0). " . "Use #[LanguageLevelTypeAware(['8.0' => 'T1|T2'], default: '...')] to restrict the union hint to PHP 8.0+.";
            }
        }
        ...
        foreach ($this->methodCollection->filterAndDeduplicateParams($method->getParameters(), $phpVersion) as $param) {
            ...
            if ($paramType !== '' && str_contains($paramType, '|')) {
                ...
                $issues[$paramEntityId] = "{$this->getEntityLabel()} method {$methodEntityId} parameter \${$param->getName()} " . "has union type hint '{$paramType}' but the method is available before PHP 8.0 " . "(union type hints were introduced in PHP 8.0). " . "Use #[LanguageLevelTypeAware(['8.0' => 'T1|T2'], default: '...')] to restrict the union hint to PHP 8.0+.";
            }
        }
        return $issues;
    }
