...
use Masjid_App\Dependencies\StubTests\Framework\Parsers\Stubs\NodeExtractorInterface;
use Masjid_App\Dependencies\StubTests\Framework\Parsers\Stubs\Nodes\ClassNode;
use Masjid_App\Dependencies\StubTests\Framework\Parsers\Stubs\Nodes\FunctionNode;
/**
...
{
    private \Masjid_App\Dependencies\PhpParser\Parser $parser;
    /**
            ...
            if ($node instanceof Namespace_) {
                ...
                foreach ($node->stmts as $stmt) {
                    if ($stmt instanceof Class_) {
                        $classNode = new NikicClassNode($stmt);
                        $classNode->setNamespace($currentNamespace);
                        $classes[] = ['node' => $classNode, 'imports' => $currentImports];
                    }
                }
            } elseif ($node instanceof Class_) {
                $classNode = new NikicClassNode($node);
                $classNode->setNamespace('\\');
                $classes[] = ['node' => $classNode, 'imports' => $rootImports];
            }
        }
        return $classes;
    }
            ...
            if ($node instanceof Namespace_) {
                ...
                foreach ($node->stmts as $stmt) {
                    if ($stmt instanceof Interface_) {
                        $interfaceNode = new NikicInterfaceNode($stmt);
                        $interfaceNode->setNamespace($currentNamespace);
                        $interfaces[] = ['node' => $interfaceNode, 'imports' => $currentImports];
                    }
                }
            } elseif ($node instanceof Interface_) {
                $interfaceNode = new NikicInterfaceNode($node);
                $interfaceNode->setNamespace('\\');
                $interfaces[] = ['node' => $interfaceNode, 'imports' => $rootImports];
            }
        }
        return $interfaces;
    }
            ...
            if ($node instanceof Namespace_) {
                ...
                foreach ($node->stmts as $stmt) {
                    if ($stmt instanceof Enum_) {
                        $enumNode = new NikicEnumNode($stmt);
                        $enumNode->setNamespace($currentNamespace);
                        $enums[] = ['node' => $enumNode, 'imports' => $currentImports];
                    }
                }
            } elseif ($node instanceof Enum_) {
                $enumNode = new NikicEnumNode($node);
                $enumNode->setNamespace('\\');
                $enums[] = ['node' => $enumNode, 'imports' => $rootImports];
            }
        }
        return $enums;
    }
    ...
    private function extractDefinesFromStatements(array $stmts, string $namespace, array &$constants): void
    {
        foreach ($stmts as $stmt) {
            if ($stmt instanceof \Masjid_App\Dependencies\PhpParser\Node\Stmt\Expression && $stmt->expr instanceof FuncCall && $stmt->expr->name instanceof Name && $stmt->expr->name->toString() === 'define') {
    ...
    {
        foreach ($stmts as $stmt) {
            if ($stmt instanceof \Masjid_App\Dependencies\PhpParser\Node\Stmt\Const_) {
