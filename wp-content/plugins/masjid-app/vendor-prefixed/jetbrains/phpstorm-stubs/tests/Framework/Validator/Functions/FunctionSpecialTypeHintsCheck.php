...
class FunctionSpecialTypeHintsCheck extends AbstractCallableCheck
{
     ...
     */
    private const MIXED_FALSE_RETURN_FUNCTIONS = ['\end', '\prev', '\next', '\reset', '\current'];
    /**
     ...
     */
    private const MIXED_NULL_RETURN_FUNCTIONS = ['\array_pop', '\array_shift'];
    ...
    public function run(StubDataQueryInterface $stubs, string $entityId, string $phpVersion): CheckResultSet
    {
        ...
        $isMixedFalse = in_array($entityId, self::MIXED_FALSE_RETURN_FUNCTIONS, \true);
        $isMixedNull = in_array($entityId, self::MIXED_NULL_RETURN_FUNCTIONS, \true);
        ...
        if ($docReturnType === null || $docReturnType === '') {
            $results->addFailure($entityId, "Function {$entityId} must have '@return {$expectedPattern}' in PhpDoc " . "but the @return tag is missing. (see https://youtrack.jetbrains.com/issue/WI-57991)");
            return $results;
        }
        $parts = array_map('trim', explode('|', $docReturnType));
        $hasMixed = in_array('mixed', $parts, \true);
        $hasSecond = in_array($requiredSecond, $parts, \true);
        if (!$hasMixed || !$hasSecond) {
            ...
            $results->addFailure($entityId, "Function {$entityId} PhpDoc `@return` must contain '{$expectedPattern}' " . "but is missing: " . implode(', ', $missing) . " (got '{$docReturnType}'). (see https://youtrack.jetbrains.com/issue/WI-57991)");
            return $results;
        }
        $results->addSuccess($entityId);
        return $results;
