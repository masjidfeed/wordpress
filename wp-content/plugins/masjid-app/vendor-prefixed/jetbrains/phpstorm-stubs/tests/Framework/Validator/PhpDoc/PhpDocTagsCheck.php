...
class PhpDocTagsCheck extends AbstractReflectionCheck
{
    ...
    private const VALID_TAGS = [
        ...
        'removed',
        // marks when an entity was removed from PHP
        'xglobal',
        // documents PHP global variables
        'meta',
        // PhpStorm metadata hint
        'public',
    ];
    ...
    public function run(StubDataQueryInterface $stubs, string $entityId, string $phpVersion): CheckResultSet
    {
        ...
        foreach ($invalidTagsByLocation as $location => $tags) {
            $results->addFailure($location, "{$location} PhpDoc contains invalid tags in PHP {$phpVersion}: @" . implode(', @', $tags));
        }
        return $results;
    }
    /**
    ...
    {
        return $this->collectByPhpDocLocation($entity, $entityId, fn(string $phpDoc): array => $this->findInvalidTagNames($phpDoc));
    }
    /**
    ...
    private function findInvalidTagNames(string $phpDoc): array
    {
        ...
        foreach ($this->extractTagNames($phpDoc) as $tag) {
            if (!in_array($tag, self::VALID_TAGS, \true)) {
                $invalid[] = $tag;
            }
        }
