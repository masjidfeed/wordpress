...
namespace Masjid_App\Dependencies\StubTests\Framework\Serialization\Stubs;
...
use Masjid_App\Dependencies\StubTests\Framework\Serialization\EntityTypeSerializerInterface;
/**
...
class PHPEnumSerializer implements EntityTypeSerializerInterface
{
    ...
    public function serialize($entity, ?PhpDocRepository $phpDocStorage = null): array
    {
        ...
        $data = ['_type' => 'PHPEnum', 'name' => $this->toJsonSafe($entity->getName()), 'id' => $this->toJsonSafe($entity->getId()), 'isFinal' => $this->toJsonSafe($entity->isFinal()), 'isReadonly' => $this->toJsonSafe($entity->isReadonly()), 'sourcePath' => $this->toJsonSafe($entity->getStubsMetadata()?->getSourcePath()), 'duplicates' => $this->toJsonSafe($entity->getStubsMetadata()?->getDuplicates() ?? [])];
        ...
        if (isset($data['interfaces']) && is_array($data['interfaces'])) {
            foreach ($data['interfaces'] as $interfaceName) {
                if (!empty($interfaceName)) {
                    $interface = new \Masjid_App\Dependencies\StubTests\Framework\Model\PHPInterface();
