...
class ClassMethodsReturnTypeForbiddenCheck extends AbstractTypeForbiddenCheck
{
    ...
    protected function collectMethodIssues(string $entityId, string $methodName, PHPMethod $method, string $phpVersion): array
    {
        ...
        return [$methodEntityId => "{$this->getEntityLabel()} method {$methodEntityId} has return type '{$returnType}' " . "but is available before PHP 7.0 (return type hints were introduced in PHP 7.0). " . "Use #[LanguageLevelTypeAware(['7.0' => '...'], default: '')] to restrict the return type to PHP 7.0+."];
    }
}
