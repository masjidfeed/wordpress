...
class PHPFunctionSerializer implements EntityTypeSerializerInterface
{
    ...
    public function serialize($entity, ?PhpDocRepository $phpDocStorage = null): array
    {
        ...
        $data = ['_type' => 'PHPFunction', 'name' => $this->toJsonSafe($entity->getName()), 'id' => $this->toJsonSafe($entity->getId()), 'sourcePath' => $this->toJsonSafe($entity->getStubsMetadata()?->getSourcePath()), 'duplicates' => $this->toJsonSafe($entity->getStubsMetadata()?->getDuplicates() ?? [])];
