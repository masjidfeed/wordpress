...
namespace Masjid_App\Dependencies\StubTests\Framework\Tools;

use Exception;
use Masjid_App\Dependencies\PhpParser\Node;
use Masjid_App\Dependencies\PhpParser\NodeTraverser;
use Masjid_App\Dependencies\PhpParser\NodeVisitor;
...
(function (): void {
    ...
    $fileVisitor = new class extends NodeVisitorAbstract
    {
        ...
        public function enterNode(Node $node): ?int
        {
            ...
            if ($node instanceof Node\Expr\FuncCall) {
                ...
                if (count($node->args) === 3 && $node->args[2]->value instanceof Node\Expr\ConstFetch && $node->args[2]->value->name->toLowerString() === 'true') {
    ...
    foreach (scandir($phpStormStubsDirectory) as $topEntry) {
        ...
        if (in_array($topEntry, ['tests', 'meta', 'vendor', 'couchbase_v2', 'ds_v2'], \true)) {
            continue;
        }
        ...
        $phpFiles = StubFileScanner::collect($topPath, static fn(string $path, string $name): bool => preg_match('/\.php$/', $name) === 1, static fn(): bool => \true);
