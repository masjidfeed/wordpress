...
use Masjid_App\Dependencies\StubTests\Framework\PhpDoc\TemplateTypeNormalizer;
use Masjid_App\Dependencies\StubTests\Framework\Parsers\Stubs\Types\DefaultTypeParser;
use Masjid_App\Dependencies\StubTests\Framework\Parsers\Stubs\Types\TypeParserInterface;
...
use Masjid_App\Dependencies\StubTests\Framework\Parsers\Stubs\Nodes\MethodNode;
/**
...
class StubMethodParser
{
    ...
    public function __construct(?PhpDocParserInterface $phpDocParser = null, ?TypeParserInterface $typeParser = null, ?AvailableVersionParserInterface $versionParser = null)
    {
    ...
    public function parseNode(MethodNode $node, array $imports = [], string $namespace = '\\', array $classTemplateNames = []): PHPMethod
    {
        ...
        $parsedReturnType = $this->typeParser->parseType($node->getReturnType(), $parsedPhpDoc->returnType, $node->getAttributes(), $imports, $namespace);
        ...
        $templateNames = array_merge($classTemplateNames, TemplateTypeNormalizer::extractTemplateNames($parsedPhpDoc->rawPhpDoc));
    ...
    private function unqualifyTemplateTypes(PHPMethod $callable, array $templateNames): void
    {
        ...
        if ($returnMeta !== null) {
            $returnMeta->setTypeFromPhpDoc(TemplateTypeNormalizer::unqualify($returnMeta->getTypeFromPhpDoc(), $templateNames));
        }
        foreach ($callable->getParameters() as $param) {
            $paramMeta = $param->getStubsMetadata();
            if ($paramMeta !== null) {
                $paramMeta->setTypeFromPhpDoc(TemplateTypeNormalizer::unqualify($paramMeta->getTypeFromPhpDoc(), $templateNames));
            }
        }
    }
}
