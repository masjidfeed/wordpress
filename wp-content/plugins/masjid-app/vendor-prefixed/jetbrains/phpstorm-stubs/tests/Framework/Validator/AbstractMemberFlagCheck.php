...
{
    private const NO_DESCRIBER = ' extends AbstractMemberFlagCheck but implements neither' . ' DescribesMethodMismatch nor DescribesPropertyMismatch, so it has no comparison to run.';
     ...
     */
    private function describeMemberMismatch(string $memberId, mixed $reflectionMember, mixed $stubMember, string $phpVersion): ?string
    {
    ...
    private function collectStubMemberMap(PHPClassLikeObject $stubEntity, string $phpVersion): array
    {
        return match ($this->memberKind()) {
            ...
            MemberKind::PROPERTY => $stubEntity instanceof PHPClass ? $this->methodCollection->collectPropertiesForClass($stubEntity, $phpVersion) : [],
        };
    }
    ...
    public function run(StubDataQueryInterface $stubs, string $entityId, string $phpVersion): CheckResultSet
    {
        ...
        foreach ($this->memberKind()->reflectionMembers($reflectionEntity) as $reflMember) {
            ...
            $hasMismatch = \true;
            if (!$this->skipWithKnownProblem($results, $this->memberKind()->knownProblemEntityType(), $memberId, $this->getCheckName(), $phpVersion)) {
