...
class FunctionOptionalParametersCheck extends AbstractCallableCheck
{
    ...
    public function run(StubDataQueryInterface $stubs, string $entityId, string $phpVersion): CheckResultSet
    {
        ...
        $mismatches = OptionalParametersComparator::findMismatches($reflFunction->getParameters(), $stubFunction->getParameters(), $phpVersion);
        if (!empty($mismatches)) {
            $paramList = implode(', ', $mismatches);
            $results->addFailure($entityId, "Function {$entityId}: parameter(s) optional in PHP {$phpVersion} but not in stubs: {$paramList}");
            return $results;
        }
        $results->addSuccess($entityId);
        return $results;
