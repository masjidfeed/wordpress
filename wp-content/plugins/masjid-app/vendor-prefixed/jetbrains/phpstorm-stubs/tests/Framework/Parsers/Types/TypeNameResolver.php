...
namespace Masjid_App\Dependencies\StubTests\Framework\Parsers\Types;
...
{
    private const BUILT_IN_TYPES = ['int', 'string', 'bool', 'float', 'array', 'object', 'mixed', 'void', 'never', 'null', 'false', 'true', 'callable', 'iterable', 'resource', 'self', 'parent', 'static'];
    /**
    ...
    public function resolve(string $name, array $imports, string $namespace): string
    {
        ...
        if (in_array(strtolower($name), self::BUILT_IN_TYPES, \true)) {
            return $name;
        }
