...
class ClassMethodsScalarTypeForbiddenCheck extends AbstractTypeForbiddenCheck
{
    ...
    protected function collectMethodIssues(string $entityId, string $methodName, PHPMethod $method, string $phpVersion): array
    {
        ...
        foreach ($this->methodCollection->filterAndDeduplicateParams($method->getParameters(), $phpVersion) as $param) {
            ...
            $scalarCandidate = $declaredType instanceof NullableType ? $this->extractNullableInnerType($declaredType) : ($declaredType instanceof StandaloneType ? $declaredType->toString() : '');
            if ($scalarCandidate !== '' && in_array($scalarCandidate, self::SCALAR_TYPES, \true)) {
                $paramType = $declaredType->toString();
                $paramEntityId = $methodEntityId . '::$' . $param->getName();
                $issues[$paramEntityId] = "{$this->getEntityLabel()} method {$methodEntityId} parameter \${$param->getName()} " . "has scalar type hint '{$paramType}' but the method is available before PHP 7.0 " . "(scalar type hints were introduced in PHP 7.0). " . "Use #[LanguageLevelTypeAware(['7.0' => '...'], default: '')] to restrict the scalar hint to PHP 7.0+.";
            }
        }
        return $issues;
    }
