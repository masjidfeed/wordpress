...
class MultiFileJsonStorage implements ParsedDataPersistentStorageProvider
{
     ...
     */
    public function __construct(string $basePathToJsonFile, EntitySerializerInterface $serializer, bool $loadExisting = \true, ?PhpDocStorage $phpDocStorage = null)
    {
        ...
        foreach ($this->router->getAllFiles() as $fileId) {
            ...
            $this->fileStorages[$fileId] = new JsonParsedDataStorage($filePath, $serializer, $loadExisting, $phpDocStorage, ownsPhpDocStorage: \false);
        }
        if ($loadExisting) {
            $this->load();
        } else {
