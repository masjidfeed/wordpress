...
class EntityNamespaceCheck implements CheckInterface
{
    ...
    public function __construct(?LookupKind $lookupKind = null, ?string $label = null, ?EntityLookupService $entityLookup = null, ?EntityTypeConfig $entityTypeConfig = null)
    {
    ...
    public function run(StubDataQueryInterface $stubs, string $entityId, string $phpVersion): CheckResultSet
    {
        ...
        if ($stubNamespace !== $expectedNamespace) {
            ...
            $results->addFailure($entityId, "Namespace mismatch for {$label} {$entityId}: expected '" . ($expectedNamespace ?? '(no namespace)') . "', found '" . ($stubNamespace ?? '(no namespace)') . "' in stubs");
        } else {
