...
use Masjid_App\Dependencies\StubTests\Framework\Validator\Services\MethodCollectionService;
use Masjid_App\Dependencies\StubTests\Framework\Validator\Services\ParameterFilterHelper;
use Masjid_App\Dependencies\StubTests\Framework\Validator\Services\PhpDocConformanceService;
use Masjid_App\Dependencies\StubTests\Framework\Validator\Contracts\ReflectionProviderInterface;
use Masjid_App\Dependencies\StubTests\Framework\Validator\Services\ReturnTypeResolver;
/**
...
class ClassMethodsPhpDocConformsSignatureCheck extends AbstractClassCheck
{
    ...
    public function __construct(?ReflectionProviderInterface $reflectionProvider = null, ?KnownProblemsRegistry $knownProblemsRegistry = null, ?EntityLookupService $entityLookupService = null, ?MethodCollectionService $methodCollectionService = null, ?EntityTypeConfig $entityTypeConfig = null, ?PhpDocConformanceService $conformanceService = null)
    {
    ...
    public function run(StubDataQueryInterface $stubs, string $entityId, string $phpVersion): CheckResultSet
    {
        ...
        foreach ($this->collectEntityMethodsByConfig($stubEntity, $phpVersion) as $methodName => $method) {
            ...
            if (!$this->skipWithKnownProblem($results, EntityType::METHOD->value, $methodEntityId, CheckType::PHPDOC_CONFORMS_SIGNATURE, $phpVersion)) {
                $results->addFailure($methodEntityId, "{$this->getEntityLabel()} {$methodEntityId} PhpDoc/signature type mismatch in PHP {$phpVersion}: " . implode('; ', $mismatches));
            }
        }
        // Check properties
        foreach ($this->collectEntityPropertiesByConfig($stubEntity, $phpVersion) as $propName => $property) {
            ...
            if (!$this->conformanceService->isPhpDocCompatibleWithSignature($sigType, $docType, $templateNames)) {
                ...
                if (!$this->skipWithKnownProblem($results, EntityType::PROPERTY->value, $propEntityId, CheckType::PHPDOC_CONFORMS_SIGNATURE, $phpVersion)) {
                    $results->addFailure($propEntityId, "{$this->getEntityLabel()} {$propEntityId} PhpDoc/signature type mismatch in PHP {$phpVersion}: sig '{$sigType}', phpdoc '{$docType}'");
                }
            }
        }
        if (!$hasMismatch) {
    ...
    private function collectMethodMismatches(PHPMethod $method, string $phpVersion, array $templateNames = []): array
    {
        ...
        if ($sigReturnType !== null && $sigReturnType !== '' && $docReturnType !== null && $docReturnType !== '') {
