...
namespace Masjid_App\Dependencies\StubTests\Framework\MetaFile;

use Masjid_App\Dependencies\PhpParser\Node;
use Masjid_App\Dependencies\PhpParser\Node\Expr;
use Masjid_App\Dependencies\PhpParser\Node\Expr\BinaryOp\BitwiseOr;
...
final class MetaFileReferenceExtractor
{
    ...
    private const SKIP_NAMESPACES = ['Psr\Log\\', 'GuzzleHttp\\', 'Illuminate\\', 'PHPUnit\\', 'PHPUnit_', 'Mockery', 'Pest\\'];
    private const SKIP_FUNCTIONS = ['\mock', '\spy', '\namedMock', '\expect', '\jexit', '\wp_die', '\dd'];
    /** Constants defined inside PHPSTORM_META namespace, not real PHP symbols */
    private const SKIP_CONSTANTS = ['\ANY_ARGUMENT'];
    /**
    ...
    {
        return array_values(array_filter($this->extractAll($rootDir), static fn(MetaReference $ref) => $ref->role === MetaReferenceRole::CALLABLE));
    }
    /**
    ...
    {
        return array_values(array_filter($this->extractAll($rootDir), static fn(MetaReference $ref) => $ref->role === MetaReferenceRole::VALUE));
    }
    /**
    ...
    private function handleRegisterArgumentsSet(FuncCall $call, string $file, array &$refs, array &$setDefinitions): void
    {
        ...
        // Extract constant values from arg[1..N]
        $setUsages = [];
        // not collected in register context
        for ($i = 1, $count = count($args); $i < $count; $i++) {
            $this->extractValueRefs($args[$i]->value, $file, $refs, $setUsages);
        }
    }
    ...
    private function extractValueRefs(Expr $expr, string $file, array &$refs, array &$setUsages): void
    {
        ...
        if ($expr instanceof ConstFetch) {
            ...
            if (in_array($name, ['true', 'false', 'null', 'TRUE', 'FALSE', 'NULL'], \true)) {
                return;
            }
            $fqn = '\\' . $name;
    ...
    private function filterSkipped(array $refs): array
    {
        return array_values(array_filter($refs, function (MetaReference $ref): bool {
            ...
            if (in_array($fqn, self::SKIP_FUNCTIONS, \true)) {
                return \false;
            }
            // Skip PHPSTORM_META internal constants
            if (in_array($fqn, self::SKIP_CONSTANTS, \true)) {
                return \false;
            }
