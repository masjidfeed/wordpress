...
use Masjid_App\Dependencies\StubTests\Framework\Runner\PhpVersionManifest;
\define('DOCKER_TAGS_ENDPOINT', 'https://hub.docker.com/v2/repositories/library/php/tags');
\define('MAX_TAG_PAGES_PER_MINOR', 3);
...
if (\is_file($manifestFile)) {
    $decoded = \json_decode((string) \file_get_contents($manifestFile), \true);
    if (\is_array($decoded)) {
        $manifest = $decoded;
    }
}
...
foreach ($tracked as $minor) {
    ...
    if ($previous === null) {
        ...
        \fwrite(\STDERR, \sprintf("Warning: %s: newest image found (%s) is older than the recorded %s — keeping %s.\n", $minor, $latest, $previous, $previous));
...
{
    $best = null;
    $url = \DOCKER_TAGS_ENDPOINT . '?page_size=100&ordering=last_updated&name=' . \rawurlencode($minor . '.');
    $pages = 0;
...
function httpGet(string $url): ?string
{
    ...
    $context = \stream_context_create(['http' => ['method' => 'GET', 'header' => $headers, 'timeout' => 30], 'https' => ['method' => 'GET', 'header' => $headers, 'timeout' => 30]]);
...
function writeOutputs(array $updatedVersions, bool $hasChanges): void
{
    ...
    $written = \file_put_contents($out, 'updated_versions=' . \implode(',', $updatedVersions) . "\n" . 'has_changes=' . ($hasChanges ? 'true' : 'false') . "\n", \FILE_APPEND);
    if ($written === \false) {
