 ...
 */
namespace Masjid_App\Dependencies\Symfony\Component\VarDumper\Caster;

use Masjid_App\Dependencies\Symfony\Component\ErrorHandler\Exception\FlattenException;
use Masjid_App\Dependencies\Symfony\Component\ErrorHandler\Exception\SilencedErrorContext;
use Masjid_App\Dependencies\Symfony\Component\VarDumper\Cloner\Stub;
        ...
        if (isset($f['file'], $f['line'])) {
            ...
            if (isset(self::$framesCache[$cacheKey])) {
                ...
                if (is_file($f['file']) && 0 <= self::$srcContext) {
                    if (!empty($f['class']) && is_subclass_of($f['class'], 'Masjid_App\Dependencies\Twig\Template')) {
                        $template = null;
                        if (isset($f['object'])) {
                            $template = $f['object'];
                        } elseif ((new \ReflectionClass($f['class']))->isInstantiable()) {
                            $template = unserialize(\sprintf('O:%d:"%s":0:{}', \strlen($f['class']), $f['class']), ['allowed_classes' => \true]);
                        }
                        if (null !== $template) {
                            $ellipsis = 0;
