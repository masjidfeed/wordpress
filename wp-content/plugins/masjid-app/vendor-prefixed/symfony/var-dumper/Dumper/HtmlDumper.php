 ...
 */
namespace Masjid_App\Dependencies\Symfony\Component\VarDumper\Dumper;

use Masjid_App\Dependencies\Symfony\Component\VarDumper\Cloner\Cursor;
...
EOHTML
    ...
    protected function style(string $style, string $value, array $attr = []): string
    {
        ...
        $v = \sprintf('<span class=%s%s%1$s%s>%s</span>', 1 === \count($dumpClasses) ? '' : '"', implode(' ', $dumpClasses), $dumpTitle ? ' title="' . $dumpTitle . '"' : '', preg_replace_callback(static::$controlCharsRx, static function ($c) use ($map) {
