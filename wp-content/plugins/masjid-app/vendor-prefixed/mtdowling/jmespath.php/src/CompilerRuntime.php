...
namespace Masjid_App\Dependencies\JmesPath;
...
class CompilerRuntime
{
    ...
    {
        return 'jmespath_' . md5('jmespath:' . \PHP_MAJOR_VERSION . '.' . \PHP_MINOR_VERSION . ':' . self::CACHE_VERSION . ':' . $expression);
    }
    /** @internal */
    ...
    private function compile($filename, $expression, $functionName)
    {
        ...
        if (!file_put_contents($tempFile, $code)) {
            throw new \RuntimeException(sprintf('Unable to write the compiled PHP code to: %s (%s)', $tempFile, var_export(error_get_last(), \true)));
        }
        if (!rename($tempFile, $filename)) {
            @unlink($tempFile);
            if (!file_exists($filename)) {
                throw new \RuntimeException("Unable to move the compiled PHP code to: {$filename}");
            }
            // Another process won the race; its file is equivalent.
        } elseif (function_exists('opcache_invalidate')) {
            opcache_invalidate($filename, \true);
        }
    }
}
