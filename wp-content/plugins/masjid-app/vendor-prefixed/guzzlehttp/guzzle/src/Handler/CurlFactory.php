...
namespace Masjid_App\Dependencies\GuzzleHttp\Handler;
...
use Masjid_App\Dependencies\GuzzleHttp\Multiplexing;
use Masjid_App\Dependencies\GuzzleHttp\Promise as P;
use Masjid_App\Dependencies\GuzzleHttp\Promise\FulfilledPromise;
use Masjid_App\Dependencies\GuzzleHttp\Promise\PromiseInterface;
use Masjid_App\Dependencies\GuzzleHttp\Psr7;
use Masjid_App\Dependencies\GuzzleHttp\Psr7\LazyOpenStream;
use Masjid_App\Dependencies\GuzzleHttp\Psr7\Uri;
use Masjid_App\Dependencies\GuzzleHttp\TransferStats;
use Masjid_App\Dependencies\GuzzleHttp\TransportSharing;
use Masjid_App\Dependencies\GuzzleHttp\Utils;
...
class CurlFactory implements CurlFactoryInterface
{
    ...
    private static function triggerConflictingCurlOptionDeprecations(array $options): void
    {
        ...
        foreach ($options['curl'] as $option => $_) {
            ...
            if ($replacement !== null) {
                \Masjid_App\Dependencies\trigger_deprecation('guzzlehttp/guzzle', $since, \sprintf('Passing %s in the "curl" request option is deprecated; guzzlehttp/guzzle 8.0 will reject this option because it conflicts with Guzzle-managed request handling. Use %s instead.', $name, $replacement));
                continue;
            }
            \Masjid_App\Dependencies\trigger_deprecation('guzzlehttp/guzzle', $since, \sprintf('Passing %s in the "curl" request option is deprecated; guzzlehttp/guzzle 8.0 will reject this option because it conflicts with Guzzle-managed cURL internals.', $name));
        }
    }
    private static function triggerUnsupportedCurlOptionDeprecations(array $options): void
    {
        ...
        foreach ($options['curl'] as $option => $_) {
            ...
            \Masjid_App\Dependencies\trigger_deprecation('guzzlehttp/guzzle', '7.12', \sprintf('Passing %s in the "curl" request option is deprecated; guzzlehttp/guzzle 8.0 will reject raw cURL options outside the built-in cURL handlers\' allow-list.', self::formatCurlOption($option)));
        }
    }
    private static function triggerUnsupportedRequestOptionDeprecations(array $options): void
    {
        if (\array_key_exists('stream_context', $options)) {
            \Masjid_App\Dependencies\trigger_deprecation('guzzlehttp/guzzle', '7.11', 'Passing the "stream_context" request option to a cURL handler is deprecated; guzzlehttp/guzzle 8.0 will reject this option because cURL handlers ignore PHP stream context options.');
        }
    }
    /**
