...
use Masjid_App\Dependencies\GuzzleHttp\Promise\EachPromise;
use Masjid_App\Dependencies\GuzzleHttp\Promise\PromiseInterface;
use Masjid_App\Dependencies\GuzzleHttp\Promise\PromisorInterface;
...
class Pool implements PromisorInterface
{
    ...
    public function __construct(ClientInterface $client, $requests, array $config = [])
    {
        ...
        if (!\is_iterable($requests)) {
            \Masjid_App\Dependencies\trigger_deprecation('guzzlehttp/guzzle', '7.11', 'Passing a non-iterable request collection to %s::__construct() or %s::batch() is deprecated; guzzlehttp/guzzle 8.0 will require an iterable.', __CLASS__, __CLASS__);
            $requests = [$requests];
        }
