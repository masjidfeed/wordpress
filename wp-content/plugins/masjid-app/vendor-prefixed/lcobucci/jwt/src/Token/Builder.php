...
use Masjid_App\Dependencies\Lcobucci\JWT\Builder as BuilderInterface;
use Masjid_App\Dependencies\Lcobucci\JWT\ClaimsFormatter;
use Masjid_App\Dependencies\Lcobucci\JWT\Encoder;
use Masjid_App\Dependencies\Lcobucci\JWT\Encoding\CannotEncodeContent;
...
final class Builder implements BuilderInterface
{
    ...
    {
        return $this->encoder->base64UrlEncode($this->encoder->jsonEncode($items));
    }
    public function getToken(Signer $signer, Key $key): UnencryptedToken
    {
        ...
        return new Plain(new DataSet($headers, $encodedHeaders), new DataSet($this->claims, $encodedClaims), new Signature($signature, $encodedSignature));
    }
}
