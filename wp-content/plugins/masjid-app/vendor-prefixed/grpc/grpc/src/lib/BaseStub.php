 ...
 *
 */
namespace Masjid_App\Dependencies\Grpc;
...
class BaseStub
{
    ...
    public function __construct($hostname, $opts, $channel = null)
    {
        ...
        if ($channel) {
            if (!is_a($channel, 'Grpc\Channel') && !is_a($channel, 'Masjid_App\Dependencies\Grpc\Internal\InterceptorChannel')) {
    ...
    private static function updateOpts($opts)
    {
        ...
        if (defined('Masjid_App\Dependencies\Grpc\VERSION')) {
            $version_str = \Masjid_App\Dependencies\Grpc\VERSION;
        } else {
    ...
    {
        if (is_a($channel, 'Masjid_App\Dependencies\Grpc\Internal\InterceptorChannel')) {
            return function ($method, $argument, $deserialize, array $metadata = [], array $options = []) use ($channel) {
