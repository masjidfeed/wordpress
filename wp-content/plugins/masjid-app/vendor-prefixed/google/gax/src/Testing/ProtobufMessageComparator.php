...
use Masjid_App\Dependencies\SebastianBergmann\Comparator\Comparator;
use Masjid_App\Dependencies\SebastianBergmann\Comparator\ComparisonFailure;
use Masjid_App\Dependencies\SebastianBergmann\Exporter\Exporter;
/**
 * @internal
 */
class ProtobufMessageComparator extends Comparator
{
    ...
    public function assertEquals($expected, $actual, $delta = 0, $canonicalize = \false, $ignoreCase = \false)
    {
        if (get_class($expected) === get_class($actual)) {
            ...
            try {
                if (json_decode($expected->serializeToJsonString(), \true) == json_decode($actual->serializeToJsonString(), \true)) {
                    return;
                }
            } catch (\Throwable $e) {
