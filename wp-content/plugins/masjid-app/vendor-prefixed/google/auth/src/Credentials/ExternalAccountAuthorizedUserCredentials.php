...
class ExternalAccountAuthorizedUserCredentials extends CredentialsLoader implements GetQuotaProjectInterface
{
     ...
     */
    public function __construct(string|array|null $scope, array $jsonKey)
    {
        ...
        $this->auth = new OAuth2(['refresh_token' => $jsonKey['refresh_token'], 'tokenCredentialUri' => $jsonKey['token_url'], 'scope' => $scope]);
    ...
    public function fetchAuthToken(?callable $httpHandler = null, array $headers = [])
    {
        ...
        return $this->auth->fetchAuthToken($httpHandler, $this->applyTokenEndpointMetrics($headers, 'at'));
    }
    /**
    ...
    public function updateMetadata($metadata, $authUri = null, ?callable $httpHandler = null)
    {
        ...
        if ($this->enableRegionalAccessBoundary) {
            $metadata = $this->updateRegionalAccessBoundaryMetadata($metadata, $this->buildRegionalAccessBoundaryLookupUrl(), $this->getUniverseDomain(), $httpHandler);
        }
        return $metadata;
    }
    /**
    ...
    {
        return hash('sha256', implode('.', [$this->clientId, $this->auth->getScope(), $this->auth->getRefreshToken()]));
    }
    /**
     * @return array<mixed>
     */
