...
namespace Masjid_App\Dependencies\Google\Auth\Credentials;

use Masjid_App\Dependencies\Google\Auth\CacheTrait;
use Masjid_App\Dependencies\Google\Auth\GetUniverseDomainInterface;
use Masjid_App\Dependencies\Google\Auth\HttpHandler\HttpClientCache;
...
use Masjid_App\Dependencies\GuzzleHttp\Psr7\Request;
...
trait RegionalAccessBoundaryTrait
{
     ...
     */
    private function getRegionalAccessBoundary(string $universeDomain, callable $httpHandler, string $regionalAccessBoundaryUrl, array $headers): array|null
    {
        ...
        $regionalAccessBoundary = $this->lookupRegionalAccessBoundary($httpHandler, $regionalAccessBoundaryUrl, $headers['authorization']);
     ...
     * @return array<mixed>
     */
    private function updateRegionalAccessBoundaryMetadata(array $headers, string $regionalAccessBoundaryUrl, string $universeDomain, ?callable $httpHandler): array
    {
        $httpHandler = $httpHandler ?: HttpHandlerFactory::build(HttpClientCache::getHttpClient());
        $regionalAccessBoundaryInfo = $this->getRegionalAccessBoundary($universeDomain, $httpHandler, $regionalAccessBoundaryUrl, $headers);
     ...
     */
    private function buildRegionalAccessBoundaryLookupUrl(?string $serviceAccountEmail = null, ?string $poolId = null, ?string $projectNumber = null): string
    {
        $baseUrl = 'https://iamcredentials.googleapis.com/v1';
        if ($serviceAccountEmail) {
            if (is_null($projectNumber) && is_null($poolId)) {
                return sprintf('%s/projects/-/serviceAccounts/%s/allowedLocations', $baseUrl, $serviceAccountEmail);
            }
        } elseif ($poolId) {
            if (is_null($projectNumber)) {
                // Workforce Identity Pools
                return sprintf('%s/locations/global/workforcePools/%s/allowedLocations', $baseUrl, $poolId);
            }
            // Workload Identity Pools
            return sprintf('%s/projects/%s/locations/global/workloadIdentityPools/%s/allowedLocations', $baseUrl, $projectNumber, $poolId);
        }
     ...
     * @return null|array{locations: array<string>, encodedLocations: string}
     */
    private function lookupRegionalAccessBoundary(callable $httpHandler, string $regionalAccessBoundaryUrl, array $authHeader): array|null
    {
        ...
        $regionalAccessBoundary = json_decode((string) $response->getBody(), \true);
        if (null === $regionalAccessBoundary) {
    ...
    private function initiateCooldown(): void
    {
        ...
        $cooldownBackoff = 15 * 60;
        // 15 minutes
        $cooldownMax = 6 * 60 * 60;
        // 6 hours
        $cooldownPeriod = min(++$attempt * $cooldownBackoff, $cooldownMax);
        $this->setCachedValue($cooldownKey, \true, (int) $cooldownPeriod);
        $this->setCachedValue($cooldownKey . ':attempt', $attempt, (int) $cooldownPeriod * 2);
    }
}
