 ...
 */
namespace Masjid_App\Dependencies\Google\Auth;

use DomainException;
use Masjid_App\Dependencies\Google\Auth\Credentials\AppIdentityCredentials;
use Masjid_App\Dependencies\Google\Auth\Credentials\GCECredentials;
use Masjid_App\Dependencies\Google\Auth\Credentials\ImpersonatedServiceAccountCredentials;
use Masjid_App\Dependencies\Google\Auth\Credentials\ServiceAccountCredentials;
use Masjid_App\Dependencies\Google\Auth\Credentials\UserRefreshCredentials;
...
use Masjid_App\Dependencies\Google\Auth\Logging\StdOutLogger;
use Masjid_App\Dependencies\Google\Auth\Middleware\AuthTokenMiddleware;
use Masjid_App\Dependencies\Google\Auth\Middleware\ProxyAuthTokenMiddleware;
use Masjid_App\Dependencies\Google\Auth\Subscriber\AuthTokenSubscriber;
...
class ApplicationDefaultCredentials
{
     ...
     * @throws DomainException if no implementation can be obtained.
     */
    public static function getCredentials($scope = null, ?callable $httpHandler = null, ?array $cacheConfig = null, ?CacheItemPoolInterface $cache = null, $quotaProject = null, $defaultScope = null, ?string $universeDomain = null, null|false|LoggerInterface $logger = null, bool $enableRegionalAccessBoundary = \false)
    {
        $creds = null;
        ...
        if (!is_null($jsonKey)) {
            ...
            $creds = CredentialsLoader::makeCredentials($scope, $jsonKey, $defaultScope, $enableRegionalAccessBoundary);
        } elseif (AppIdentityCredentials::onAppEngine() && !GCECredentials::onAppEngineFlexible()) {
            $creds = new AppIdentityCredentials($anyScope);
        } elseif (self::onGce($httpHandler, $cacheConfig, $cache)) {
            $creds = new GCECredentials(scope: $anyScope, quotaProject: $quotaProject, universeDomain: $universeDomain, enableRegionalAccessBoundary: $enableRegionalAccessBoundary);
    ...
    public static function getIdTokenCredentials($targetAudience, ?callable $httpHandler = null, ?array $cacheConfig = null, ?CacheItemPoolInterface $cache = null)
    {
        ...
        if (!is_null($jsonKey)) {
            ...
            $creds = match ($jsonKey['type']) {
                ...
                'impersonated_service_account' => new ImpersonatedServiceAccountCredentials(scope: null, jsonKey: $jsonKey, targetAudience: $targetAudience),
                'service_account' => new ServiceAccountCredentials(scope: null, jsonKey: $jsonKey, targetAudience: $targetAudience),
