...
namespace Masjid_Feed\Dependencies\Firebase\JWT;
...
class JWT
{
    ...
    {
        if (!\function_exists('sodium_crypto_sign_verify_detached') && !\function_exists('Masjid_Feed\Dependencies\sodium_crypto_sign_verify_detached')) {
    ...
    private static function signPS256(
        #[\SensitiveParameter]
        string|OpenSSLAsymmetricKey|OpenSSLCertificate $key,
        string $message
    ): string
    {
        if (!class_exists('Masjid_Feed\Dependencies\phpseclib3\Crypt\RSA')) {
        ...
        $rsa = \Masjid_Feed\Dependencies\phpseclib3\Crypt\PublicKeyLoader::load($key);
        return $rsa->withPadding(\Masjid_Feed\Dependencies\phpseclib3\Crypt\RSA::SIGNATURE_PSS)->withHash('sha256')->sign($message);
    }
    /**
    ...
    ): bool
    {
        ...
        $rsa = \Masjid_Feed\Dependencies\phpseclib3\Crypt\PublicKeyLoader::load($key);
        return $rsa->withPadding(\Masjid_Feed\Dependencies\phpseclib3\Crypt\RSA::SIGNATURE_PSS)->withHash('sha256')->verify($message, $signature);
    }
}
