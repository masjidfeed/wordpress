...
class Masjid_Feed_Push_Notifications {

    const MAX_ATTEMPTS = 5;

    public function __construct() {
        add_action('transition_post_status', array($this, 'maybe_enqueue_first_publish'), 20, 3);
        add_action('masjidfeed_maybe_enqueue_first_publish', array($this, 'enqueue_first_publish'));
        add_action('masjidfeed_process_push_job', array($this, 'process_job'));
        add_action('masjidfeed_cleanup_push_jobs', array($this, 'cleanup_jobs'));
        add_action('add_meta_boxes_post', array($this, 'add_meta_box'));
        add_action('admin_post_masjidfeed_resend_push', array($this, 'handle_resend'));
        add_action('admin_post_masjidfeed_retry_push', array($this, 'handle_retry'));
    }
    ...
    public function maybe_enqueue_first_publish($new_status, $old_status, $post) {
        ...
        wp_schedule_single_event(time() + 1, 'masjidfeed_maybe_enqueue_first_publish', array((int) $post->ID));
    }
    ...
    public function enqueue($post_id, $trigger_type, $dedupe_key = null) {
        ...
        if (!$inserted) {
            return new WP_Error('masjidfeed_push_not_enqueued', __('The notification was already queued or could not be saved.', 'masjidfeed-app'));
        }
    ...
    public function process_job($job_id) {
        ...
        try {
            ...
            if ($retry) {
                wp_schedule_single_event(time() + $delay, 'masjidfeed_process_push_job', array((int) $job_id));
            }
        }
    }
    ...
    public function add_meta_box() {
        add_meta_box('masjidapp-push', __('MasjidFeed App Push', 'masjidfeed-app'), array($this, 'render_meta_box'), 'post', 'side');
    }

    public function render_meta_box($post) {
        if (!$this->post_is_eligible($post->ID)) {
            echo '<p>' . esc_html__('Assign the configured push tag to enable notifications.', 'masjidfeed-app') . '</p>';
            return;
        }
        if ('publish' !== $post->post_status) {
            echo '<p>' . esc_html__('Publish this post before sending a notification.', 'masjidfeed-app') . '</p>';
            return;
        }
        $url = wp_nonce_url(admin_url('admin-post.php?action=masjidfeed_resend_push&post_id=' . $post->ID), 'masjidfeed_resend_push_' . $post->ID);
        echo '<p><a class="button" href="' . esc_url($url) . '">' . esc_html__('Send notification', 'masjidfeed-app') . '</a></p>';
    }

    public function handle_resend() {
        $post_id = absint($_GET['post_id'] ?? 0);
        if (!current_user_can('edit_post', $post_id) || !check_admin_referer('masjidfeed_resend_push_' . $post_id)) {
            wp_die(esc_html__('You are not allowed to send this notification.', 'masjidfeed-app'));
        }
        $post = get_post($post_id);
        if (!$post || 'publish' !== $post->post_status || !$this->post_is_eligible($post_id)) {
            wp_die(esc_html__('Only tagged, published posts can send notifications.', 'masjidfeed-app'));
        }
    ...
    public function handle_retry() {
        ...
        if (!current_user_can('manage_options') || !check_admin_referer('masjidfeed_retry_push_' . $job_id)) {
    ...
    private function post_is_eligible($post_id) {
        ...
        $tag_id = absint(Masjid_Feed_Settings::get_option('push_tag'));
    ...
    private function is_retryable($exception) {
        return $exception instanceof Masjid_Feed_Firebase_Exception && $exception->is_retryable();
    }

    private function get_retry_delay($exception, $attempts) {
        $retry_after = $exception instanceof Masjid_Feed_Firebase_Exception ? $exception->retry_after() : null;
